...
MV_SPD_AUTHORITY_ARCHIVE_PATH = ROOT / "woek-parlament-app/data/state-programmes/fach-reviews/mecklenburg-vorpommern-2026-spd-p1-p54-authority-index-v1.json"
...
def validate() -> dict:
    ...
    mv_spd_authority_archive = load(MV_SPD_AUTHORITY_ARCHIVE_PATH)
    ...
    require(berlin_fach["summary"]["remaining_page_review_envelopes"] == 1189, "ISSUE_241_BE_FACH_ENVELOPE_COUNT_DRIFT")
    require(berlin_fach["summary"]["remaining_exact_effect_objects_identified"] == 0, "ISSUE_241_BE_FACH_EXACT_OBJECT_COUNT_DRIFT")
    require(berlin_fach["summary"]["remaining_review_scope_count"] == 1189, "ISSUE_241_BE_FACH_SCOPE_COUNT_DRIFT")
    require(berlin_fach["summary"]["remaining_exact_effect_object_count"] is None, "ISSUE_241_BE_FALSE_EFFECT_OBJECT_COUNT")
    spd = next(item for item in berlin_fach["programmes"] if item["party"] == "SPD")
    spd_scope = spd["protected_fach_scope"]
    berlin_state = matrix["current_state"]["berlin"]
    require(berlin_state["spd_materialised_terminal_pages"] == spd_scope["materialized_page_set"] == [22, 23, 24, 25, 26], "ISSUE_241_BE_SPD_TERMINAL_PAGE_DRIFT")
    require(berlin_state["spd_next_source_order_frontier"] == spd_scope["next_unreviewed_source_order_frontier"], "ISSUE_241_BE_SPD_FRONTIER_DRIFT")
    require(berlin_state["spd_cross_page_objects_consumed_once"] == spd_scope["consumed_cross_page_objects"], "ISSUE_241_BE_SPD_CROSS_PAGE_DRIFT")
    p25 = load(ROOT / "woek-parlament-app/data/state-programmes/fach-reviews/berlin-2026-spd-p25-explicit-v1.json")
    require(berlin_state["spd_p25_remaining_source_objects"] == p25["coverage"]["remaining_p25_source_object_ids"] == [], "ISSUE_241_BE_SPD_P25_RESIDUAL")
    p26 = load(ROOT / "woek-parlament-app/data/state-programmes/fach-reviews/berlin-2026-spd-p26-explicit-v1.json")
    require(berlin_state["spd_p26_remaining_source_spans"] == p26["coverage"]["uncovered_source_spans"] == [], "ISSUE_241_BE_SPD_P26_RESIDUAL")
    require(p26["coverage"]["terminal_pages"] == [26] and not p26["constraints"]["p27_authorised"], "ISSUE_241_BE_SPD_P26_SCOPE_DRIFT")
    for field, source_field in [("remaining_review_envelopes", "remaining_page_review_envelopes"), ("remaining_review_scopes", "remaining_review_scope_count")]:
        require(berlin_state[field] == berlin_fach["summary"][source_field], "ISSUE_241_BE_CURRENT_STATE_SET_DRIFT")
    ...
    require(current_readiness["blocking_lanes"]["berlin"]["remaining_review_envelopes"] == 1189, "ISSUE_241_CURRENT_GOLDEN_BE_RESIDUAL_DRIFT")
    ...
    require(current_readiness["blocking_lanes"]["berlin"]["remaining_review_scopes"] == 1189, "ISSUE_241_CURRENT_GOLDEN_BE_SCOPE_DRIFT")
    ...
    require(fach[0]["remaining_review_envelopes"] == 1189, "ISSUE_241_BE_FACH_FINITE_ENVELOPE_DRIFT")
    require(fach[0]["remaining_exact_effect_objects_identified"] == 0, "ISSUE_241_BE_FACH_FINITE_EXACT_OBJECT_DRIFT")
    require(fach[0]["remaining_review_scopes"] == 1189, "ISSUE_241_BE_FACH_FINITE_SCOPE_DRIFT")
    ...
    return {
        ...
        "mv_spd_materialised_terminal_pages": len(mv_spd["summary"]["materialised_terminal_pages"]),
        "mv_spd_authority_pointer_gaps": matrix["current_state"]["mecklenburg_vorpommern"]["spd_authority_pointer_gap_count"],
