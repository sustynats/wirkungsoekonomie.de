...
async function openReview(id){
 ...
 if(['AWAITING_FINAL_APPROVAL','APPROVED_FOR_PUBLICATION','HOLD','NEEDS_REVIEW'].includes(r.status)){
  const label=element('label','Kommentar an die Redaktion','field'),comment=element('textarea');comment.rows=3;comment.maxLength=10000;label.append(comment);mount.append(label);
