#!/usr/bin/env bash
# Regenerates the public URL list from _site and diffs it against the
# protected baseline (reports/url-baseline.txt). Output must be empty:
# any "<" line = URL removed (forbidden), any ">" line = URL added (ok, but note it).
