...
<p>Ein Projekt des Wirkungsinstituts. <a href="/institut/projekte/umfragen/">Entwicklung, Aufgaben und nächste Umfrage ansehen</a>.</p>
<section aria-label="Öffentliche Umfragen">${publicPolls.length?publicPolls.map(p=>`<article class="poll-card" data-poll-list-slug="${esc(p.slug)}"><p class="poll-kicker" data-poll-list-status aria-live="polite">${labels[p.effective_status||p.status]||''}</p><h2><a href="/umfragen/${esc(p.slug)}/">${esc(p.title)}</a></h2><p>${esc(p.intro)}</p><a class="btn btn-primary" href="/umfragen/${esc(p.slug)}/">Umfrage öffnen</a></article>`).join('\n'):'<p>Zurzeit ist keine öffentliche Umfrage verfügbar.</p>'}</section>
