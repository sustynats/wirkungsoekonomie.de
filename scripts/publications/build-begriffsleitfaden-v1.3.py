#!/usr/bin/env python3
"""Build the cumulative v1.3 terminology guide without changing older editions."""
...
def main() -> None:
    ...
    deltas = f"\n\n{DELTA_12.read_text(encoding='utf-8').strip()}\n\n{DELTA_13.read_text(encoding='utf-8').strip()}"
    source = source.replace(marker, f"{deltas}{marker}", 1)
    ...
    try:
        os.environ.update({
            ...
            "WOEK_PUBLICATION_PDF": "public/downloads/originals/WOeK_Begriffsleitfaden_fuehrend_v1.3.pdf",
            "WOEK_PUBLICATION_TITLE": "Führender Begriffsleitfaden der Wirkungsökonomie",
            "WOEK_PUBLICATION_EDITION": "Version 1.3 · Stand 30. Juli 2026 · Führendes Referenzdokument",
        })
