...
const docs = {
  ...
  hostingDossier: {
    ...
    download: "assets/pdf/wirkungsraeume-gestalten-hosting.pdf",
    relatedTerms: [
...
function downloads(base, entries) {
  ...
  return `<section class="section" aria-labelledby="downloads"><div class="card"><p class="hero-kicker">Dossier & Export</p>${h2("downloads", "Downloads und Druck")}<p>Online-Volltext ist der Hauptzugang. PDF- und Exportfassungen bleiben ergänzende Downloads.</p><div class="portal-card-actions no-print"><button class="btn btn-secondary" type="button" onclick="window.print()" aria-label="Diese Seite drucken">Seite drucken</button>${links.join("")}</div></div></section>`;
}
