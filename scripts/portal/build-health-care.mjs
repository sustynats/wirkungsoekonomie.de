...
const documents = [
  ...
  },
  {
    ...
    pdf: "woek_gesundheit_pflege_einzeldossier_set_v0_3.pdf",
    archivePdf: "woek_gesundheit_pflege_einzeldossier_set_v0_2.pdf",
...
const twoBalanceExample = JSON.parse(read(`${SOURCE}/rechenbeispiel-zwei-bilanzen-v0_3.json`));
...
function downloadButton(doc, base, className = "btn btn-secondary") {
  const filename = doc.pdf || doc.download;
  if (!filename || !exists(`assets/downloads/${filename}`)) return "";
  const label = doc.pdf ? `${doc.shortTitle} als PDF v0.3` : `${doc.shortTitle} herunterladen`;
  return `<a class="${className}" href="${href(base, `assets/downloads/${filename}`)}">${esc(label)}</a>`;
}

function correctionNotice(doc, base) {
  if (!doc.pdf || !doc.archivePdf || !exists(`assets/downloads/${doc.archivePdf}`)) return "";
  return `<aside class="reference-term-notice"><strong>Rechenlogik v0.3</strong><p>Die aktuelle Fassung führt zwei getrennte Darstellungen: einen diskontierten Saldo in EUR und ein nichtmonetäres Wirkungsprofil. Lebensqualität, Autonomie, Teilhabe und Resilienz werden nicht in Geld umgerechnet. Die frühere <a href="${href(base, `assets/downloads/${doc.archivePdf}`)}">PDF v0.2</a> bleibt als historische Quellenfassung erreichbar. Für T-SROI gilt der <a href="${href(base, "werkzeuge/impact-controlling/methodenpapiere/t-sroi-transformationsmessung/")}">Rechenstandard v1.1</a> mit <a href="${href(base, "quellenarchiv/wok-q-1024/")}">WÖK-Q-1024</a>.</p></aside>`;
}

function twoBalanceExampleSection() {
  const example = twoBalanceExample;
  const required = ["title", "label", "assumptions", "monetary", "profile", "gate"];
  const missing = required.filter((key) => !example[key]);
  if (missing.length) throw new Error(`Rechenbeispiel v0.3 unvollständig: ${missing.join(", ")}`);
  const profile = String(example.profile).replace(/^Nichtmonetäres Wirkungsprofil:\s*/u, "");
  const gate = String(example.gate).replace(/^Schutz-Gate:\s*/u, "");
  return `<section class="section" id="zwei-bilanzen-beispiel" aria-labelledby="zwei-bilanzen-beispiel-title"><div class="card"><p class="hero-kicker">Nachvollziehbares Modell</p>${h2("zwei-bilanzen-beispiel-title", example.title)}<p><strong>${esc(example.label)}</strong></p><p><strong>Annahmen:</strong> ${esc(example.assumptions)}</p><p><strong>Monetäre Bilanz:</strong> ${esc(String(example.monetary).replace(/^Monetäre Bilanz:\s*/u, ""))}</p><p><strong>Nichtmonetäres Wirkungsprofil:</strong> ${esc(profile)}</p><p><strong>Schutz-Gate:</strong> ${esc(gate)}</p></div></section>`;
}

function downloads(base) {
  const links = documents.map((doc) => downloadButton(doc, base)).join("");
  return `<section class="section" aria-labelledby="downloads"><div class="card"><p class="hero-kicker">Arbeitsmaterial</p>${h2("downloads", "Downloads und Druck")}<p>Die Seite bleibt der Einstieg. Download- und Druckfassungen sind ergänzende Arbeitsmaterialien für Vertiefung, Zitation und Weiterarbeit.</p><div class="portal-card-actions no-print"><button class="btn btn-secondary" type="button" onclick="window.print()" aria-label="Diese Seite drucken">Seite drucken</button>${links}</div></div></section>`;
}
