...
function modulePage([slug, title, summary, toolTitle], index) {
  ...
  page({
    ...
    body: (base) => `<section class="hero portal-hero"><div class="hero-grid"><div><nav class="breadcrumb"><a href="${href(base, "index.html")}">Start</a> / <a href="${href(base, "wirkungsfelder/wissenschaft-innovation-digitalisierung/")}">Wissenschaft, Innovation & Digitalisierung</a></nav><p class="hero-kicker">Unterbereich · Onlinefassung</p><h1>${esc(title)}</h1><p class="hero-subtitle">${esc(summary)}</p><div class="hero-actions no-print"><button class="btn btn-secondary" type="button" onclick="window.print()" aria-label="Diese Seite drucken">Seite drucken</button><a class="btn btn-primary" href="#detailkonzept">Konzeptabschnitt anzeigen</a><a class="btn btn-secondary" href="#einzeldossier">Dossierabschnitt anzeigen</a></div></div><aside class="card"><p class="card-kicker">Primäres Werkzeug</p><h2 class="card-title">${esc(toolTitle)}</h2><p class="card-text">${esc(tool[3])}</p><a class="text-link" href="${href(base, `werkzeuge/${tool[0]}/`)}">Methodik lesen</a></aside></div></section><details class="toc-card no-print" aria-label="Inhaltsverzeichnis"><summary class="card-title">Auf dieser Seite</summary><ol>${[...t1, ...t2, ...t3].map((item) => `<li class="toc-level-${esc(item.level)}"><a href="#${esc(item.id)}">${esc(item.text)}</a></li>`).join("")}</ol></details><section class="section" aria-labelledby="kurzfassung"><div class="prose">${h2("kurzfassung", "Kurzfassung")}${overviewHtml}</div></section><section class="section" id="detailkonzept" aria-labelledby="detailkonzept-title"><div class="prose">${h2("detailkonzept-title", "Konzeptpapier")}${detailHtml}</div></section><section class="section" id="einzeldossier" aria-labelledby="einzeldossier-title"><div class="prose">${h2("einzeldossier-title", "Dossier")}${dossierHtml}</div></section>${toolGrid(base)}${politicalBlock()}${referenceBlock(base)}${bookBlock(base)}${relatedBlock(base)}${protectionBlock()}${sourcesBlock()}${downloads(base)}`,
  });
}
