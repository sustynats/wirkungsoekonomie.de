...
function modulePage(module, index) {
  ...
  page({
    ...
    type: "Unterbereich",
    body: (base) => `<section class="hero portal-hero"><div class="hero-content"><nav class="breadcrumb"><a href="${href(base, "index.html")}">Start</a> / <a href="${href(base, "wirkungsfelder/rente-soziale-sicherung/")}">Rente & soziale Sicherung</a></nav><p class="hero-kicker">Rente & soziale Sicherung</p><h1>${esc(module.title)}</h1><p class="hero-subtitle">${esc(module.shift)}</p><p>${esc(module.problem)}</p><div class="hero-actions no-print"><button class="btn btn-secondary" type="button" onclick="window.print()" aria-label="Diese Seite drucken">Seite drucken</button><a class="btn btn-primary" href="#detailkonzept">Konzeptabschnitt anzeigen</a><a class="btn btn-secondary" href="#dossier">Dossierabschnitt anzeigen</a></div></div></section>${publicationAccess(base, [{ kicker: "Konzeptpapier", title: "Konzeptpapier lesen", text: "Die fachliche Langfassung dieses Unterbereichs ist direkt auf dieser Seite eingebunden.", href: "#detailkonzept", label: "Konzeptabschnitt anzeigen" }, { kicker: "Dossier", title: "Dossier lesen", text: "Anwendung, Annahmen, Bewertungslogik, politische Optionen und Grenzen.", href: "#dossier", label: "Dossierabschnitt anzeigen" }, { kicker: "Download", title: "Konzeptpapier-PDF", text: "PDF-Fassung der langen Konzeptpapiere.", href: docs.detail.download, label: "PDF herunterladen" }, { kicker: "Download", title: "Dossier-Sammlung", text: "PDF-Fassung der Dossiers.", href: docs.singleDossier.download, label: "PDF herunterladen" }])}${tocBlock([...detail.toc, ...dossier.toc])}<section class="section" aria-labelledby="kurzfassung"><div class="section-header"><p class="hero-kicker">Kurzfassung</p>${h2("kurzfassung", "Kurzfassung")}</div><div class="card-grid three"><article class="card"><h3 class="card-title">Alte Logik</h3><p class="card-text">${esc(module.problem)}</p></article><article class="card"><h3 class="card-title">Perspektivwechsel</h3><p class="card-text">${esc(module.shift)}</p></article><article class="card"><h3 class="card-title">Schutzlinie</h3><p class="card-text">Bewertet werden Systeme, Regeln, Fonds, Nachweise und Wirkungsräume, nicht der Wert von Menschen. Keine automatisierte Letztentscheidung.</p></article></div></section><section class="section article-section" aria-labelledby="detailkonzept"><article class="article-body fulltext-reader"><p class="hero-kicker">Konzeptpapier</p>${h2("detailkonzept", "Konzeptpapier")}${detail.html}</article></section><section class="section article-section" aria-labelledby="dossier"><article class="article-body fulltext-reader"><p class="hero-kicker">Dossier</p>${h2("dossier", "Dossier")}${dossier.html}</article></section>${toolGrid(base)}${crossLinkBlock(base)}${politicalBlock()}${referenceBlock(base)}${bookBlock(base)}${sourceBlock(base)}${downloads(base, [{ label: "Konzeptpapier PDF", href: docs.detail.download }, { label: "Dossier-Sammlung PDF", href: docs.singleDossier.download }])}`,
  });
}
