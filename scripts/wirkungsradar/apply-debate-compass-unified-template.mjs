...
const VERSION = "20260612-mobile-headline-fix";
const MAIN_JS_VERSION = "20260612-mobile-headline-fix";
...
function sourceCards(section) {
  const links = [...String(section || "").matchAll(/<a\b[^>]*href="(https?:\/\/[^"]+)"[^>]*>([\s\S]*?)<\/a>/gi)]
    ...
    .slice(0, 8);
  if (!links.length) {
    return `<article class="card source-proof-card"><p class="v2-badge">Interne WÖk-Quelle</p><h3 class="card-title"><a class="text-link" href="/wirkungsradar/quellen/">Quellenhub des Debatten-Kompasses</a></h3><p class="card-text"><strong>Belegt hier:</strong> Die Seite ist als Debatten-Kompass-Karte eingeordnet und muss externe Tatsachenbehauptungen mit konkreten Quellen am Seitenende belegen.</p><p class="card-text"><strong>Grenze:</strong> Der Quellenhub ersetzt keine externe Primärquelle für einzelne Zahlen, Rechtsfragen oder tagesaktuelle Behauptungen.</p><p class="card-text"><strong>Datenstand / Prüfung:</strong> 2026-06-05 · strukturell geprüft</p><p><a class="text-link" href="/wirkungsradar/quellen/">Quelle öffnen</a></p></article>`;
  }
  return links.map((source) => `<article class="card source-proof-card"><p class="v2-badge">${esc(sourceType(source.href, source.label))}</p><h3 class="card-title"><a class="text-link" href="${esc(source.href)}">${esc(source.label)}</a></h3><p class="card-text"><strong>Belegt hier:</strong> ${esc(source.proof.replace(/^Belegt:\s*/i, ""))}</p><p class="card-text"><strong>Grenze:</strong> Die Quelle belegt den genannten Prüfpunkt, ersetzt aber keine vollständige Folgen- und Wirkungsabwägung.</p><p class="card-text"><strong>Datenstand / Prüfung:</strong> 2026-06-05 · Link strukturell geprüft</p><p><a class="text-link" href="${esc(source.href)}">Quelle öffnen</a></p></article>`).join("");
}
