...
function resonanceIndexPage() {
  const main = `<section class="hero radar-page-hero"><div><nav class="breadcrumb" aria-label="Breadcrumb"><a href="../../index.html">Start</a> / <a href="../">Öffentlicher Wirkungsraum</a></nav><p class="hero-kicker">Resonanz-Kompass</p><h1 class="hero-title">Welche Themen bekommen Aufmerksamkeit - und passt das zu ihrer wirklichen Bedeutung?</h1><p class="hero-subtitle">Der Resonanz-Kompass prüft nicht, ob über ein Thema gesprochen werden darf. Er zeigt, was die öffentliche Gewichtung bewirkt: Was wird laut? Was bleibt leise? Welche Ursache verschwindet?</p></div></section>${radarNav("../")}
...
function resonanceDetailPage(card) {
  ...
  const main = `<section class="hero radar-page-hero"><div><nav class="breadcrumb" aria-label="Breadcrumb"><a href="../../../index.html">Start</a> / <a href="../../">Öffentlicher Wirkungsraum</a> / <a href="../">Resonanz-Kompass</a></nav><p class="hero-kicker">Resonanzkarte</p><h1 class="hero-title">${esc(card.title)}</h1><p class="hero-subtitle">${esc(card.subtitle || "")}</p><p class="meta-line">${esc(relationLabel(card))} · Datenstand: ${DATA_STAND}</p></div></section>${radarNav("../../")}
...
function agendaPage() {
  ...
  const main = `<section class="hero radar-page-hero"><div><nav class="breadcrumb" aria-label="Breadcrumb"><a href="../../index.html">Start</a> / <a href="../">Öffentlicher Wirkungsraum</a></nav><p class="hero-kicker">Agenda-Radar</p><h1 class="hero-title">Welche wichtigen Wirkungsfragen bekommen zu wenig Raum?</h1><p class="hero-subtitle">Öffentliche Aufmerksamkeit ist begrenzt. Wenn laute Themen alles überdecken, verschwinden oft die Fragen, die langfristig am meisten bewirken.</p></div></section>${radarNav("../")}
...
function causesIndexPage() {
  const main = `<section class="hero radar-page-hero"><div><nav class="breadcrumb" aria-label="Breadcrumb"><a href="../../index.html">Start</a> / <a href="../">Öffentlicher Wirkungsraum</a></nav><p class="hero-kicker">Ursachen-Navigator</p><h1 class="hero-title">Vom Aufreger zur Ursache.</h1><p class="hero-subtitle">Viele Debatten drehen sich um Symptome. Der Ursachen-Navigator fragt, was darunter liegt: falsche Preise, fehlende Rückkopplung, kaputte Infrastruktur, schlechte Daten, Wohnkosten, Vertrauensverlust oder Plattformlogik.</p></div></section>${radarNav("../")}
...
function causeDetailPage(cause) {
  ...
  const main = `<section class="hero radar-page-hero"><div><nav class="breadcrumb" aria-label="Breadcrumb"><a href="../../../index.html">Start</a> / <a href="../">Ursachen</a></nav><p class="hero-kicker">Ursachen-Navigator</p><h1 class="hero-title">${esc(cause.title)}</h1><p class="hero-subtitle">${esc(cause.text)}</p></div></section>${radarNav("../../")}
...
function trapsPage() {
  const main = `<section class="hero radar-page-hero"><div><nav class="breadcrumb" aria-label="Breadcrumb"><a href="../../index.html">Start</a> / <a href="../">Öffentlicher Wirkungsraum</a></nav><p class="hero-kicker">Musterbibliothek</p><h1 class="hero-title">Nicht jedes Stöckchen verdient einen Sprung.</h1><p class="hero-subtitle">Aufmerksamkeitsfallen verengen Debatten: Ein Einzelfall ersetzt die Ursache. Kosten werden ohne Gegenkosten gezeigt. Eine Zahl wird zum Ohnmachtsbeweis.</p></div></section>${radarNav("../")}
...
function resiliencePage() {
  ...
  const main = `<section class="hero radar-page-hero"><div><nav class="breadcrumb" aria-label="Breadcrumb"><a href="../../index.html">Start</a> / <a href="../">Öffentlicher Wirkungsraum</a></nav><p class="hero-kicker">Resilienz-Prinzipien</p><h1 class="hero-title">Wie Öffentlichkeit widerstandsfähiger gegen Empörung, Ablenkung und Manipulation wird.</h1><p class="hero-subtitle">Eine Demokratie ist nicht resilient, weil sie keine Konflikte hat. Sie ist resilient, wenn sie trotz Konflikten noch prüfen, unterscheiden, korrigieren und lernen kann.</p></div></section>${radarNav("../")}
...
function publicImpactRoomPage() {
  const main = `<section class="hero radar-page-hero"><div><nav class="breadcrumb" aria-label="Breadcrumb"><a href="../index.html">Start</a></nav><p class="hero-kicker">Öffentlicher Wirkungsraum</p><h1 class="hero-title">Debatten verstehen. Aufmerksamkeit gewichten. Demokratische Resilienz stärken.</h1><p class="hero-subtitle">Viele Debatten scheitern nicht erst an falschen Fakten. Sie kippen früher: durch Frames, Empörung, Ablenkung und falsche Gewichtung. Der öffentliche Wirkungsraum zeigt, was ein Narrativ behauptet, warum es Aufmerksamkeit bekommt, welche Ursachen es verdeckt und wie Öffentlichkeit widerstandsfähiger wird.</p></div></section>${radarNav("../wirkungsradar/")}
  <section class="section"><div><article class="card important-card"><p class="card-kicker">Auf einen Blick</p><h2>Antwort, Resonanz, Agenda, Ursache.</h2><p>Wenn eine Aussage schon im Raum steht, hilft der Debatten-Kompass. Wenn eine Debatte den Raum beherrscht, hilft der Resonanz-Kompass. Wenn wichtige Themen kaum vorkommen, hilft das Agenda-Radar. Wenn Aufregung immer wieder an derselben Stelle entsteht, führt der Ursachen-Navigator zur Systemfrage.</p></article></div></section>
  <section class="section section-soft"><div><div class="section-header"><p class="hero-kicker">Was brauchst du gerade?</p><h2>Der schnellste Einstieg.</h2></div>${cardGrid([["Ich habe eine konkrete Aussage gehört.", "Debatten-Kompass", "Wenn ein Satz schon im Raum steht: Was wird behauptet? Welcher Frame steckt dahinter? Wie antworte ich, ohne in die Falle zu laufen?", "wirkungsradar/live/"], ["Ein Thema dominiert gerade alles.", "Resonanz-Kompass", "Wenn alle über dasselbe reden: Ist das Thema wirklich so wichtig oder bindet es Aufmerksamkeit, die woanders fehlt?", "wirkungsradar/resonanz-kompass/"], ["Ich frage mich, was gerade übersehen wird.", "Agenda-Radar", "Welche Wirkungsfragen sind wichtig, aber leise? Welche Probleme haben hohes Wirkungsgewicht, aber zu wenig öffentliche Aufmerksamkeit?", "wirkungsradar/agenda-radar/"], ["Ich will zur Ursache.", "Ursachen-Navigator", "Vom Aufreger zur Systemfrage: Welche Ursache liegt unter der Oberfläche? Welche Hebel würden wirklich etwas verändern?", "wirkungsradar/ursachen-navigator/"], ["Ich will resiliente Debatten.", "Resilienz-Prinzipien", "Wie bleibt Öffentlichkeit prüffähig, lernfähig und handlungsfähig, auch wenn Empörung, Angst und Manipulation zunehmen?", "wirkungsradar/resilienz-prinzipien/"]], ([question, title, text, href]) => `<article class="card"><p class="card-kicker">${esc(question)}</p><h2>${esc(title)}</h2><p>${esc(text)}</p><p><a class="btn btn-secondary" href="../${href}">Öffnen</a></p></article>`)}</div></section>
  <section class="section"><div><article class="card important-card"><p class="card-kicker">Zwei Werkzeuge, zwei Ebenen</p><h2>Debatten-Kompass = Antwortqualität. Resonanz-Kompass = Aufmerksamkeitsqualität.</h2><p>Der Debatten-Kompass hilft, wenn ein Narrativ schon wirkt. Der Resonanz-Kompass setzt früher an: Er fragt, warum genau dieses Thema so viel Raum bekommt, was es verdrängt und ob die Aufmerksamkeit zu seiner tatsächlichen Wirkung passt.</p></article></div></section>`;
  return shell({ title: "Öffentlicher Wirkungsraum", description: "Debatten-Kompass, Resonanz-Kompass, Agenda-Radar und Ursachen-Navigator der Wirkungsökonomie.", canonical: `${SITE_URL}/oeffentlicher-wirkungsraum/`, base: "../", main });
}
