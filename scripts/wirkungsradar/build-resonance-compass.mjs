...
const debateCardsMaster = JSON.parse(fs.readFileSync(path.join(ROOT, "content/wirkungsradar/debattenkarten-master.json"), "utf8"));
const debateCardCount = (debateCardsMaster.cards || []).filter((card) => !card.redirectTarget).length;
...
function shell({ title, description, canonical, base = "../../", main, extraHead = "" }) {
  const optionalHead = extraHead ? `  ${extraHead}\n` : "";
  return `<!doctype html>
<html lang="de">
<head>
  <meta charset="utf-8">
  ...
  <link rel="canonical" href="${esc(canonical)}">
${optionalHead}  <link rel="stylesheet" href="${base}assets/css/style.css?v=20260606-nav-cache-fix">
</head>
<body>
...
function publicImpactRoomPage() {
  const impactNav = `<nav class="radar-subnav" aria-label="Öffentlicher Wirkungsraum Navigation" data-search-exclude><a class="pill-link" href="#verstehen">Verstehen</a><a class="pill-link" href="#anwenden">Anwenden</a><a class="pill-link" href="#vertiefen">Vertiefen</a><a class="pill-link" href="../wirkungsradar/debattenkarten/">Debattenkarten</a><a class="pill-link" href="../wirkungsradar/narrative/">Narrative</a><a class="pill-link" href="../wirkungsradar/">Debatten-Kompass</a><a class="pill-link" href="../wirkungsradar/narrativ-einreichen/">Einreichen</a></nav>`;
  const understandCards = [
    ["Fünf Wellen", "Wie öffentliche Wirkung entsteht.", "Ein Satz wird sichtbar, aktiviert Gefühle, bietet eine Deutung an, findet Resonanz und kann Debatten, Vertrauen oder Entscheidungen verschieben.", "#fuenf-wellen"],
    ["Tiefe", "Warum diese Wirkung anschlussfähig wird.", "Unter der Oberfläche liegen Erfahrungen, Anreize, Institutionen, Plattformlogiken, Machtverhältnisse und ungelöste Systemfragen.", "#tiefe"],
  ];
  const entryCards = [
    ["Ich habe eine konkrete Aussage gehört.", "Debatten-Kompass", "Der Debatten-Kompass ist der Werkzeugbereich für konkrete Aussagen: Er führt dich von Behauptung und Frame zur Sofortantwort, zum Folgencheck und zu Quellen.", "wirkungsradar/", "Debatten-Kompass öffnen"],
    ["Ich suche eine geprüfte Antwort.", "Debattenkarten", `Debattenkarten sind die einzelnen Antworten im Debatten-Kompass. Hier findest du alle ${debateCardCount} Karten nach Aussage, Thema, Narrativ und Antwortbedarf.`, "wirkungsradar/debattenkarten/", "Alle Debattenkarten öffnen"],
    ["Ich sehe ein wiederkehrendes Narrativ.", "Narrative", "Welche Geschichte wird erzählt, was soll sie auslösen und wie kann ich den Frame ruhig drehen?", "wirkungsradar/narrative/", "Narrative öffnen"],
    ["Ich brauche ein Antwortformat.", "Antwort-Playbooks", "Kurze Antworten für Gespräch, Kommentarspalte, Panel, Interview oder Host-Situation.", "wirkungsradar/antwort-playbooks/", "Playbooks öffnen"],
  ...
  const readingLevels = [
    ["1. Aufmerksamkeitswelle", "Was wird sichtbar?", "Eine Aussage erzeugt Sichtbarkeit und rückt etwas in den Fokus."],
    ["2. Emotionswelle", "Was wird gefühlt?", "Die Aussage löst Gefühle aus - von Hoffnung über Empörung bis Angst."],
    ["3. Deutungswelle", "Was bedeutet es?", "Menschen ordnen ein, geben Bedeutung und bilden Meinungen."],
    ["4. Resonanzwelle", "Wer greift es auf?", "Die Aussage wird geteilt, diskutiert, verstärkt oder widersprochen."],
    ["5. Verschiebungswelle", "Was verändert sich?", "Einstellungen, Entscheidungen, Normen und Handlungen können sich verschieben."],
  ];
  const discourseCards = cards.slice(0, 4).map((card) => [
    card.cluster || "Öffentlicher Wirkungsraum",
    card.title,
    card.subtitle || relationLabel(card),
    `wirkungsradar/resonanz-kompass/${card.slug}/`,
    "Einordnung öffnen",
  ]);
  const depthItems = causePages.slice(0, 6).map((cause) => [
    "Tiefe",
    cause.title,
    cause.question,
    `wirkungsradar/ursachen-navigator/${cause.slug}/`,
    "Ursache prüfen",
  ]);
  const vertiefenCards = [
    ["Dossier", "Der öffentliche Wirkungsraum: Stein, Wellen und Tiefe", "Langfassung zum Modell: Aussage, Resonanz, Aufmerksamkeit, Ursache, Wellen und demokratische Resilienz.", "dossier-stein-wellen-tiefe/", "Onlinefassung lesen"],
    ["PDF", "Langdossier herunterladen", "Die PDF-Fassung als Arbeits- und Vertiefungsdokument.", "../assets/downloads/woek_dossier_oeffentlicher_wirkungsraum_wellen_tiefe_v0_1.pdf", "PDF herunterladen"],
    ["Journal", "Beiträge zum öffentlichen Wirkungsraum", "Journalartikel, Einordnungen und neue Texte zum Debatten- und Resonanzraum.", "../blog.html", "Journal öffnen"],
    ["Glossar", "Begriffe sauber verwenden", "Wirkung, Wirkungspotenzial, Resonanzraum, Wirkpfad, Wirkungsrisiko und weitere Begriffe nachschlagen.", "../begriffe/", "Glossar öffnen"],
    ["Akademie", "Lernen und Einreichen", "Vertiefen, Fragen stellen und Narrative redaktionell einreichen.", "https://akademie.wirkungsoekonomie.de/narrativ-einreichen/", "Akademie öffnen"],
    ["Forschung", "Methode und Quellen prüfen", "Methodik, Schutzlinien, Quellenlogik und offene Forschungsfragen einordnen.", "../wirkungsradar/methode/", "Methode öffnen"],
  ];
  const main = `<section class="hero radar-page-hero"><div><nav class="breadcrumb" aria-label="Breadcrumb"><a href="../index.html">Start</a></nav><p class="hero-kicker">Die fünf Wellen öffentlicher Wirkung</p><h1 class="hero-title">Öffentlicher Wirkungsraum</h1><p class="hero-subtitle">Antworten finden. Narrative einordnen. Debatten wirksamer führen. Öffentliche Aussagen wirken nicht nur über Fakten. Sie treffen auf Erfahrungen, Ängste, Loyalitäten, Plattformlogiken und politische Interessen. Der Öffentliche Wirkungsraum hilft, konkrete Aussagen schnell einzuordnen, passende Antworten zu finden und die tieferen Wirkpfade sichtbar zu machen.</p><div class="hero-audio" aria-label="Audio: Öffentlicher Wirkungsraum"><label class="audio-label" for="oeffentlicher-wirkungsraum-audio">Anhören: Öffentlicher Wirkungsraum</label><audio id="oeffentlicher-wirkungsraum-audio" controls preload="metadata" aria-label="Anhören: Öffentlicher Wirkungsraum"><source src="../assets/audio/explanations/oeffentlicher-wirkungsraum.mp3" type="audio/mpeg">Dein Browser kann diese Audiodatei nicht direkt abspielen.</audio><p class="audio-transcript-note">Audio-Einführung verfügbar.</p></div></div></section>
  ${impactNav}
  <section class="section" id="verstehen"><div><div class="section-header"><p class="hero-kicker">Ebene 1 · Verstehen</p><h2>Erst die Logik. Dann das Werkzeug.</h2><p>Wer eine Aussage beantworten will, muss zuerst sehen, worauf er antwortet: auf Fakten, auf ein Gefühl, auf einen Frame, auf Resonanz - oder auf eine tiefere Systemfrage.</p></div>${cardGrid(understandCards, ([kicker, title, text, href]) => `<article class="card"><p class="card-kicker">${esc(kicker)}</p><h3>${esc(title)}</h3><p>${esc(text)}</p><p><a class="text-link" href="${esc(href)}">Mehr verstehen</a></p></article>`)}</div></section>
  ...
  <figure class="woek-visual-figure woek-visual-figure--compact"><img class="woek-visual" src="../assets/img/generated/oeffentlicher-wirkungsraum-fuenf-wellen.webp" alt="Infografik zum Wellen- und Tiefenmodell des öffentlichen Wirkungsraums: Aussage, Aufmerksamkeitswelle, Emotionswelle, Deutungswelle, Verschiebungswelle und darunterliegende Ursachenräume."><figcaption class="woek-visual-caption">Nicht der einzelne Stein verändert den öffentlichen Raum allein, sondern die Wellen - und die Tiefe darunter: Erfahrungen, Machtstrukturen, Medienlogik, Interessen, Institutionen und Vertrauen.</figcaption></figure>
  <section class="section section-soft" id="fuenf-wellen"><div><div class="section-header"><p class="hero-kicker">Modell in 30 Sekunden</p><h2>Die fünf Wellen öffentlicher Wirkung.</h2><p>Die neue Bezeichnung benennt die fünf Schritte ausdrücklich: Aufmerksamkeitswelle, Emotionswelle, Deutungswelle, Resonanzwelle und Verschiebungswelle. Das Modell ordnet öffentliche Dynamik, ohne Debatten zu bewerten oder Menschen zu etikettieren.</p></div>${cardGrid(readingLevels, ([kicker, title, text]) => `<article class="card"><p class="card-kicker">${esc(kicker)}</p><h3>${esc(title)}</h3><p>${esc(text)}</p></article>`)}</div></section>
  <section class="section" id="tiefe"><div><div class="section-header"><p class="hero-kicker">Tiefe</p><h2>Warum konnte es so wirken?</h2><p>Unter der sichtbaren Aussage liegen oft Erfahrungen, Anreize, Infrastruktur, Verfahren, Plattformlogiken und Machtverhältnisse. Die Tiefe fragt nach dem Hebel, der den Zustand wirklich verändert.</p></div>${cardGrid(depthItems, ([kicker, title, text, href, label]) => `<article class="card" data-radar-card data-topic="Tiefe" data-search="${attr([title, text, kicker].join(" "))}"><p class="card-kicker">${esc(kicker)}</p><h3>${esc(title)}</h3><p>${esc(text)}</p><p><a class="btn btn-secondary" href="../${href}">${esc(label)}</a></p></article>`)}</div></section>
  <section class="section section-soft" id="anwenden" data-radar-live-filter><div><div class="section-header"><p class="hero-kicker">Ebene 2 · Anwenden</p><h2>Welche Aussage willst du beantworten?</h2><p>Suche nach Aussage, Thema, Narrativ oder Werkzeug. Die Werkzeuge kommen nach der Grundlogik - aber sie bleiben schnell erreichbar. Die vollständige Sammlung liegt im Debattenkarten-Hub mit ${debateCardCount} kanonischen Debattenkarten.</p></div><label class="radar-search-field"><span>Inhalte im öffentlichen Wirkungsraum finden</span><input type="search" placeholder="z. B. Migration kostet nur, Klimaschutz kostet, Zensur, Radwege..." data-live-query></label><div class="filter-chip-row" aria-label="Werkzeugfilter"><button type="button" data-live-filter="all" aria-pressed="true">Alle</button><button type="button" data-live-filter="Antworten">Antworten</button><button type="button" data-live-filter="Narrative">Narrative</button><button type="button" data-live-filter="Resonanz">Resonanz</button><button type="button" data-live-filter="Agenda">Agenda</button><button type="button" data-live-filter="Ursachen">Ursachen</button><button type="button" data-live-filter="Resilienz">Resilienz</button></div><p class="radar-search-status" data-live-count data-live-count-label="Einstiege in dieser Übersicht">Inhalte gefunden</p><p><a class="text-link" href="../wirkungsradar/debattenkarten/">Alle ${debateCardCount} Debattenkarten öffnen</a></p>${cardGrid(entryCards, ([question, title, text, href, label]) => `<article class="card" data-radar-card data-topic="${attr(title.includes("Narrative") ? "Narrative" : title.includes("Resonanz") ? "Resonanz" : title.includes("Agenda") ? "Agenda" : title.includes("Ursachen") ? "Ursachen" : title.includes("Resilienz") ? "Resilienz" : "Antworten")}" data-search="${attr([question, title, text, label].join(" "))}"><p class="card-kicker">${esc(question)}</p><h2>${esc(title)}</h2><p>${esc(text)}</p><p><a class="btn btn-secondary" href="../${href}">${esc(label)}</a></p></article>`)}</div></section>
  <section class="section" id="aktuell"><div><div class="section-header"><p class="hero-kicker">Aktuell im Diskurs</p><h2>Resonanzräume und leise Wirkungsfragen.</h2><p>Diese Einstiege zeigen nicht nur Werkzeuge, sondern konkrete öffentliche Gewichtungen: Was wird laut, was bleibt leise, welche Ursache liegt darunter?</p></div>${cardGrid(discourseCards, ([kicker, title, text, href, label]) => `<article class="card" data-radar-card data-topic="Resonanz" data-search="${attr([kicker, title, text].join(" "))}"><p class="card-kicker">${esc(kicker)}</p><h3>${esc(title)}</h3><p>${esc(text)}</p><p><a class="btn btn-secondary" href="../${href}">${esc(label)}</a></p></article>`)}</div></section>
  ...
  <section class="section" id="vertiefen"><div><div class="section-header"><p class="hero-kicker">Ebene 3 · Vertiefen</p><h2>Dossiers, Journal, Glossar, Akademie und Forschung.</h2><p>Wer tiefer einsteigen will, findet hier die Langfassungen, Begriffe, Lernwege und methodischen Grundlagen.</p></div>${cardGrid(vertiefenCards, ([kicker, title, text, href, label]) => `<article class="card" data-radar-card data-topic="Vertiefen" data-search="${attr([kicker, title, text].join(" "))}"><p class="card-kicker">${esc(kicker)}</p><h3>${esc(title)}</h3><p>${esc(text)}</p><p><a class="btn btn-secondary" href="${esc(href)}">${esc(label)}</a></p></article>`)}</div></section>
  ...
  return shell({
    ...
    description: "Antworten finden, Narrative einordnen und Debatten wirksamer führen: der Öffentliche Wirkungsraum erklärt die fünf Wellen öffentlicher Wirkung und führt zu Debattenkarten, Resonanz-Kompass, Agenda-Radar, Ursachen-Navigator und Vertiefungen.",
    canonical: `${SITE_URL}/oeffentlicher-wirkungsraum/`,
    base: "../",
    main,
    extraHead: `<link rel="alternate" type="application/rss+xml" title="Öffentlicher Wirkungsraum - Wirkungsökonomie" href="${SITE_URL}/feeds/oeffentlicher-wirkungsraum.xml">`,
  });
}
