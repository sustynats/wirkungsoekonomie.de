...
ARTICLE_PATH = ROOT / "blog" / "der-verbrenner-stirbt-durch-risikomanagement-des-finanzsystems.html"
IMAGE_TARGET = ROOT / "assets" / "img" / "blog" / "2026-06-30-der-verbrenner-risikomanagement-finanzsystem.webp"
...
TITLE = "Der Verbrenner stirbt nicht durch Verbot, sondern durch Risikomanagement des Finanzsystems."
SUBTITLE = (
    "Warum Nachhaltigkeit im Finanzsystem Risikomanagement ist, und warum Physik "
    "und Finanzmathematik härter sind als Ideologie."
)
DESCRIPTION = (
    "Journal-Beitrag darüber, warum der Verbrenner im Finanzsystem nicht zuerst durch Verbote "
    "verliert, sondern durch Restwertrisiken, CO2-Kosten, Versicherbarkeit, Refinanzierung "
    "und Stranded-Asset-Gefahr."
)
DATE_LABEL = "30. Juni 2026"
DATE_ISO = "2026-06-30T17:01:00+02:00"
AUTHOR = "Natalie Weber"
ARTICLE_URL = "https://wirkungsoekonomie.de/blog/der-verbrenner-stirbt-durch-risikomanagement-des-finanzsystems.html"
IMAGE_URL = "https://wirkungsoekonomie.de/assets/img/blog/2026-06-30-der-verbrenner-risikomanagement-finanzsystem.webp"
IMAGE_ALT = (
    "Ein älterer Verbrenner auf einem Prüfstand vor Risikodashboards zu sinkendem Restwert, "
    "steigenden CO2-Kosten, Versicherungsrisiko, Refinanzierungsdruck und Stranded-Asset-Gefahr."
)
FIGCAPTION = (
    "Der Verbrenner verliert seine Selbstverständlichkeit, wenn Restwert, CO2-Kosten, "
    "Versicherung und Refinanzierung neu gerechnet werden."
)
...
def clean_body_text(text: str) -> str:
    ...
    substitutions = [
        ("Für Diesel-Dieter sieht das dann ganz banal aus.", "Für Kundinnen und Kunden sieht das dann ganz banal aus."),
        ("Nehmen wir nicht Diesel-Dieter, sondern ein Unternehmen", "Nehmen wir keine Privatperson, sondern ein Unternehmen"),
        ("Diesel-Dieter wird nicht verboten. Er wird bepreist.", "Der Verbrenner wird nicht verboten. Er wird bepreist."),
        ("Der Verbrenner verliert nicht, weil jemand böse guckt.", "Der Verbrenner verliert nicht wegen schlechter Stimmung."),
        ("Er merkt nicht, dass irgendwo ein Risikokomitee tagt.", "Sie merken nicht, dass irgendwo ein Risikokomitee tagt."),
        ("Er merkt nur:", "Sie merken nur:"),
    ]
    for old, new in substitutions:
        cleaned = cleaned.replace(old, new)
    cleaned = cleaned.replace("Diesel-Dieter", "die Privatperson")
    return cleaned
...
def article_blocks(blocks: list[dict]) -> list[dict]:
    ...
    for block in blocks:
        if block["type"] == "paragraph":
            ...
            if packed == "DerVerbrennerverliertnichtdurchVerbote.ErverliertdurchRisiko.":
                continue
            if text == "Warum Nachhaltigkeit im Finanzsystem Risikomanagement ist - und warum Physik und Finanzmathematik härter sind als Ideologie":
                continue
            if text.startswith("Entwurf für Journal"):
                continue
            if text == "Journalbeitrag":
                continue
        kept.append(block)
    return kept
...
def is_editorial_callout(text: str) -> bool:
    first_line = next((line.strip() for line in text.splitlines() if line.strip()), "")
    lowered = first_line.lower()
    return lowered.startswith(("tiktok-tauglicher satz", "sätze zum rausnehmen", "knackig"))


def render_callout(text: str, prefix: str, linked_terms: set[str]) -> str:
    if is_editorial_callout(text):
        return ""
    ...
    if any(marker in lowered for marker in ("kernsatz", "merksatz", "kurzformel", "schlusssatz")):
        class_name += " highlight"
    elif any(marker in lowered for marker in ("risiko", "bank", "versicherbarkeit")):
        class_name += " warning"
...
def render_article(body_html: str, toc: list[tuple[int, str, str]], reading_time: str) -> str:
    ...
    search_tags = ", ".join(tags + ["Restwert", "Refinanzierung", "Leasing", "Klimarisiko", "Wirkungsrisiko"])
...
{header}    <main id="inhalt" data-pagefind-body>
      <article class="hero">
        ...
        <figure class="hero-system-visual article-visual">
          <img src="../assets/img/blog/2026-06-30-der-verbrenner-risikomanagement-finanzsystem.webp" width="1376" height="768" alt="{esc(IMAGE_ALT)}" decoding="async" fetchpriority="high">
          <figcaption>{esc(FIGCAPTION)}</figcaption>
        </figure>
      </article>
      <section class="article-page">
        <div class="article-body">
          <div class="status-note"><strong>Faktenstand:</strong> Juni 2026. Der Beitrag ist eine Einordnung, keine Rechts-, Kredit-, Versicherungs-, Steuer- oder Anlageberatung.</div>
          <div class="callout"><p><strong>Vertiefung:</strong> Zur systemischen Einordnung passt das <a class="text-link" href="../bibliothek/klimawandel-finanzmarkt/">Dossier Klimawandel und der Finanzmarkt</a>.</p></div>
{toc_details(toc)}
{body_html}
        </div>
      </section>
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Import the combustion-engine risk-management journal article.")
...
def main() -> int:
    ...
    body_html, toc, word_count = render_blocks(blocks, "../", "journal-verbrenner-risiko")
