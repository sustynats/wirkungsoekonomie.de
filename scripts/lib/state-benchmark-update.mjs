...
export function applyStateBenchmarkUpdate(root='.'){
 ...
 const full=`<section class="section" id="staatliche-wirkungskontrolle-2026"><p class="hero-kicker">Fachliche Ergänzung · 6. September 2026</p><h2>${e(data.title)}</h2>${data.sections.map(s=>`<h3>${e(s.title)}</h3>${s.paragraphs.map(p=>`<p>${e(p).replaceAll("NOT_PUBLICLY_ESTABLISHED", "„öffentlich nicht belegt“")}</p>`).join('')}${s.sources.length?`<p class="meta-line">Belege: ${s.sources.map(n=>`<a href="${e(data.sources[n].url)}">${n+1}. ${e(data.sources[n].title)}</a>`).join(' · ')}</p>`:''}`).join('')}<p><a class="btn btn-secondary" href="${e(edition.url)}">Fachhinweis als PDF lesen (${edition.pages} Seiten)</a></p></section>`;
