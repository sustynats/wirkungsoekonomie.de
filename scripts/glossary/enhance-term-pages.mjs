...
function termFilterData(term) {
  return {
    ...
    theme: asList(term.theme || term.themes || term.category).map(filterToken),
...
function termBadges(term) {
  const badges = [
    term.type || term.begriffstyp || term.conceptStatus || term.concept_status || term.status,
    ...asList(term.theme || term.themes || term.category).slice(0, 2),
...
function indexPage() {
  ...
  const quickFilters = [
    ...
    ["Energie & Strommarkt", { theme: "energie-und-strommarkt" }],
    ["Psychologische Wirkmechanismen", { theme: "psychologie-und-resonanz" }],
    ["Philosophie & Werte", { theme: "philosophie-ethik-und-werte" }],
  ];
  const typeValues = [...filterValues("type"), ...filterValues("begriffstyp"), ...filterValues("conceptStatus")]
  ...
  const body = `      <section class="hero compact-hero">
      ...
      <section class="content-band glossary-filter-panel" aria-labelledby="glossary-filter-title">
        <div class="section-header compact">
          ...
          <p>Mehrfachfilter grenzen die Ergebnisliste ein; innerhalb der Treffer bleibt die alphabetische Ordnung erhalten.</p>
        </div>
