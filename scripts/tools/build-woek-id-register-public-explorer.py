#!/usr/bin/env python3
"""Build the public WÖk master-register from the canonical v1.5 workbook."""
...
DOWNLOAD_DIR = ROOT / "assets" / "downloads" / "woek-register" / "v1.5"
...
def normalize(items_raw, benchmarks_raw, rules_raw, sources_raw):
    ...
    for row in items_raw:
        ...
        item = {
            ...
            "ontology_role": row.get("Ontology_Role", ""),
            "state_variable_status": row.get("StateVariable_Status", ""),
            "indicator_mapping_status": row.get("Indicator_Mapping_Status", ""),
            "indicator_registry_reference": row.get("Indicator_Registry_Reference", ""),
            "permitted_data_functions": row.get("Permitted_Data_Functions", ""),
            "causality_status": row.get("Causality_Status", ""),
            "reality_check_role": row.get("RealityCheck_Role", ""),
            "architecture_note": row.get("Architecture_Note", ""),
...
def overview_page(items: list[dict[str, object]]) -> str:
    body = f'''<section class="hero portal-hero"><div class="hero-content"><nav class="breadcrumb"><a href="../index.html">Start</a> / Register</nav><p class="hero-kicker">Kanonische MasterItems-Fassung {VERSION}</p><h1>WÖk-Masterregister</h1><p class="hero-subtitle">Die Wirkungsontologie für WÖk-IDs und betrachtete Wirkungsgegenstände - getrennt vom Wirkindikatorenregister.</p><p>Das Register dokumentiert {len(items)} stabile MasterItems. Es zeigt Messregeln und Prüfstatus, ohne Indikator, Beobachtung, Wirkung oder Attribution gleichzusetzen.</p><p class="callout"><strong>Zwei Ebenen:</strong> Masterregister/MasterItems = WAS betrachten wir? Das separate Wirkindikatorenregister = WOMIT beobachten wir reale Zustände?</p><div class="hero-actions"><a class="btn btn-primary" href="../woek-id-register/">MasterItems durchsuchen</a><a class="btn btn-secondary" href="../bibliothek/woek-master-items-register/">Registerbeschreibung und Downloads</a><a class="btn btn-secondary" href="https://parlament.wirkungsoekonomie.de/methodik/wirkindikatoren">Wirkindikatorenregister öffnen</a></div></div></section>'''
    return layout("WÖk-Masterregister", "Kanonisches WÖk-Masterregister v1.5 mit 621 MasterItems, Zwei-Ebenen-Architektur, Quellen, Regeln und sichtbarem Prüfstatus.", body, "../", "/register/")
...
def register_page(items, rules, audit, changelog) -> str:
    ...
    body = f'''<section class="hero compact-hero"><nav class="breadcrumb"><a href="../index.html">Start</a> / <a href="../register/">Register</a></nav><p class="hero-kicker">Kanonische Fassung {VERSION} · 21. August 2026</p><h1>WÖk-MasterItems durchsuchen</h1><p class="hero-subtitle">{len(items)} stabile WÖk-IDs mit Wirkungsgegenstand, Messrichtung, Regel, Quellenfunktion, Architektur- und Prüfstatus.</p><p class="callout"><strong>v1.5 schließt die Architekturlücke.</strong> Alle 621 IDs bleiben erhalten. StateVariable- und Indicator-Mappings stehen auf OPEN_REVIEW_REQUIRED, bis sie fall- und quellenbezogen geprüft sind.</p></section>
...
<section class="section"><div class="card-grid three"><article class="card"><h2 class="card-title">Daten und Downloads</h2><p>XLSX, CSV, JSON, Version und Prüfsummen stehen auf einer vorgeschalteten Dokumentseite.</p><a class="text-link" href="../bibliothek/woek-master-items-register/">Registerbeschreibung öffnen</a></article><article class="card"><h2 class="card-title">Zwei Registerebenen</h2><p>MasterItems beschreiben den Wirkungsgegenstand. Indikatoren beobachten Zustände; sie sind nicht selbst die Wirkung.</p><a class="text-link" href="methodik/">Methodik und Grenzen lesen</a></article><article class="card"><h2 class="card-title">Wirkindikatorenregister</h2><p>Die Mess- und Beobachtungsontologie bleibt als eigenes Register erhalten.</p><a class="text-link" href="https://parlament.wirkungsoekonomie.de/methodik/wirkindikatoren">Indikatoren öffnen</a></article></div></section>
<script id="woekRegisterData" type="application/json">{embedded}</script><script>const data=JSON.parse(document.getElementById('woekRegisterData').textContent),items=data.items,$=id=>document.getElementById(id),safe=v=>String(v??'').replace(/[&<>"']/g,c=>({{'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}}[c]));function render(){{const q=$('woekSearch').value.toLowerCase().trim(),sdg=$('filterSdg').value,rule=$('filterRule').value,status=$('filterStatus').value,open=$('filterOpen').value,rows=items.filter(i=>(!q||[i.id,i.item,i.definition,i.sdg_or_sdgplus,i.indicator_family,i.source_detail,i.source_function].join(' ').toLowerCase().includes(q))&&(!sdg||i.sdg_or_sdgplus===sdg)&&(!rule||i.rule_id===rule)&&(!status||i.publication_readiness===status)&&(!open||String(i.open_status)===open));$('registerCount').textContent=`${{rows.length}} von ${{items.length}} WÖk-IDs sichtbar`;$('registerRows').innerHTML=rows.slice(0,250).map(i=>`<tr><td><strong>${{safe(i.id)}}</strong></td><td>${{safe(i.item)}}</td><td>${{safe(i.sdg_or_sdgplus)}}</td><td>${{safe(i.polarity_label)}}</td><td><code>${{safe(i.rule_id)}}</code></td><td>${{safe(i.publication_readiness)}}</td><td><a class="btn btn-secondary table-action" href="${{safe(i.slug)}}/">Eintrag prüfen</a></td></tr>`).join('')+(rows.length>250?'<tr><td colspan="7">Weitere Treffer vorhanden. Bitte Filter verfeinern.</td></tr>':'');}}document.querySelectorAll('.woek-register-filters input,.woek-register-filters select').forEach(element=>element.addEventListener('input',render));render();</script>'''
    return layout("WÖk-Masterregister v1.5", "Durchsuchbares WÖk-Masterregister v1.5 mit 621 MasterItems, Regeln, Quellen, Architektur- und Prüfstatus.", body, "../", "/woek-id-register/")
...
def source_index_page(sources) -> str:
    ...
    return layout("Quellen des WÖk-Masterregisters", "Interne Detailseiten zu den Quellen des WÖk-Masterregisters v1.5 mit Herausgeber, Funktion, Datenfunktion, Version und Originalzugang.", body, "../../", "/woek-id-register/quellen/")
...
def methodology_page() -> str:
    body = '''<section class="hero compact-hero"><nav class="breadcrumb"><a href="../../register/">Register</a> / <a href="../">WÖk-Masterregister</a></nav><p class="hero-kicker">Methodik und Grenzen</p><h1>Was das Masterregister leistet - und was nicht</h1><p class="hero-subtitle">Das Masterregister strukturiert betrachtete Wirkungsgegenstände. Es ersetzt weder das separate Wirkindikatorenregister noch Wirkpfad, Gegenfaktum und fachliche Prüfung.</p></section><section class="section narrow"><div class="card"><h2>Zwei getrennte Ebenen</h2><p><strong>Masterregister/MasterItems:</strong> WAS betrachten wir? <strong>Wirkindikatorenregister:</strong> WOMIT beobachten wir reale Zustände?</p><p>Die vollständige Kette lautet: MasterItem → StateVariable → Indicator → Observation → Analysis / RealityCheck.</p><h2>Vom Fall zum Indikator</h2><p>Zuerst werden Originalquelle, Problem, Ziel, Wirkmechanismus, mögliche Zustandsveränderung und Referenz bestimmt. Danach werden Zustandsvariable und geeigneter Indikator fallbezogen zugeordnet.</p><h2>Pflichttrennungen</h2><p>Indikator ≠ Wirkung. Zielbezug ≠ Kausalitätsnachweis. Output ≠ Outcome. Beobachtung ≠ Attribution. Directionality ≠ Score.</p><h2>Staatliche Architektur und WÖk</h2><p>DNS, GGO/GFA, Nachhaltigkeitsprüfung, eNAP/eGFA und parlamentarische Begleitung bilden bereits eine institutionelle Architektur. WÖk ersetzt sie nicht, sondern ergänzt sie um einen durchgängigen Problem-, Ziel-, Mechanismus-, Gegenfaktums-, Attributions-, Options- und Reality-Check-Pfad.</p><h2>Keine Daten bedeuten keine Bewertung</h2><p>Ein fehlender Messwert wird nicht als null und nicht als neutral behandelt. Er bleibt unbewertet.</p><h2>Schwellen bleiben prüfbar</h2><p>Externe Schwellen und WÖk-Kalibrierungen werden getrennt. v1.5 bezeichnet die führende Architektur- und MasterItems-Fassung, nicht die externe Validierung jeder einzelnen Grenze oder Zuordnung.</p><h2>Nichtkompensation</h2><p>Schwere negative Wirkungen oder verletzte Schutzgrenzen dürfen nicht durch gute Werte an anderer Stelle weggemittelt werden.</p></div></section>'''
...
def library_detail_page(statistics: dict[str, int]) -> str:
    body = f'''<section class="hero compact-hero document-detail-hero"><nav class="breadcrumb"><a href="../../bibliothek/">Bibliothek</a> / Datenregister</nav><p class="hero-kicker">MasterItems-Register · führende Fassung</p><h1>WÖk-Masterregister v1.5</h1><p class="hero-subtitle">Kanonische Wirkungsontologie für stabile WÖk-IDs - mit expliziter Trennung von Zustandsvariablen, Indikatoren, Beobachtung und Attribution.</p><div class="document-card-badges"><span class="status-badge">621 WÖk-IDs</span><span class="status-badge">Version 1.5</span><span class="status-badge">0 IDs verändert</span></div></section><section class="section document-detail-grid"><aside class="document-detail-aside" data-search-exclude><dl><dt>Dokumentart</dt><dd>MasterItems- und Wirkungsontologie-Register</dd><dt>Status</dt><dd>führende Architektur- und MasterItems-Fassung</dd><dt>Stand</dt><dd>21. August 2026</dd><dt>Umfang</dt><dd>{statistics["woek_ids"]} WÖk-IDs · {statistics["indicator_families"]} Familien · {statistics["scoring_rules"]} Regeln</dd><dt>Herausgeber</dt><dd>{PUBLISHER}</dd></dl><div class="document-action-row"><a class="btn btn-secondary" href="../../woek-id-register/">Online-Register öffnen</a><a class="btn btn-primary" href="../../assets/downloads/woek-register/v1.5/{FILENAME}">XLSX herunterladen</a><a class="btn btn-secondary" href="../../assets/downloads/woek-register/v1.5/register-v1.5.csv">CSV herunterladen</a><a class="btn btn-secondary" href="../../assets/downloads/woek-register/v1.5/register-v1.5.json">JSON herunterladen</a></div><a class="text-link" href="../../assets/downloads/woek-register/v1.5/manifest.json">Prüfsummen und Manifest</a></aside><article class="document-detail-main"><div class="callout"><strong>Kurz gesagt:</strong> Das Masterregister beantwortet WAS betrachtet wird. WOMIT reale Zustände beobachtet werden, steht im separaten Wirkindikatorenregister.</div><h2>Was ist neu in v1.5?</h2><ul><li>Alle 621 WÖk-IDs aus v1.4 bleiben unverändert.</li><li>MasterItem, StateVariable, Indicator, Observation und Analysis/RealityCheck sind als Kette getrennt.</li><li>Offene StateVariable- und Indikatorzuordnungen heißen ausdrücklich OPEN_REVIEW_REQUIRED.</li><li>DNS, GGO/GFA, Nachhaltigkeitsprüfung, eNAP/eGFA und PBnEZ sind über amtliche Quellen verankert.</li><li>Der führende Begriffsleitfaden v1.6 ist als Methodenreferenz eingebunden.</li></ul><h2>Was kann ich prüfen?</h2><p>Jede WÖk-ID hat eine Detailseite mit Objektrolle, Mappingstatus, Messgröße, Regel, Quellenfunktion, Datenqualität und Fachstatus.</p><h2>Wichtige Grenze</h2><p>Indikator ist nicht Wirkung; Zielbezug ist kein Kausalitätsnachweis; Beobachtung ist nicht Attribution. Offene Mappings wurden nicht automatisch geschlossen.</p><h2>Versionierung</h2><p>v1.5 ersetzt v1.4 als führende MasterItems-Fassung. v1.4 bleibt unverändert im historischen Downloadpfad erhalten.</p><p><a class="text-link" href="../../woek-id-register/methodik/">Methodische Grenzen lesen</a> · <a class="text-link" href="https://parlament.wirkungsoekonomie.de/methodik/wirkindikatoren">Wirkindikatorenregister öffnen</a></p></article></section>'''
    return layout("WÖk-Masterregister v1.5 | Bibliothek der Wirkungsökonomie", "Führendes WÖk-Masterregister v1.5 mit 621 stabilen MasterItems, Zwei-Ebenen-Architektur, Quellen-, Kalibrierungs- und Prüfstatus.", body, "../../", "/bibliothek/woek-master-items-register/")
...
def build_exports(items_raw, schema_raw, statistics) -> dict[str, object]:
    ...
    csv_path = DOWNLOAD_DIR / "register-v1.5.csv"
    with csv_path.open("w", encoding="utf-8-sig", newline="") as handle:
        writer = csv.DictWriter(handle, fieldnames=fields, lineterminator="\n")
        writer.writeheader(); writer.writerows(public_records)
    json_path = DOWNLOAD_DIR / "register-v1.5.json"
    write_json(json_path, {"schema_version": "1.1.0", "register_version": "1.5", "published_at": PUBLISHED_AT, "publisher": PUBLISHER, "status": "kanonische MasterItems- und Architekturfassung", "interpretation_boundary": "MasterItem, StateVariable, Indicator, Observation und Analysis/RealityCheck bleiben getrennt. Offene Zuordnungen, Kalibrierungen, Benchmarks und Fachprüfungen bleiben sichtbar. Fehlende Daten erzeugen keine Bewertung.", "two_level_architecture": {"master_register": "WAS betrachten wir?", "indicator_register": "WOMIT beobachten wir reale Zustände?", "chain": "MasterItem → StateVariable → Indicator → Observation → Analysis / RealityCheck", "indicator_register_url": "https://parlament.wirkungsoekonomie.de/methodik/wirkindikatoren"}, "statistics": statistics, "public_schema": schema_raw, "items": public_records})
    manifest = {"manifest_version": "1.1.0", "register_version": "1.5", "published_at": PUBLISHED_AT, "publisher": PUBLISHER, "canonical_source": FILENAME, "canonical_document_page": "/bibliothek/woek-master-items-register/", "historical_predecessor": "/assets/downloads/woek-register/v1.4/manifest.json", "files": [{"filename": path.name, "sha256": digest(path), "bytes": path.stat().st_size} for path in [public_xlsx, csv_path, json_path]], "statistics": statistics, "id_delta": {"retained": 621, "added": 0, "removed": 0, "renamed": 0}, "changelog": "v1.5 trennt MasterItems und Wirkindikatorenregister, ergänzt die staatliche Nachhaltigkeitsarchitektur und erhält alle 621 IDs unverändert. Offene Zuordnungen und Kalibrierungen bleiben offen."}
...
def main() -> None:
    ...
    if len(items) != 621 or len(raw_rules) != 28:
        raise SystemExit(f"Unerwarteter v1.5-Import: {len(items)} Einträge / {len(raw_rules)} Regeln")
    if len({str(item["id"]) for item in items}) != 621:
        raise SystemExit("v1.5 enthält keine 621 eindeutigen WÖk-IDs.")
    if {str(item["ontology_role"]) for item in items} != {"MASTER_ITEM"}:
        raise SystemExit("v1.5 weist nicht alle Einträge als MASTER_ITEM aus.")
    if {str(item["state_variable_status"]) for item in items} != {"OPEN_REVIEW_REQUIRED"}:
        raise SystemExit("StateVariable-Status wurde unzulässig automatisch geschlossen.")
    if {str(item["indicator_mapping_status"]) for item in items} != {"OPEN_REVIEW_REQUIRED"}:
        raise SystemExit("Indicator-Mapping-Status wurde unzulässig automatisch geschlossen.")
    ...
    ensure_public_safety([SOURCE_XLSX, DOWNLOAD_DIR / FILENAME, DOWNLOAD_DIR / "register-v1.5.csv", DOWNLOAD_DIR / "register-v1.5.json", DOWNLOAD_DIR / "manifest.json"])
