...
export function evaluateChecks(data, now) {
  ...
  if (bridgeMode) {
    ...
    const api=b.api_processor_health || {}, apiEnabled=api.actor==='oracle_api' && api.enabled===true;
    const apiAvailable=apiEnabled && age(api.at,now)<=30 && api.preflight?.status==='PASS'
      && api.preflight.write_ok===true && ['98_CONFIG','00_INBOX','10_CLAIMED','20_OUTPUT_READY','30_ACK'].every(k=>api.preflight.reads?.[k]===true);
    const apiCoversWork=apiAvailable && (!api.news_only || b.open_personal_count===0);
    ...
    checks.push(
      ...
      {id:'bridge-processor',name:'Redaktionsworker',ok:b.reachable!==true || !Number(b.open_count||0) || apiCoversWork || chatAvailable,reason:'Offene Aufträge ohne passenden frischen Lese-/Schreibnachweis. Der API-Nachrichtenworker ersetzt keinen fehlenden Bearbeiter persönlicher Aufträge.',immediate:false},
      {id:'bridge-shards',name:'Vollständigkeit der Redaktionsworker',ok:b.reachable!==true || !Number(b.open_count||0) || apiCoversWork || health.all_shards_available===true,reason:'Die offenen Auftragsarten sind nicht vollständig durch nachgewiesene Worker abgedeckt. Ein Nachrichtenworker allein belegt keine Verarbeitung persönlicher Beiträge.',immediate:false},
      {id:'bridge-api-results',name:'API-Redaktionsverarbeitung',ok:!apiEnabled || api.status==='RUN_COMPLETED',reason:'Der Oracle-API-Worker meldet unvollständige oder ungewisse Ergebnisse beziehungsweise ein Budgetproblem. Gelieferte Entwürfe sind noch keine Veröffentlichungen.',immediate:false},
