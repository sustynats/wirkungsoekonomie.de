...
export function isHistoricalJob(value) {
  ...
  return input.job_type === 'impact_reassessment' || input.backfill === true || value.backfill === true
    || input.discovery?.trigger_type === 'backfill';
}
export function processorPriority(value, now) {
  const input = value.input || value, candidate = value.candidate || {};
  ...
  let band = input.urgent || input.request?.urgent || candidate.urgent || tier === 'TOP' ? 600
    : input.job_type === 'story_update' || input.job_type === 'correction' || input.job_type === 'impact_semantic_review' ? 500
    : ['critical', 'very_high'].includes(importance) ? 400
    : importance === 'high' || tier === 'HIGH' ? 300 : 200;
  if (isHistoricalJob(value)) band = 0;
  // Aging helps within/across ordinary bands but can never overtake urgent work.
  const age = Math.max(0, (Date.parse(now) - Date.parse(input.created_at || value.created_at)) / HOUR) || 0;
  return Math.min(band >= 500 ? band + 49 : 499, band + Math.min(199, Math.floor(age / 2) * 10));
}
export function selectProcessorBatch(jobs, shard, now, { maxJobs = 10, queueCritical = false } = {}) {
  ...
  return jobs.filter(j => !closed.has(j.status) && !j.ack && !j.output && !j.claim
    ...
    .sort((a, b) => processorPriority(b, now) - processorPriority(a, now)
      || String((a.input || a).created_at).localeCompare(String((b.input || b).created_at))
