...
export async function ensureSemanticReview(bridge, job, output, record, proposed, now) {
  ...
  // The brief belongs to the immutable parent input, not a transient record
  // copy. Canonical content changes still fail the independent source binding.
  const requestedCorrection = job.input.job_type === 'impact_reassessment'
    ? job.input.article?.requested_correction : record.impact_reassessment_request;
  const reviewRecord = { title: record.title, source_summary: record.source_summary || record.research_summary || '',
        ...(requestedCorrection ? {requested_correction:requestedCorrection} : {}),
