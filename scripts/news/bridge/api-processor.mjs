...
export const API_VALIDATION_REVISION = 'single-paid-attempt-3';
...
function reviewAssignment(original) {
  ...
  if (typeof assignment.instructions === 'string') assignment.instructions = assignment.instructions.replace(
    ...
    'Bestätige das unveränderte proposed_assessment über assessment_confirmation, oder liefere bei notwendigen Korrekturen das vollständige impact_assessment.');
  return assignment;
}
...
export function prepareApiJob(packet, knowledge, { priorOutput = null } = {}) {
  ...
  const prompt = kind === 'news' && original.wirkungsticker?.analysis_prompt ? [
    ...
    transport_revision: 'review-confirmation-6',
    ...
    output_contract: kind === 'review' ? {response_format:REVIEW_RESPONSE_FORMAT,
      confirmation_rule:'Prüfe jeden Faktor und jeden der 14 Checks unabhängig. Wenn die fachlichen Werte, Pfade und Belegbindungen richtig sind, impact_assessment:null und assessment_confirmation liefern: eigenes research_check und für jeden low/not_assessable/high_uncertainty-Pfad den tatsächlich geprüften Suchindex sowie ein konkretes Ergebnis. Auch erfolglose Recherche kann abgeschlossen sein. Nicht durchgeführte notwendige Recherche bleibt needs_research; dann keine Bestätigung. Bei echten Korrekturen vollständiges korrigiertes impact_assessment liefern und assessment_confirmation:null. Kein Wortlaut muss umformuliert werden, nur weil Du der zweite Prüfer bist. Kurze konkrete Prüfbegründungen; keine wiederholten Artikel oder URLs in Prüflabels. Bereits gelieferte Quellenauszüge nicht als neue research_sources duplizieren. Neue Quellen nur mit einem zusammenhängenden wörtlichen Originalauszug; getrennte Sätze nicht zu einem angeblichen Zitat zusammensetzen.',
