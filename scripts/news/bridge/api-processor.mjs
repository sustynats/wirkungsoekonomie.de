...
export const API_VALIDATION_REVISION = 'single-paid-attempt-5';
...
export function prepareApiJob(packet, knowledge, { priorOutput = null } = {}) {
  ...
  const prompt = kind === 'news' && original.wirkungsticker?.analysis_prompt ? [
    ...
    transport_revision: 'bound-review-confirmation-8',
    ...
    ...(kind === 'review' && needsMediaReview(original.record) ? {media_completion:'Der erste Entwurf hat trotz eines lokalen Triggers keinen Mediencheck geliefert. Prüfe die tatsächliche kommunikative Relevanz eigenständig und gib media_applicability:{relevant:boolean,reason:konkrete Begründung} zurück. Der Regex-Trigger ist keine fachliche Entscheidung. Nur wenn kein eigenständiger substanzieller Medienwirkungscheck erforderlich ist, darf relevant:false gelten. Ist ein solcher Check nötig und fehlt er im Artikel, relevant:true und review.status needs_review/blocked; niemals zur Freigabe wegklassifizieren. Alle 14 Checks bleiben erforderlich.'} : {}),
