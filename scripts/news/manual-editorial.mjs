...
export function renderManualArticle(analysis) {
  const cover = analysis.book;
  const contents = `<details class="news-editorial-toc"><summary>Inhalt der Buchbesprechung</summary><ol>${analysis.rendered.headings.filter(h => h.level === 2).map(h => `<li><a href="#${h.id}">${escape(h.title)}</a></li>`).join("")}</ol></details>`;
