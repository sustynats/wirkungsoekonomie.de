...
export function consolidateLeizenDuplicates(stories, at) {
  const byId = new Map(stories.map((story) => [story.story_id, story]));
  const canonical = byId.get('wt-1d3fd4e1a02fe3e4');
  const duplicates = ['wt-7699cc5cf04f6af2', 'wt-9bfe2b84b6ed7d1e']
  ...
  for (const duplicate of duplicates) {
    ...
    changes.push({ story_id: duplicate.story_id, canonical_story_id: canonical.story_id,
      reason: 'source_verified_leizen_duplicate' });
  }
  return changes;
}
