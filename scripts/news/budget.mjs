...
export function modelRates(model = "gpt-5.5") {
  ...
  if (/^gpt-6-astra(?:-|$)/.test(model)) return { inputUsdPerMillion: 10, outputUsdPerMillion: 50, cachedInputUsdPerMillion: 1 };
  return { inputUsdPerMillion: 5, outputUsdPerMillion: 30, cachedInputUsdPerMillion: 0.5 };
}

export function requestReservationUsd(model = '') {
  return /^gpt-6-astra(?:-|$)/.test(model) ? ASTRA_REQUEST_RESERVATION_USD : NEWS_REQUEST_RESERVATION_USD;
}
...
export function costFromUsage(result, estimated = {}) {
  ...
  const reservation = requestReservationUsd(result.model);
  const priorReserve = (attempts - 1) * reservation;
  ...
  if (!validReportedUsage(usage) || !/^(?:gpt-5\.(?:4-mini|5|6-luna)(?:-|$)|gpt-6-astra(?:-|$))/.test(result.model || '')) return { input_tokens: 0, output_tokens: 0, ...estimated, estimated_cost_usd: Number((priorReserve + Math.max(reservation, Number(estimated.estimated_cost_usd) || 0)).toFixed(6)), token_source: "conservative_reservation_usage_unavailable" };
