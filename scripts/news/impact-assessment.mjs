...
import { MAGNITUDE_PATH_SCHEMA, MAGNITUDE_STATE_RULE, MAGNITUDE_FACTORS, pathwayMagnitudeErrors, aggregateMainPaths, calculateMagnitude } from './impact-magnitude.mjs';
import { POTENTIAL_REVISION, PATH_QUALITIES, POTENTIAL_RESEARCH_RULE, potentialDimensionErrors, deriveStatusPresentation } from './impact-potential.mjs';
...
export const PATH_STATUS = ['modelled', 'insufficient_basis'];
...
export function deriveImpactPresentation(input = {}, context = {}) {
  ...
  const dimensions = Object.fromEntries(IMPACT_KEYS.map(key => {
    ...
    const potentialPresentation = deriveStatusPresentation({...d,label:d.primary_paths?.[0]?.label || d.rationale});
...
export const IMPACT_PROMPT_RULE = MAGNITUDE_STATE_RULE + ' ' + `Wirkungsticker2.1/all-dimensions-1: Alle3MPD modelled,magnitude0..5: bedingt/quellengebunden. Keine Messdaten≠kein Potenzial; nichts erfinden. Ohne vertretbares Modell: needs_research/HOLD; null nur historisch. 0=begründet vernachlässigbar, nie fehlende Daten. Relevanz≠Tragweite≠Eintritt≠Evidenz. Unsicherer Eintritt≠offene Richtung. news_event/evaluation_target/baseline/counterfactual/reference_frame trennen. Sachverhalt/Risiko statt Warnung. Politische Forderung: Maßnahme bewerten. Kommunikation begründen. Dossier erhalten; Vergleich≠Datenlücke. Pfade: Empfänger,Mechanismus,Bedingung,Raum,Zeit,Ordnungen1–3,Quellenfunktion,Annahmen,Grenzen. supported_mechanism vs model_hypothesis; Hypothesen nur low/not_assessable. low/high_uncertainty: zweiter Recherchepass mit Ergebnis+magnitude_range. research_sources: echte URL,Funktion,exakter kurzer Beleg; Ereignis≠Folgenbeleg. Mechanismus/Referenz/Gegenevidenz: Wissenschaft/Recht/Statistik. Keine erfundenen Quellen/Fakten/Prozente. Hauptpfade ex_ante/ongoing; ongoing nur mit Signal. Belegte Änderungen: observed_effects mit Faktoren,Empfängern,Zeit,Attribution. Schaden≠Risiko; Beobachtung≠Ursachennachweis. Ring=Zeitstatus; original/current_potential_assessment erhalten. R/I/D/U/V/S je0..5 quellenbegründet; Summe/6, Stufen0=>0,>0<1.5=>1,<2.5=>2,<3.5=>3,<4.5=>4,sonst5. Kein Ableiten aus Relevanz/Evidenz/Eintritt. Begründete schwere Schutzgrenze=>mindestens4; ex ante nur genau dieser bedingte Grenzpfad. Nichtkompensation/Reverse Merit Order, keine gemittelte MPD-Gesamtnote. Materielle +/- Hauptpfade: höchste Stufe je Richtung, Differenz≤1=>mixed/balanced,≥2=>mixed/dominant. Negative Schutzgrenze zuerst. Nebenrisiko,Schutzplanke,Restschaden,besserer Entwurf,Reaktion,fremder Nutzen≠Ausgleich. Kooperation: Macht,Mehrheiten,Ämter,Personal,Budget,Institutionen,Normalisierung. Energie: EE/Hürden,Netze,Speicher,fossile Bindungen,Emissionen,Importe und gekoppelte Gesundheit,Versorgung,Arbeit,Verteilung,Demokratie. Keine Ressort-/Parteienautomatik. system_check: MPD-Verbindungen,Ordnungen1–3,Enablement,Gegenevidenz,Quellenunabhängigkeit,Rechtsstatus. Programme≠Folgenbeweis.`;
