...
export const SINGLE_CALL_INSTRUCTIONS = [
  ...
  'Jede Dimension trägt zusätzlich rationale (Begründungstext) und balance. balance ist IMMER ein Objekt mit comparable_material_paths, protection_boundary_decisive und rationale - niemals null, auch bei eindeutiger Richtung (dann ein Satz, warum keine Abwägung nötig ist). Die beiden Booleans setzt der Server aus den Wirkpfaden; ein falscher Wert kostet nichts, ein fehlendes Objekt einen zweiten Aufruf. Diese beiden Schlüssel fehlten in der Hälfte der Antworten.',
