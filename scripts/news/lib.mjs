...
export function buildAnalysisPrompt(stories) {
  ...
  return [
    ...
    "Beachte review_mode: Bei historical_relevance_reassessment prüfst du, ob die damals gemeldete Quelleninformation zum angegebenen Quelldatum eine veröffentlichungswürdige Neuigkeit war. Markiere sie nicht allein deshalb als Dublette, weil existing_history dieselbe frühere Ticker-Version zeigt. related_ticker_history nennt dagegen eigenständige andere Ticker-Akten und ist auch bei einer historischen Prüfung als Dublettenvergleich zu verwenden. Bei new_or_updated_story muss gerade die neue Entwicklung gegenüber der Vorgeschichte materiell sein.",
