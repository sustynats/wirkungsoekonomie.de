...
export function renderImpactRing(presentation, dimensionLabel) {
  ...
  const label=`${dimensionLabel}: Status ${presentation.ringLabel}; Tragweite ${Number.isInteger(presentation.magnitudeBars) ? `${presentation.magnitudeBars} von 5` : "offen"}; Richtung ${presentation.directionLabel}. Statuskategorie, kein Prozentwert.`;
...
export function renderDimensionMeters(analysis = {}, { compact = false, context = {} } = {}) {
  ...
  const items = Object.entries(DIMENSIONS).map(([key, meta]) => {
    ...
    // The modelled potential is the core reading and leads every dimension.
    // A documented, already observed consequence follows on its own labelled
    // row (16.09.: two unlabelled rows with equal values read as a duplicate).
    const observed = d.displayed_observation ? d : null;
    const main = observed ? d.potential_presentation : d;
    const level = observed ? (Number.isInteger(d.magnitude) ? d.magnitude : null) : (d.magnitudeBars ?? d.magnitude);
    const bars = (value) => Array.from({ length: 5 }, (_, index) => `<i${Number.isInteger(value) && index < value ? ' class="is-filled"' : ""}></i>`).join("");
    const segments = bars(level);
    const track = `<span class="wt-meter wt-dim__track${level === null ? ' wt-meter--unknown' : ''}" data-magnitude="${level ?? 'open'}" role="img" aria-label="${level === null ? `Tragweite offen für ${meta.label}; gestrichelte Skala, keine Bewertung mit null Punkten` : `Tragweite für ${meta.label}: ${level} von 5`}"${level === null ? ' title="Tragweite offen – gestrichelte Skala bedeutet keine Nullbewertung"' : ''}>${segments}</span>`;
    return `<div class="wt-dim wt-dim--${key}" data-potential-model="${escapeHtml(profile.version)}" data-direction="${(observed ? main.direction : d.display_direction) || d.direction}" data-path-status="${d.path_status || "modelled"}"><div class="wt-dim__head">${renderIcon(meta.icon)}<strong>${meta.label}</strong></div><div class="wt-impact-reading">${renderImpactRing(main,meta.label)}${track}<span class="wt-impact-direction" title="${escapeHtml(main.directionLabel || d.long_label)}">${escapeHtml(main.directionLabel || d.label)}</span></div>${observed ? `<p class="wt-impact-path-title">${escapeHtml(main.shortPathLabel)}</p><div class="wt-impact-additional wt-impact-additional--observed"><span>Beobachtet:</span>${renderImpactRing(observed,meta.label)}<span class="wt-meter wt-dim__track" data-magnitude="${observed.magnitudeBars ?? "open"}" role="img" aria-label="Beobachtete Wirkung für ${meta.label}: Tragweite ${Number.isInteger(observed.magnitudeBars) ? `${observed.magnitudeBars} von 5` : "offen"}">${bars(observed.magnitudeBars)}</span><span>${escapeHtml(observed.directionLabel)}</span></div><p class="wt-impact-path-title">${escapeHtml(observed.shortPathLabel)}</p>` : ""}${compact ? (d.displayed_observation ? "" : `<p class="wt-impact-path-title">${escapeHtml(d.primary_paths?.[0]?.label || d.rationale || "Einordnung offen")}</p>`) : `<p class="wt-impact-evidence">Potenzialtragweite: ${escapeHtml(d.magnitude_label)} · Eintritt: ${escapeHtml(d.likelihood_label)} · Evidenz: ${escapeHtml(d.evidence_label)} · ${escapeHtml(d.time_label)}</p><ul class="wt-impact-paths">${(d.primary_paths || []).map(path => pathMarkup(path)).join("")}${(d.secondary_paths || []).map(path => pathMarkup(path, true)).join("")}</ul><p>${escapeHtml(d.rationale || "Die Einordnung bleibt offen.")}</p>${profile.observed_effects?.filter(e=>e.dimension===key).map(e=>`<p class="wt-impact-observed"><strong>${e.direction === "negative" ? "−" : e.direction === "positive" ? "+" : "±"} beobachtete Wirkung:</strong> ${escapeHtml(e.change)} ${e.attribution === "open" ? "Die Ursachenattribution bleibt gesondert offen." : ""}</p><ul class="wt-impact-paths">${pathMarkup({...e,label:e.label || e.change},false,true)}</ul>`).join("") || ""}${d.observed_outcome?.change ? `<p><strong>Beobachtet:</strong> ${escapeHtml(d.observed_outcome.change)}${d.observed_outcome.attribution === "open" ? " Die Ursachenzurechnung bleibt offen." : ""}</p>` : ""}`}</div>`;
  }).join("");
  ...
  return `<p class="wt-impact-heading">Wirkungspotenzial</p>${reference}<div class="wt-dims wt-dims--impact${compact ? " wt-dims--compact" : ""}"${context.privateImpactPreview ? "" : ' data-reviewed-impact-profile="2.1"'}>${items}</div>${compact ? "" : `<p class="wt-dims__legend">${IMPACT_LEGEND}. Nichtkompensation: Schwere Schäden werden nicht mit Vorteilen anderer Dimensionen verrechnet. Reverse Merit Order berücksichtigt maßgebliche Schutzgrenzen.</p>`}${review}`;
}
