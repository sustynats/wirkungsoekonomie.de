...
export function renderDimensionMeters(analysis = {}, { compact = false, context = {} } = {}) {
  ...
  const items = Object.entries(DIMENSIONS).map(([key, meta]) => {
    ...
    const track = level === null ? '<span class="wt-dim__track" aria-label="Tragweite offen, nicht null oder neutral">—</span>' : `<span class="wt-meter wt-dim__track" data-magnitude="${level}" role="img" aria-label="Tragweite für ${meta.label}: ${level} von 5">${segments}</span>`;
