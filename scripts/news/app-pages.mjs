...
import {EDITORIAL_RESPONSIBILITY} from './editorial-authorship.mjs';
...
// Ein Ressort, zu dem es keinen einzigen Beitrag gibt, wird nicht angeboten.
// Gemessen am 17.09.2026: "Wissenschaft" traf 0 von 363 veroeffentlichten
// Meldungen - die Quellenliste hat keine Wissenschaftsquelle. Ein Filter, der
// immer leer antwortet, ist ein Fehler und keine Auswahl. (Die Luecke selbst
// bleibt eine redaktionelle Frage: sie schliesst sich mit Quellen, nicht mit
// einem weicheren Muster - "Studie" im Titel macht eine Pisa-Debatte nicht zur
// Wissenschaftsmeldung.)
function ressorts(vorhanden){
 return Object.entries(APP_TOPICS).filter(([key])=>key==='alle'||!vorhanden||vorhanden.has(key));
}
function filters(mode,vorhanden=null){
 const types=[['alle','Alle Formate'],...Object.entries(APP_TYPES).filter(([key])=>mode!=='analysen'||key!=='news')];
 const entries=mode==='news'?ressorts(vorhanden):types.map(([key,label])=>[key,key==='alle'?'Alle':label]);
 const options=list=>list.map(([key,label])=>`<option value="${key}">${esc(label)}</option>`).join('');
 return `<div class="ticker-app-filter-row"><details class="ticker-app-filter-menu"><summary>Filter<span data-app-filter-count></span></summary><form class="ticker-app-filter-panel" data-app-filter-form><strong>Beiträge filtern</strong>${mode!=='news'?`<label>Format<select name="typ">${options(types)}</select></label>`:''}<label>Ressort<select name="ressort">${options(ressorts(vorhanden).map(([k,v])=>[k,k==='alle'?'Alle Ressorts':v]))}</select></label>${mode==='suche'?'<label>Sortieren<select name="sort"><option value="relevanz">Passendste zuerst</option><option value="neueste">Neueste zuerst</option></select></label>':''}${viewSwitch()}<div class="ticker-filter-actions"><button class="btn btn-primary" type="submit">Anwenden</button><button class="btn btn-secondary" type="button" data-app-clear-filters>Zurücksetzen</button></div></form></details><div class="ticker-app-filters" role="group" aria-label="${mode==='news'?'Ressort':'Beitragsart'}">${entries.map(([key,label])=>`<a href="?${mode==='news'?'ressort':'typ'}=${key}" data-app-filter="${key}"${key==='alle'?' aria-current="true"':''}>${esc(label)}</a>`).join('')}</div></div>`;
}
export function buildAppPages({root,stories,analyses,storiesById,storyCard,editorialCard,pageShell,write,updatedAt,appTools="",lagen=[]}){
 const records=appRecords(stories,analyses,storiesById,{storyCard,editorialCard});
 const vorhandeneRessorts=new Set(records.flatMap((record)=>record.topics||[]));
 ...
 // nicht alles zeigen (Natalie am 17.09.: „Die Startseite koennte sehr klar
 // werden: Morgenlage, Mittagslage, Abendlage").
 const lagenAbschnitt=()=>{
  const gueltig=(lagen||[]).filter(l=>l&&l.lage_id&&l.date&&Number.isFinite(Date.parse(l.stand)));
  if(!gueltig.length) return '';
  const tag=gueltig.map(l=>l.date).sort().at(-1);
  const heute=gueltig.filter(l=>l.date===tag).sort((a,b)=>Date.parse(b.stand)-Date.parse(a.stand));
  const datum=new Intl.DateTimeFormat('de-DE',{timeZone:'Europe/Berlin',day:'numeric',month:'long'}).format(new Date(heute[0].stand));
  const stand=(l)=>new Intl.DateTimeFormat('de-DE',{timeZone:'Europe/Berlin',hour:'2-digit',minute:'2-digit',hourCycle:'h23'}).format(new Date(l.stand));
  return `<section class="section ticker-home-lagen" aria-labelledby="lagen-titel"><div class="ticker-section-heading"><h2 id="lagen-titel">Die Lage am ${esc(datum)}</h2></div><ol class="ticker-lagen-liste">${heute.map(l=>`<li><a href="${base}lage/${esc(l.lage_id)}/"><p class="ticker-lage-kicker">${esc(l.label)} · Stand ${esc(stand(l))} Uhr</p><p class="ticker-lage-satz">${esc(l.headline)}</p><p class="ticker-lage-zahl">${l.counts&&l.counts.total?`${l.counts.total} ${l.counts.total===1?'Entwicklung':'Entwicklungen'}${l.counts.fortgeschrieben?` · ${l.counts.fortgeschrieben} fortgeschrieben`:''}`:'keine belastbare neue Entwicklung'}</p></a></li>`).join('')}</ol></section>`;
 };
