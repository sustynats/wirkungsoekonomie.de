# Glossar der Wirkungsökonomie

Stand: 2026-05-23T23:52:44.986Z

## Archetyp

Ein Archetyp beschreibt ein typisches Wirkungsprofil, das Bewertungen und Vergleiche strukturierbar macht.

In der Wirkungsökonomie ordnen Archetypen wiederkehrende Muster von Wirkung, Risiko, Datenlage und Steuerungsbedarf.

## Benchmark

Ein Benchmark ist ein Vergleichs- oder Schwellenwert, an dem Wirkung, Datenqualität oder Risiken eingeordnet werden.

Benchmarks übersetzen abstrakte Wirkungsziele in prüfbare Vergleichswerte, ohne selbst die Gesamtbewertung zu ersetzen.

## CSRD

CSRD ist die europäische Richtlinie zur Nachhaltigkeitsberichterstattung von Unternehmen.

Die Corporate Sustainability Reporting Directive erweitert und standardisiert Nachhaltigkeitsberichte. Für die WÖk ist sie Anschlussraum und Datenquelle, nicht die Kernlogik.

## Digitaler Produktpass

Der Digitale Produktpass macht produktbezogene Daten entlang von Wertschöpfung und Nutzung maschinenlesbar verfügbar.

In der WÖk ist der Digitale Produktpass eine mögliche Dateninfrastruktur für Produktwirkung, Lieferketten, Kreislauffähigkeit und Rückkopplung in Preise oder Beschaffung.

## ESG

ESG steht für Umwelt-, Sozial- und Governance-Faktoren in Unternehmen, Finanzierung und Reporting.

ESG hilft, Risiken und Nachhaltigkeitsaspekte zu strukturieren. Die Wirkungsökonomie nutzt ESG als Daten- und Anschlussraum, bewertet aber Wirkung als Zustandsveränderung und koppelt sie zurück.

## ESRS

ESRS sind europäische Standards für Nachhaltigkeitsberichterstattung.

European Sustainability Reporting Standards legen fest, welche Nachhaltigkeitsinformationen Unternehmen nach CSRD berichten.

## EU-Taxonomie

Die EU-Taxonomie klassifiziert wirtschaftliche Tätigkeiten nach definierten Nachhaltigkeitskriterien.

Die EU-Taxonomie ordnet Tätigkeiten anhand von Umweltzielen, DNSH, Mindestschutz und technischen Bewertungskriterien ein.

## FinalScore

Der FinalScore ist ein methodisch verdichtetes Ergebnis einer Wirkungsbewertung.

Der FinalScore verdichtet Einzelscores, Datenqualität, rote Linien und Reverse Merit Order. Er darf nicht als bloße Summe positiver und negativer Wirkungen missverstanden werden.

## GRI

GRI ist ein international verbreiteter Rahmen für Nachhaltigkeitsberichterstattung.

Die Global Reporting Initiative stellt Standards zur Offenlegung von Nachhaltigkeitsthemen bereit.

## Mensch, Planet und Demokratie

Mensch, Planet und Demokratie bilden den normativen Wirkungsrahmen der Wirkungsökonomie.

Der Rahmen erweitert die klassischen Nachhaltigkeitsziele um demokratische Korrekturfähigkeit, Rechtsstaatlichkeit, Diskursfähigkeit und institutionelles Vertrauen.

## NACE

NACE ist die europäische Klassifikation wirtschaftlicher Tätigkeiten.

NACE-Codes helfen, Unternehmen und Tätigkeiten statistisch und regulatorisch zu klassifizieren.

## Nachhaltigkeit

Nachhaltigkeit bedeutet in der Wirkungsökonomie: Ein System funktioniert auch morgen noch.

Nachhaltigkeit ist in der Wirkungsökonomie kein Zusatzlabel, keine Berichtspflicht und keine einzelne Umweltstrategie. Sie beschreibt die resiliente Fähigkeit eines Systems, Lebensgrundlagen, soziale Stabilität, wirtschaftliche Tragfähigkeit und demokratische Korrekturräume dauerhaft zu sichern.

## negative Wirkung

Negative Wirkung ist eine Zustandsveränderung, die Mensch, Planet oder Demokratie schädigt oder destabilisiert.

Negative Wirkung kann direkt, indirekt, kurzfristig oder langfristig auftreten und wird im Referenzrahmen bewertet.

## Netto-Wirkung

Netto-Wirkung beschreibt die bewertete Gesamtwirkung unter Berücksichtigung positiver, negativer und neutraler Wirkungen.

Netto-Wirkung ordnet Wirkungen im Referenzrahmen von SDGs, Agenda 2030 und SDG+ ein. Rote Linien, Wirkungsgrenzen und Nicht-Kompensation begrenzen die Verrechnung.

## NWI

Der Netto-Wirkungs-Index bewertet operative Netto-Wirkung.

Der NWI ist die operative Kennzahl für Netto-Wirkung. Er ist vom T-SROI zu unterscheiden, der Transformationswirkung bewertet.

## neutrale Wirkung

Neutrale Wirkung ist eine feststellbare Zustandsveränderung, die im Bewertungsrahmen weder positiv noch negativ ins Gewicht fällt.

Neutrale Wirkung ist nicht Wirkungslosigkeit, sondern eine bewertete Veränderung ohne relevante positive oder negative Einordnung.

## Nichtkompensationsprinzip

Das Nichtkompensationsprinzip verhindert, dass schwere Schäden durch positive Teilwirkungen schöngerechnet werden.

Das Prinzip verbindet Netto-Wirkung mit Wirkungsgrenzen und Reverse Merit Order. Es schützt zentrale Lebens-, Sozial- und Demokratiefunktionen.

## positive Netto-Wirkung

Positive Netto-Wirkung ist die Zielgröße der Wirkungsökonomie für Mensch, Planet und Demokratie.

Positive Netto-Wirkung verbindet tatsächliche Zustandsveränderungen, Bewertungsrahmen, Nicht-Kompensation und Rückkopplung in Entscheidungen.

## positive Wirkung

Positive Wirkung ist eine Zustandsveränderung, die im Referenzrahmen von SDGs, Agenda 2030 und SDG+ positiv bewertet wird.

Positive Wirkung ist keine Behauptung guten Willens, sondern eine bewertete Zustandsveränderung mit Bezugspunkt.

## Resonanzraum

Ein Resonanzraum ist ein sozialer, medialer oder institutioneller Raum, in dem Aussagen Wirkungspotenzial entfalten können.

Resonanzräume beeinflussen, ob Informationen, Narrative oder Entscheidungen aufgenommen, verstärkt, abgeschwächt oder umgedeutet werden.

## Reverse Merit Order

Reverse Merit Order stellt sicher, dass das schwächste kritische Wirkungsfeld die Bewertung begrenzt.

Die Reverse Merit Order ist eine Schutzlogik gegen Schönrechnung. Sie macht kritische Schäden entscheidungsrelevant und verbindet Netto-Wirkung mit Nicht-Kompensation.

## Scorecard

Eine Scorecard strukturiert Wirkungsfelder, Indikatoren, Datenqualität und Bewertung.

Scorecards verbinden WÖk-IDs, Benchmarks, Datenqualität, Wirkungsgrenzen und Bewertungslogik.

## SDG+

SDG+ ist die WÖk-Erweiterung der SDGs um demokratische, mediale, institutionelle und digitale Wirkungsfelder.

SDG+ ergänzt die SDGs dort, wo demokratische Korrekturfähigkeit, Diskursfähigkeit, institutionelles Vertrauen, Medienqualität und digitale Selbstbestimmung für positive Netto-Wirkung entscheidend sind. SDG+ ist keine offizielle UN-Kategorie.

## Social Credit

Social Credit bezeichnet die umfassende Bewertung von Menschen nach Verhalten, Konformität oder Loyalität.

Die Wirkungsökonomie bewertet keine Personen, keine Gesinnungen und keine Lebensstile. Bewertet werden Produkte, Tätigkeiten, Organisationen, Kapitalflüsse, öffentliche Mittel, Datenlagen und nachweisbare Wirkungszusammenhänge.

## Stranded Assets

Stranded Assets sind Vermögenswerte, die durch veränderte Risiken, Regeln, Märkte, Technologien oder ökologische Bedingungen an Wert verlieren.

In der WÖk zeigen Stranded Assets, dass negative oder verdrängte Wirkungen irgendwann in Kapitalwert, Versicherbarkeit, Finanzierung, Betriebskosten oder Nutzbarkeit zurückkehren können.

## T-SROI

T-SROI beschreibt transformationsbezogene Wirkung im Verhältnis zu eingesetzten Ressourcen.

Transformational Social Return on Investment ist ein Instrument zur Betrachtung von Transformationswirkung, nicht bloß klassischer Rendite. T-SROI ist keine operative Netto-Wirkungskennzahl. Diese Rolle liegt beim NWI.

## Transformationswirkung

Transformationswirkung verändert Systemlogiken, Standards, Anreize oder Handlungspfade.

Transformationswirkung beschreibt, ob eine Maßnahme Strukturen, Märkte, Regeln, Technologien oder kulturelle Muster so verändert, dass positive Netto-Wirkung wahrscheinlicher wird.

## Wirkmechanismus

Ein Wirkmechanismus beschreibt, wie ein Auslöser über Bedingungen, Akteure und Rückkopplungen Wirkung erzeugen kann.

Wirkmechanismen verbinden Wirkstoff-Analogie, Wirkungspotenzial, Wirkpfade, Resonanzräume und tatsächliche Zustandsveränderungen.

## Wirkstoff

Wirkstoff ist in der WÖk eine didaktische Analogie für einen Auslöser mit Wirkungspotenzial.

Der Begriff darf nur als gesellschaftlicher Wirkstoff im Sinne einer Analogie verwendet werden. Er beschreibt Auslöser, die unter bestimmten Bedingungen Wirkungspotenzial entfalten können. Ein Wirkstoff ist nicht selbst Wirkung.

## Wirkung

Wirkung ist die tatsächliche Veränderung von Zuständen.

Wirkung ist neutral und relational. Bewertet wird sie am Referenzrahmen der SDGs, Agenda 2030 und SDG+.

## Wirkungsarchitektur

Wirkungsarchitektur ist das Gesamtsystem aus Daten, Regeln, Institutionen, Anreizen, Governance, Kontrolle und Lernen.

Wirkungsarchitektur ist keine Website und kein Diagramm, sondern die Ordnung, in der Daten, Regeln, Institutionen, Kapital, Preise, Steuern, Beschaffung und demokratische Kontrolle zusammenwirken.

## Wirkungsbewertung

Wirkungsbewertung ordnet tatsächliche Zustandsveränderungen im Referenzrahmen von SDGs, Agenda 2030 und SDG+ ein.

Wirkungsbewertung verbindet Daten, Indikatoren, Benchmarks, Wirkungsgrenzen und normative Rahmung.

## Wirkungsblindheit

Wirkungsblindheit entsteht, wenn Entscheidungen ihre tatsächlichen Folgen nicht sehen oder nicht berücksichtigen.

Wirkungsblindheit kann in Preisen, Kennzahlen, Medien, Politik, Kapitalflüssen oder Institutionen entstehen.

## Wirkungsdaten

Wirkungsdaten machen Zustandsveränderungen, Risiken, Potenziale oder Bewertungsgrundlagen sichtbar.

Wirkungsdaten können Primärdaten, geprüfte Sekundärdaten, Produktdaten, Lieferkettendaten, Benchmarks, Prüfstatus und Unsicherheiten umfassen. Sie sind Grundlage für Scorecards, FinalScore, NWI und spätere Rückkopplung.

## Wirkungsdatenraum

Ein Wirkungsdatenraum macht relevante Wirkungsdaten strukturiert, interoperabel und prüfbar nutzbar.

Wirkungsdatenräume verbinden Berichtsdaten, Produktdaten, Lieferkettendaten, Standards, Register und Bewertungslogik.

## Wirkungseinkommen

Wirkungseinkommen beschreibt Einkommenslogiken, die gesellschaftlich relevante positive Wirkung berücksichtigen.

Das Konzept fragt, wie Sorgearbeit, Bildung, Prävention, demokratische Arbeit und andere wirksame Tätigkeiten besser sichtbar und abgesichert werden können.

## Wirkungsempfänger

Wirkungsempfänger sind Menschen, Gruppen, Ökosysteme, Institutionen oder Systeme, auf die Wirkung eintritt.

Der Begriff macht sichtbar, dass Wirkung relational ist und immer einen Bezugspunkt braucht.

## Wirkungsgrenze

Eine Wirkungsgrenze markiert Schäden, die nicht beliebig kompensiert oder schöngerechnet werden dürfen.

Wirkungsgrenzen betreffen unter anderem Menschenwürde, Kinderrechte, ökologische Lebensgrundlagen, Biodiversität, Rechtsstaatlichkeit und demokratische Stabilität.

## Wirkungshaushalt

Ein Wirkungshaushalt macht öffentliche Mittel nach erwarteter und geprüfter Wirkung steuerbar.

Der Wirkungshaushalt fragt, welche Zustände öffentliche Mittel verändern, welche Risiken entstehen und welche positive Netto-Wirkung erzeugt wird.

## Wirkungskompetenz

Wirkungskompetenz ist die Fähigkeit, Wirkung, Wirkungspotenzial, Wirkungsrisiken und Rückkopplungen zu erkennen und zu gestalten.

Wirkungskompetenz verbindet Systemverständnis, Datenkompetenz, ethische Urteilsfähigkeit und praktische Steuerung.

## Wirkungslenkung

Wirkungslenkung ist die bewusste Ausrichtung von Anreizen, Regeln und Entscheidungen auf positive Netto-Wirkung.

Wirkungslenkung nutzt Daten, Preise, Steuern, Kapital, Förderung, Beschaffung und Governance, um wirksame Entscheidungen wahrscheinlicher zu machen.

## Wirkungsnetz

Ein Wirkungsnetz beschreibt miteinander verbundene Wirkpfade, Akteure, Räume und Rückkopplungen.

Wirkungsnetze machen sichtbar, dass Wirkung selten linear entsteht und oft über Wechselwirkungen, Nebenwirkungen und Systemhebel läuft.

## Wirkungspfad

Ein Wirkungspfad beschreibt die nachvollziehbare Kette von Auslöser, Bedingungen, Wirkungspotenzial und möglicher Zustandsveränderung.

Wirkungspfade helfen, Annahmen, Daten, Risiken und Rückkopplungen in einer Wirkungsanalyse transparent zu machen.

## Wirkungspotenzial

Wirkungspotenzial ist die Möglichkeit, dass Wirkung eintreten kann.

Wirkungspotenzial beschreibt erwartbare oder mögliche Zustandsveränderungen unter bestimmten Bedingungen. Es braucht spätere Prüfung, ob Wirkung tatsächlich eingetreten ist.

## Wirkungspunkte

Wirkungspunkte sind eine mögliche produktbezogene Informations- oder Bonuslogik, keine Personenbewertung.

Wirkungspunkte dürfen nur als freiwillige, datensparsame und transparente Produkt- oder Bonusinformation gedacht werden. Sie dürfen keine Bürger:innen bewerten, keine Kaufprofile erzwingen und keine Nachteile bei Nichtteilnahme erzeugen.

## Wirkungsradar

Ein Wirkungsradar ist ein Orientierungsinstrument, das Wirkungsfelder, Risiken und blinde Flecken sichtbar macht.

Das Wirkungsradar unterstützt Analyse, Kommunikation und Priorisierung, ersetzt aber keine vollständige Wirkungsbewertung.

## Wirkungsrat

Der Wirkungsrat ist eine Governance-Idee zur Prüfung, Weiterentwicklung und demokratischen Kontrolle von Wirkungsmaßstäben.

Er verbindet Expertise, öffentliche Nachvollziehbarkeit, Korrekturfähigkeit und Schutz vor Gaming the System.

## Wirkungsraum

Ein Wirkungsraum ist ein Bereich, in dem Entscheidungen Zustände für Mensch, Planet oder Demokratie verändern.

Wohnen, Bildung, Gesundheit, Medien, Mobilität, Kultur oder Finanzsysteme können Wirkungsräume sein.

## Wirkungsrente

Wirkungsrente beschreibt eine rentenpolitische Perspektive, die Lebensleistung und gesellschaftliche Wirkung sichtbarer macht.

Das Konzept öffnet eine Wirkungslogik für Rente, Care-Arbeit, Prävention, Bildung und gesellschaftliche Stabilität.

## Wirkungsresilienz

Wirkungsresilienz ist die Fähigkeit eines Systems, unter Stress handlungsfähig zu bleiben und positive Netto-Wirkung zu sichern.

Wirkungsresilienz ist normativ gebunden. Auch destruktive Systeme können robust sein; im WÖk-Sinn ist Resilienz nur positiv, wenn sie Mensch, Planet und Demokratie stärkt.

## Wirkungsrisiko

Wirkungsrisiko ist die Möglichkeit, dass eine Entscheidung negative Wirkung erzeugt oder positive Wirkung verfehlt.

Wirkungsrisiken können ökologisch, sozial, demokratisch, finanziell, infrastrukturell oder reputationsbezogen zurückkehren.

## Wirkungsrückkopplung

Wirkungsrückkopplung führt bewertete Wirkung in Preise, Steuern, Kapital, Förderung, Beschaffung oder Entscheidungen zurück.

Reporting beschreibt. Rückkopplung verändert. Wirkungsrückkopplung ist der Mechanismus, durch den Wirkungsbewertung steuerungsrelevant wird.

## Wirkungssteuer

Eine Wirkungssteuer koppelt bewertete Wirkung in steuerliche Anreize zurück.

Wirkungssteuern sind Instrumente, um positive Netto-Wirkung günstiger und negative Wirkung teurer zu machen.

## Wirkungssteuergesetz

Das Wirkungssteuergesetz ist ein Rahmengesetz-Konzept für steuerliche Wirkungsrückkopplung.

Das WStG bildet den Rahmen, in dem Wirkungsklassen, Daten, Governance und Steuerungslogiken rechtlich strukturiert werden können.

## Wirkungsträger

Ein Wirkungsträger ist ein Produkt, eine Handlung, Entscheidung, Institution oder Struktur, über die Wirkung vermittelt wird.

Wirkungsträger können materiell oder immateriell sein: Produkte, Preise, Regeln, Narrative, Gebäude, Plattformen oder Infrastrukturen.

## Wirkungsumsatzsteuer

Die Wirkungsumsatzsteuer ist ein Konzept, Umsatzsteuerlogik nach Produkt- oder Leistungswirkung zu differenzieren.

Die WUStG-Logik nutzt Wirkungsdaten, WÖk-IDs, Scorecards und Reverse Merit Order für steuerliche Rückkopplung.

## Wirkungswahrheit

Wirkungswahrheit meint Wirkungsnähe, Datenklarheit und Transparenz über Folgen.

Der Begriff beschreibt, ob Entscheidungen, Preise, Narrative oder Kennzahlen den tatsächlichen Folgen näherkommen oder sie verdecken.

## WÖk-ID

Eine WÖk-ID ist eine strukturierte Kennung für Wirkungsindikatoren, Wirkungsfelder oder Bewertungsitems.

WÖk-IDs verbinden SDGs, SDG+, Targets, Indikatorfamilien, Datenquellen und Bewertungslogik in einem Register.

## WStG

WStG steht für Wirkungssteuergesetz, den Rahmengesetz-Entwurf für steuerliche Wirkungsrückkopplung.

Das WStG strukturiert die rechtliche Rahmung von Wirkungssteuerung und Wirkungsrückkopplung.

## WUStG

WUStG steht für Wirkungsumsatzsteuergesetz, den Entwurf für produkt- und leistungsbezogene Wirkungsrückkopplung.

Das WUStG beschreibt, wie Produkt- und Leistungswirkung über Umsatzsteuerlogiken differenziert werden könnte.
