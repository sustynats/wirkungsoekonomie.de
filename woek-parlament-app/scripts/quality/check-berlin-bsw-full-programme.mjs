...
assert.equal(bsw.terminal_object_count, 575, "BSW exact issue #240 terminal stock drifted");
assert.equal(bsw.remaining_review_envelope_count, 33, "BSW page-envelope residual must be physical PDF pages 34-66");
assert.equal(bsw.remaining_exact_object_count, 0, "BSW exact child residual must remain zero");
assert.equal(bsw.remaining_review_scope_count, 33, "BSW finite residual must be 33 page envelopes");
assert.deepEqual(
...
assert.equal(bsw.remaining_review_objects.length, 0, "exact child residual set drifted");
...
assert.ok(
  ledger.effect_atoms.filter((atom) => atom.pdf_page < 14 || atom.pdf_page > 33).every((atom) => !currentIds.has(atom.atom_id)),
  "rejected BSW generic terminal outside explicit pages 14-33 leaked into current truth",
);
