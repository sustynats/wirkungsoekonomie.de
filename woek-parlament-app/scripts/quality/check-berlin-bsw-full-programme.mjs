...
assert.equal(bsw.terminal_object_count, 237, "BSW exact issue #240 terminal stock drifted");
assert.equal(bsw.remaining_review_envelope_count, 45, "BSW page-envelope residual must be physical PDF pages 22-66");
...
assert.ok(
  ledger.effect_atoms.filter((atom) => atom.pdf_page < 14 || atom.pdf_page > 21).every((atom) => !currentIds.has(atom.atom_id)),
  "rejected BSW generic terminal outside explicit pages 14-21 leaked into current truth",
);
