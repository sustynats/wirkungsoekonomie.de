...
assert.equal(bsw.terminal_object_count, 198, "BSW exact issue #240 terminal stock drifted");
assert.equal(bsw.remaining_review_envelope_count, 47, "BSW page-envelope residual must be physical PDF pages 20-66");
assert.equal(bsw.remaining_exact_object_count, 8, "BSW P19 exact child residual drifted");
assert.deepEqual(
...
assert.ok(
  ledger.effect_atoms.filter((atom) => atom.pdf_page < 14 || atom.pdf_page > 19).every((atom) => !currentIds.has(atom.atom_id)),
  "rejected BSW generic terminal outside explicit pages 14-19 leaked into current truth",
);
