...
const files = ["aktionsplan-nachhaltigkeit-2026-meta.md", "aktionsplan-nachhaltigkeit-2026-missions-01-10.jsonl", "aktionsplan-nachhaltigkeit-2026-missions-11-19.jsonl"];
const metaId = "WOEK-META-BUND-AKTIONSPLAN-NACHHALTIGKEIT-2026";
const missions = files.slice(1).flatMap((file) => readFileSync(path.join(dataDir, file), "utf8").split(/\r?\n/).filter(Boolean).map(JSON.parse));
...
const report = {
  ...
  records: { meta: 1, missions: missions.length, deep_dives: 2 },
  ...
  analysis_layers_by_object: {
    ...
    ...Object.fromEntries(missions.map((mission) => [mission.id, ["DNS_REFERENCE", "ACTUAL_IMPACT_ANALYSIS", "COVERAGE_SCOPE", "REALITY_CHECK", ...(mission.id === "WOEK-AKN-2026-M02" || mission.id === "WOEK-AKN-2026-M04" ? ["PROBLEM_REVIEW", "GOAL_REVIEW", "MATERIAL_OMISSIONS", "POLICY_COHERENCE", "DELIVERY_FEASIBILITY", "RESOURCE_FINANCING", "SPATIAL_DISTRIBUTION", "INTERNATIONAL_LEAKAGE", "ROBUSTNESS_STRESS_TEST", "REVERSIBILITY_LOCKIN", "FALSIFICATION_TRIGGERS", "LIFECYCLE_TRACEABILITY", "VERSION_DELTA"] : [])]])),
  },
  ...
  invariants: { DNS_REFERENCE_IS_NOT_CAUSALITY: !metaText.includes("DNS-Ziel erreicht durch"), NO_AGGREGATE_SCORE: !metaText.includes("Gesamtscore"), NO_CODEX_RECOMMENDATION: metaText.includes("kein fachlich freigegebener RecommendationRecord") },
  failures,
};
