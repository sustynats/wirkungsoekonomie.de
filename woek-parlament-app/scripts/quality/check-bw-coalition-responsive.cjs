...
const baseUrl = (process.env.WOEK_BW_COALITION_BROWSER_BASE_URL ?? "http://127.0.0.1:3018").replace(/\/$/, "");
const output = process.env.WOEK_BW_COALITION_BROWSER_REPORT ?? path.resolve("data/autopilot/audit/2.3-remediated/BW-COALITION-2026-2031-RESPONSIVE-A11Y.json");
const chrome = process.env.WOEK_AUDIT_CHROMIUM;
const widths = [320, 360, 375, 390, 428, 768, 1024, 1280, 1440];
const coalitionRoute = "/laender/baden-wuerttemberg/mandat-und-praxis";
const routes = [
  coalitionRoute,
  "/laender/baden-wuerttemberg",
  "/laender/baden-wuerttemberg/regierung",
  "/suche",
  "/quellen/quelle-d82b7679ec0f9d98",
  "/quellen/quelle-e7464f97069f2958",
  "/quellen/quelle-453b1bfc2e4a6276",
];
...
(async () => {
      ...
      for (const width of widths) {
        ...
        const metrics = navigationError ? null : await page.evaluate((expectedRoute) => {
          ...
          return {
            ...
            coalitionHeading: expectedRoute === location.pathname ? /Aus Verantwortung fürs Land/.test(visible) : true,
            objectLeak: /Objects are not valid as a React child|\[object Object\]/.test(visible),
            internalIdLeak: /\bBW-IMPACT-\d{4}-\d{2}\b/.test(visible),
            rawTokens: [...new Set(visible.match(/\b(?:DEEP_REVIEW|HIGH_MATERIALITY_REVIEW|RecommendationRecord|[A-Z][A-Z0-9]+_[A-Z0-9_]+)\b/g) ?? [])],
          };
        }, coalitionRoute);
        let axeViolations = [];
  ...
  const report = {
    schema_version: "woek-bw-coalition-responsive-1.0",
