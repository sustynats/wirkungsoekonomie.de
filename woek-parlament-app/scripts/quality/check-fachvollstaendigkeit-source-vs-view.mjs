...
const euEditorialExclusions = new Set(["EU-IMPACT-2026-001", "EU-IMPACT-2026-003", "EU-IMPACT-2026-005", "EU-IMPACT-2026-019", "EU-IMPACT-2026-020"]);
...
for (const review of targets) {
  ...
  const pass = response.status === 200 && missingProse.length === 0 && rawIds.length === 0 && rawEnums.length === 0 && !html.includes("/WOEK/");
  results.push({ recommendation_id: review.recommendation_id, impact_case_id: review.impact_case_id, route, expected: "PUBLIC_COMMON_TARGETS", status: response.status, missing_prose: missingProse, raw_ids: rawIds, raw_enums: rawEnums, pass });
