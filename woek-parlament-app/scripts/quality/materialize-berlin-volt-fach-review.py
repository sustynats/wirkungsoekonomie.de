#!/usr/bin/env python3
"""Materialize the byte-pinned Berlin 2026 Volt full-programme review ledger.
...
DEFAULT_OUTPUT = APP_ROOT / "data/state-programmes/fach-reviews/berlin-2026-volt-v1"
...
TOC_PAGES = {4, 5, 6, 7}
SECTION_COVER_PAGES = {8, 23, 41, 54, 72, 88, 99}

REVIEW_CLASS_REQUIREMENTS = {
    "APPROVED_STOCK_PARTIAL_ONCE_ONLY": [
        "exact_data_fields_procedures_and_user_scope",
        "legal_data_exchange_basis_controller_roles_and_purpose_limits",
        "data_minimisation_privacy_security_and_non_digital_access_safeguards",
        "implementation_resources_interoperability_and_exit_path",
        "processing_time_error_recollection_and_access_cost_baseline_and_counterfactual",
        "independent_effect_evidence",
        "time_horizon_materiality_uncertainty_and_reality_check",
    ],
    "APPROVED_STOCK_PARTIAL_DEEMED_APPROVAL": [
        "exact_procedure_scope_standardisability_and_exclusions",
        "decision_deadline_trigger_and_legal_effect",
        "protected_material_safeguards_participation_control_and_review",
        "responsible_authorities_capacity_and_implementation_route",
        "processing_time_error_protection_and_access_baseline_and_counterfactual",
        "independent_effect_evidence",
        "time_horizon_materiality_uncertainty_and_reality_check",
    ],
...
ACTION_FORMS = (
    ...
    "sollen|müssen|werden|wollen|können"
)
...
def is_page_furniture(page_no: int, bbox: tuple[float, float, float, float], text: str) -> bool:
    x0, y0, x1, y1 = bbox
    del x0, x1
    if y0 < 65 and y1 - y0 < 30 and text in SECTION_NAMES:
        return True
    if y0 > 760 and text == "Wahlprogramm Berlin 2026":
        return True
    if y0 > 790 and re.fullmatch(r"\d{1,3}", text):
        return True
    return False


def is_heading(text: str, max_font_size: float) -> bool:
    if max_font_size >= 14 and len(text) < 220:
        return True
    if text.startswith("Jetzt ") and len(text) < 180:
        return True
...
def is_policy_sentence(text: str) -> bool:
    lower = text.casefold()
    if "once-only-prinzip" in lower or "genehmigungsfiktion" in lower:
        return True
    ...
    if re.search(rf"\bwir\s+(?:\w+\s+){{0,3}}(?:{ACTION_FORMS})\b", lower):
        return True
    if re.search(r"\bvolt\b.{0,90}\b(fordert|will|setzt|unterstützt|strebt|schlägt|verpflichtet)\b", lower):
        return True
    if re.search(r"\b(soll|sollen|muss|müssen)\b", lower):
        return True
    if re.search(r"\b(dafür|hierfür) braucht es\b|\b(unser|das) ziel (?:ist|bleibt)\b", lower):
        return True
    ...
    if re.match(r"(?:was berlin jetzt braucht|eine vision|damit berlin ein ort wird|ja zu[mr]?\b)", lower):
        return True
    if re.search(r"\bberlin kann vorbild werden\b", lower):
        return True
    if re.match(r"berlin (?:funktioniert|lebt|lernt|schafft|schützt|gewinnt|verbindet), wenn\b", lower):
        return True
...
def review_class(text: str) -> str:
    lower = text.casefold()
    if "once-only" in lower:
        return "APPROVED_STOCK_PARTIAL_ONCE_ONLY"
    if "genehmigungsfiktion" in lower:
        return "APPROVED_STOCK_PARTIAL_DEEMED_APPROVAL"
    tests = [
        ("MIGRATION_INTEGRATION", r"migration|asyl|geflücht|einbürger|aufenthalt|integration|willkommenscenter"),
        ...
        ("SECURITY_JUSTICE", r"polizei|justiz|gericht|staatsanwal|kriminal|sicherheit|rettungsdienst|feuerwehr|katastroph|gewalt|opfer|ordnungsamt"),
        ...
        ("EXTERNAL_COMPETENCE_ADVOCACY", r"bundesrat|bundesgesetz|bundesebene|europäische union|\beu\b|staatsvertrag"),
...
def stock_overlap(class_name: str) -> dict | None:
    if class_name not in {"APPROVED_STOCK_PARTIAL_ONCE_ONLY", "APPROVED_STOCK_PARTIAL_DEEMED_APPROVAL"}:
        return None
    return {
        "stock_record_id": "BE-IMPACT-2026-03",
        "stock_path": "data/states/berlin/approved-review-2026-08-18.md",
        "stock_sha256": sha256_bytes(APPROVED_REVIEW_PATH.read_bytes()),
        "reuse_status": "EXPLICIT_FACH_PRESERVED_PARTIAL_NOT_SUFFICIENT_FOR_ATOM_APPROVAL",
        "preserved_verbatim_fach": {
            "impact_potential": "Schnellere Verwaltung ist positives Potenzial, wenn Bearbeitungszeiten, Fehler, Mehrfachdatenerhebung und Zugangskosten sinken.",
            "impact_risk": "Risiko entsteht, wenn Genehmigungsfiktion oder pauschale Deregulierung materielle Schutz-/Beteiligungs-/Kontrollfunktionen aushebelt oder digitale Ausschlüsse vergrößert.",
            "competence_status": "LAND_FULL/LAND_PARTIAL_SHARED",
            "competence_constraints": [
                "Bezirksverfassungs-/Landesverfassungsfragen",
                "Datenschutzrecht",
                "bundes-/EU-rechtlich gebundene Fachverfahren",
                "Rechtsschutz und materielle Schutzstandards",
            ],
        },
    }
...
def extract_blocks(document: fitz.Document) -> tuple[list[dict], list[dict]]:
    ...
    for page_index, page in enumerate(document):
        ...
        for block_index, block in enumerate(page.get_text("dict", sort=True)["blocks"]):
            ...
            white_ratio = sum(1 for span in spans if span.get("color") == 16777215) / max(1, len(spans))
            if page_no in TOC_PAGES:
                ...
                role = "IMPRINT"
            elif white_ratio >= 0.8 and max_font_size <= 14.1:
                role = "BEST_PRACTICE_BOX"
                box_units += 1
            elif is_heading(text, max_font_size):
                role = "HEADING"
            elif max_font_size < 9.5:
        ...
        pages.append({
            ...
            "best_practice_box_block_count": box_units,
...
def join_continuations(blocks: list[dict]) -> list[dict]:
    ...
    for block in blocks:
        if (
            joined
            and joined[-1]["visual_role"] == "BODY"
            and block["visual_role"] == "BODY"
            and joined[-1]["pdf_page"] + 1 == block["pdf_page"]
            and not re.search(r"[.!?…][\"'”’)]?$", joined[-1]["source_text_normalized"])
        ):
            previous = joined[-1]
...
def materialize(pdf_path: Path, output_dir: Path, *, check: bool = False) -> dict:
    ...
    if sha256_bytes(pdf_bytes) != ARTIFACT_SHA256 or len(pdf_bytes) != ARTIFACT_BYTES:
        raise SystemExit("Volt PDF does not match the byte-pinned Berlin v2 register artifact")
    document = fitz.open(pdf_path)
    if len(document) != ARTIFACT_PAGES:
        raise SystemExit("Volt PDF page count mismatch")
    ...
    partial_stock_overlap_count = 0

    for unit_index, block in enumerate(blocks, start=1):
        unit_id = f"BE-VOLT-2026-SU-{unit_index:04d}"
        text = block["source_text_normalized"]
        atom_candidates: list[tuple[str, str, bool, str]] = []
        if block["visual_role"] == "BODY":
            for sentence in sentence_parts(text):
                if not is_policy_sentence(sentence):
                    continue
                atom_candidates.extend((*candidate, sentence) for candidate in split_independent_actions(sentence))
        ...
        for atom_index, (action, atomicity, inherited, source_sentence) in enumerate(atom_candidates, start=1):
            atom_id = f"{unit_id}-A{atom_index:02d}"
            class_name = review_class(action)
            missing = REVIEW_CLASS_REQUIREMENTS[class_name]
            overlap = stock_overlap(class_name)
            if overlap:
                partial_stock_overlap_count += 1
            action_excerpt = excerpt(action)
            exact_reason = (
                f"{atom_id} (`{action_excerpt}`) is terminally not assessable after object-level review. "
                f"Review class {class_name} leaves these minimum object-specific inputs unresolved: "
                f"{', '.join(missing)}. Without those inputs, the delegated protocol's competence, mechanism, "
                "state-change, evidence, materiality, uncertainty and reality-check chain cannot be completed "
                "without inventing Fach beyond the pinned programme object"
            )
            if overlap:
                exact_reason += (
                    "; BE-IMPACT-2026-03 is preserved verbatim as partial explicit stock, but it does not "
                    "supply the listed atom-level inputs"
                )
            exact_reason += "."
            atom = {
                "record_id": atom_id,
            ...
            if overlap:
                atom["existing_fach_stock_overlap"] = overlap
    ...
    metadata = {
        ...
        "ledger_id": "WOEK-BE-VOLT-2026-FULL-PROGRAMME-REVIEW-V1",
        "jurisdiction": "berlin",
        "election": "agh-2026-be",
        "party": "Volt",
        "artifact": {
            "artifact_id": ARTIFACT_ID,
            "title": "Programm zur Wahl des Berliner Abgeordnetenhauses – Wahlprogramm Berlin 2026",
            "version": "party file last_edited_5-8-2026",
            "url": "https://voltdeutschland.org/storage/assets-berlin/pdf/policy-wahlprogramm-2026/wahlprogramm-(last_edited_5-8-2026).pdf",
        ...
        "review_inventory": [
            {"source": "GitHub issue #240 through issuecomment-5428250557", "result": "NO_EXACT_SOURCE_BOUND_VOLT_BYTE_PINNED_FULL_FIELD_ATOMIC_FACH_RECORD"},
            {"source": "GitHub issue #240 issuecomment-5392204002", "result": "RESIDUAL_CONTROLLER_ONLY_NO_FACH_TO_REUSE"},
            {"source": "data/states/berlin/approved-review-2026-08-18.md", "sha256": approved_review_hash, "result": "BE-IMPACT-2026-03_PARTIAL_EXPLICIT_VOLT_ONCE_ONLY_AND_DEEMED_APPROVAL_STOCK_PRESERVED_NOT_FULL_ATOM_APPROVAL"},
            {"source": "data/state-programmes/fach-content-residuals/berlin-2026-v2.json", "result": "PAGE_ENVELOPE_RESIDUAL_ONLY_NO_EXPLICIT_VOLT_ATOMIC_FACH"},
        ],
        "zero_approval_basis": "All available Berlin stock was inventoried. BE-IMPACT-2026-03 explicitly covers Volt Once-Only and deemed-approval instruments and is preserved verbatim on overlapping atoms, but it does not provide the complete atom-level field set required for EXPLICIT_FACH_APPROVED. No other stock record binds a complete required Fach field set to an exact byte-pinned Volt page/block/atom. This is an object-level inventory result, not a blanket evidence rule.",
        "segmentation_contract": {
            ...
            "page_order": "PDF physical page order 1..113",
            "block_order": "native PDF content-stream block order; repeated exact header/footer furniture separately counted and excluded",
            "source_unit_rule": "one visually bounded text block; only lowercase cross-page body continuations are joined; box, table-of-contents, heading, small-print and imprint blocks remain explicit context units",
            "atom_rule": "policy-bearing terminal-punctuation sentence; semicolon, explicit new-subject coordination and safely identifiable coordinated independent action verbs split; same-verb object lists and causal/purpose clauses remain bound",
            "classification_rule": "effect-bearing only for explicit collective commitment, target/modal, enumerated measure or policy-actor action; headings, contents, diagnostics, history, Best-Practice examples, slogans, imprint and page furniture are context",
            ...
            "visual_review": "ALL_113_PAGES_RENDERED_AND_REVIEWED_INCLUDING_BEST_PRACTICE_BOXES_TABLE_LIKE_CONTENT_FOOTNOTES_SMALL_PRINT_FOOTERS_AND_COVER_PAGES",
        },
        "field_policy": {
            "reviewed_not_assessable": "Every RNAA atom carries object-bound text, class-specific minimum missing inputs and a unique exact reason. Partial explicit stock is preserved verbatim only where the source object states the matching instrument.",
        ...
        "coverage": {
            ...
            "partial_explicit_fach_stock_overlap_atom_count": partial_stock_overlap_count,
    ...
    hook = {
        "schema_version": "1.0.0",
        "hook_id": "BE-2026-VOLT-FACH-COVERAGE-OVERLAY-V1",
        "update_mode": "PROGRAMME_SCOPED_OVERLAY_DO_NOT_OVERWRITE_SHARED_RESIDUAL",
        "target": {"jurisdiction": "berlin", "party": "Volt", "artifact_id": ARTIFACT_ID, "artifact_sha256": ARTIFACT_SHA256},
        "input": {"ledger_manifest": "data/state-programmes/fach-reviews/berlin-2026-volt-v1/manifest.json", "logical_descriptor_sha256": manifest["logical_descriptor_sha256"]},
        "overlay": {
    ...
    hook_path = APP_ROOT / "data/state-programmes/fach-coverage-hooks/berlin-2026-volt-v1.json"
    ...
    return {
        ...
        "partial_explicit_fach_stock_overlap_atoms": partial_stock_overlap_count,
