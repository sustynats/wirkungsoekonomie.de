...
const LEDGER_DIR = path.join(APP_ROOT, 'data/state-programmes/fach-reviews/berlin-2026-gruene-v1');
const MANIFEST_PATH = path.join(LEDGER_DIR, 'manifest.json');
const HOOK_PATH = path.join(APP_ROOT, 'data/state-programmes/fach-coverage-hooks/berlin-2026-gruene-v1.json');
const REGISTER_PATH = path.join(APP_ROOT, 'data/state-programmes/current-source-registers/berlin-2026-v2.json');
const ARTIFACT_ID = 'BE-AGH-2026-GRUENE-WAHLPROGRAMM';
const ARTIFACT_SHA256 = 'db07990ef613bf239691980873dbfaff3df98e07e8a27c5e05edc2363d9dade2';
const PARTY = 'BÜNDNIS 90/DIE GRÜNEN';
const EXPECTED = {
  sourceUnits: 354,
  effectUnits: 327,
  contextUnits: 27,
  ...
  multiAtomUnits: 324,
  effectAtoms: 4119,
  logicalDescriptor: 'b1e15d8a65cdeb099abdf28bc47ecbdf14e3bdc1bfea59eb9a75bc78979f98b8',
  hookDescriptor: '0e358c9a3084fd03909e99666a70e501531ecebcb4b6d8239b4e9ddf34ca1fdd',
};
const BOX_PAGES = [42, 43, 76, 77, 126, 127, 176, 177, 220, 221, 252, 253];
...
export function loadBerlinGrueneReviewBundle() {
...
export function validateBerlinGrueneReviewBundle(bundle, { verifyLogicalDescriptor = true } = {}) {
  ...
  assert.equal(sha256(canonicalJson(unhashedManifest)), manifest.manifest_sha256, 'GRUENE manifest descriptor mismatch');
  if (verifyLogicalDescriptor) {
    assert.equal(
      ...
      'GRUENE logical ledger descriptor mismatch',
    );
  }

  assert.equal(metadata.ledger_id, 'WOEK-BE-GRUENE-2026-FULL-PROGRAMME-REVIEW-V1');
  ...
  assert.equal(metadata.artifact.byte_length, 2212991);
  assert.equal(metadata.artifact.page_count, 256);
  ...
  assert.equal(metadata.segmentation_contract.visual_review.includes('ALL_256_PHYSICAL_PAGES'), true);
  assert.equal(metadata.segmentation_contract.visual_review.includes('DARK_GREEN_ZUKUNFTSPROJEKTE_BOXES'), true);
  ...
  const registered = register.parties.find((party) => party.party === PARTY);
  assert.ok(registered, 'GRUENE missing from current-source register');
  ...
  assert.equal(registered.canonical_artifact.byte_length, 2212991);
  assert.equal(registered.canonical_artifact.page_count, 256);
  assert.equal(registered.canonical_artifact.artifact_url, metadata.artifact.url);

  assert.equal(metadata.pages.length, 256);
  assert.deepEqual(metadata.pages.map((page) => page.pdf_page), Array.from({ length: 256 }, (_, index) => index + 1));
  assert.ok(metadata.pages.every((page) => page.visual_reviewed && page.page_status !== 'UNREVIEWED'));
  assert.deepEqual(metadata.pages.filter((page) => page.source_unit_count === 0).map((page) => page.pdf_page), [2, 6, 251, 254]);
  ...
  assertUnique(sourceUnits.map((unit) => unit.source_unit_id), 'GRUENE source_unit_id');
  assertUnique(effectAtoms.map((atom) => atom.atom_id), 'GRUENE atom_id');
  assertUnique(effectAtoms.map((atom) => atom.record_id), 'GRUENE record_id');
  assertUnique(effectAtoms.map((atom) => atom.exact_reason), 'GRUENE object-specific exact_reason');
  ...
  for (const unit of sourceUnits) {
    assert.match(unit.source_unit_id, /^BE-GRUENE-2026-SU-\d{4}$/);
    assert.ok(unit.pdf_pages.includes(unit.pdf_page));
    assert.ok(unit.pdf_pages.every((page, index) => page === unit.pdf_page + index && page >= 1 && page <= 256));
  ...
  for (const atom of effectAtoms) {
    ...
    assert.match(atom.atom_id, /^BE-GRUENE-2026-SU-\d{4}-A\d{2}$/);
  ...
  assert.equal(reviewClasses.size, 20, 'GRUENE exact reasons collapsed into blanket review classes');
  ...
  const boxedUnits = sourceUnits.filter((unit) => BOX_PAGES.some((page) => unit.pdf_pages.includes(page)));
  assert.ok(BOX_PAGES.every((page) => boxedUnits.some((unit) => unit.effect_bearing && unit.pdf_pages.includes(page))), 'Zukunftsprojekte box page lost effect-bearing source');
  assert.ok(boxedUnits.filter((unit) => unit.effect_bearing).flatMap((unit) => unit.atom_ids).length >= 127, 'Zukunftsprojekte atom coverage regressed');

  assert.equal(coverage.expected_page_count, 256);
  assert.equal(coverage.reviewed_page_count, 256);
  ...
  assert.equal(sha256(canonicalJson(unhashedHook)), hook.descriptor_sha256, 'GRUENE hook descriptor mismatch');
  ...
  assert.equal(hook.overlay.reviewed_page_count, 256);
  assert.equal(hook.overlay.source_unit_count, EXPECTED.sourceUnits);
  assert.equal(hook.overlay.effect_atom_count, EXPECTED.effectAtoms);
...
if (invokedDirectly) {
  ...
  if (artifactIndex >= 0) {
    assert.ok(process.argv[artifactIndex + 1], '--artifact requires a path');
    execFileSync('python3', [
      path.join(APP_ROOT, 'scripts/quality/materialize-berlin-gruene-fach-review.py'),
      '--artifact', process.argv[artifactIndex + 1], '--check',
    ], { cwd: APP_ROOT, stdio: 'inherit' });
  }
  console.log(JSON.stringify(validateBerlinGrueneReviewBundle(loadBerlinGrueneReviewBundle())));
}
