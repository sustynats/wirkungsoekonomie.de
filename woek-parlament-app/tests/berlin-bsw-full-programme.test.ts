...
test("BSW Berlin preserves protected P1-14 and exact issue #240 P15-29 terminals", () => {
  ...
  assert.equal(bsw.terminal_objects.length, 469);
  assert.equal(new Set(bsw.terminal_objects.map((item: { object_id: string }) => item.object_id)).size, 469);
...
test("BSW P23 and P26-P29 are closed while P24/P25 exact children and pages 30-66 remain current", () => {
  const bsw = residual.programmes.find((programme: { party: string }) => programme.party === "BSW");
  assert.ok(bsw);
  assert.equal(bsw.programme_analysis_complete, false);
  assert.equal(bsw.fach_state, "GENUINE_FACH_REVIEW_REQUIRED");
  assert.equal(bsw.remaining_review_envelope_count, 37);
  assert.equal(bsw.remaining_exact_object_count, 22);
  assert.equal(bsw.remaining_review_scope_count, 59);
...
test("the former BSW atom ledger is retained only as rejected historical evidence", () => {
  ...
  assert.ok(ledger.effect_atoms
    .filter((atom: { pdf_page: number }) => atom.pdf_page < 14 || atom.pdf_page > 29)
  ...
  assert.equal(residual.summary.remaining_page_review_envelopes, 1252);
  assert.equal(residual.summary.remaining_exact_effect_objects_identified, 22);
  assert.equal(residual.summary.remaining_review_scope_count, 1274);
});
