...
test("BSW Berlin preserves protected P1-14 and exact issue #240 P15-25 terminals", () => {
  ...
  assert.ok(bsw);
  assert.equal(bsw.terminal_object_count, 356);
  assert.equal(bsw.terminal_objects.length, 356);
  assert.equal(new Set(bsw.terminal_objects.map((item: { object_id: string }) => item.object_id)).size, 356);
  assert.deepEqual(bsw.terminal_status_counts, {
    EXPLICIT_FACH_APPROVED: 160,
    REVIEWED_NOT_ASSESSABLE_WITH_EXACT_REASON: 24,
    NON_EFFECT_CONTEXT_REVIEWED: 133,
    SOURCE_UNIT_RECLASSIFIED_VERSIONED: 39,
  });
});

test("BSW P23 is closed while P24/P25 exact children and pages 26-66 remain current", () => {
  const bsw = residual.programmes.find((programme: { party: string }) => programme.party === "BSW");
  assert.ok(bsw);
  assert.equal(bsw.programme_analysis_complete, false);
  assert.equal(bsw.fach_state, "GENUINE_FACH_REVIEW_REQUIRED");
  assert.equal(bsw.remaining_review_envelope_count, 41);
  assert.equal(bsw.remaining_exact_object_count, 22);
  assert.equal(bsw.remaining_review_scope_count, 63);
  assert.equal(bsw.remaining_review_objects.length, 22);
  assert.deepEqual(
...
test("the former BSW atom ledger is retained only as rejected historical evidence", () => {
  ...
  assert.ok(ledger.effect_atoms
    .filter((atom: { pdf_page: number }) => atom.pdf_page < 14 || atom.pdf_page > 25)
  ...
  assert.equal(residual.summary.remaining_page_review_envelopes, 1256);
  assert.equal(residual.summary.remaining_exact_effect_objects_identified, 22);
  assert.equal(residual.summary.remaining_review_scope_count, 1278);
});
