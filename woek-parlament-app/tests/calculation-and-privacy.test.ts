...
import { recurringUnsubscribeToken, subscriptionDeliveryReady, subscriptionRequestSchema, validRecurringUnsubscribeToken } from "@/lib/wirkungsradar/subscriptions";
import { dailyDigestEmail } from "@/lib/wirkungsradar/email-templates";
...
test("recurring digest unsubscribe tokens are signed and bound to one subscription", () => {
  const previous = process.env.WIRKUNGSRADAR_UNSUBSCRIBE_SIGNING_SECRET;
  process.env.WIRKUNGSRADAR_UNSUBSCRIBE_SIGNING_SECRET = "test-secret-with-at-least-thirty-two-characters";
  try {
    const token = recurringUnsubscribeToken("subscription-a");
    assert.equal(validRecurringUnsubscribeToken("subscription-a", token), true);
    assert.equal(validRecurringUnsubscribeToken("subscription-b", token), false);
    assert.equal(validRecurringUnsubscribeToken("subscription-a", `${token}x`), false);
  } finally {
    if (previous === undefined) delete process.env.WIRKUNGSRADAR_UNSUBSCRIBE_SIGNING_SECRET;
    else process.env.WIRKUNGSRADAR_UNSUBSCRIBE_SIGNING_SECRET = previous;
  }
});

test("daily digest names publisher, responsible person, legal pages, unsubscribe and epistemic limits", () => {
  const message = dailyDigestEmail({
    dateLabel: "18. August 2026",
    portalUrl: "https://parlament.wirkungsoekonomie.de",
    privacyUrl: "https://wirkungsoekonomie.de/datenschutz.html",
    imprintUrl: "https://wirkungsoekonomie.de/impressum.html",
    unsubscribeUrl: "https://parlament.wirkungsoekonomie.de/wirkungsradar-updates/abmelden?subscription=a&token=b",
    items: [{
      title: "Beispielanalyse",
      summary: "Ein fachlich freigegebener Wirkungsfall wurde veröffentlicht.",
      url: "https://parlament.wirkungsoekonomie.de/wirkungsfaelle/beispiel",
      section: "WIRKUNGSANALYSE",
    }],
  });
  for (const required of ["Institut für Wirkungsökonomie", "Natalie Weber", "Datenschutz", "Impressum", "Hinweise abbestellen", "Wirkungspotenzial ist keine bereits eingetretene Wirkung", "Wir messen weder Öffnungen noch Klicks"]) {
    assert.match(`${message.text}\n${message.html}`, new RegExp(required));
  }
});
