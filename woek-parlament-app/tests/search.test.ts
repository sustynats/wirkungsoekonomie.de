...
test("search finds public cases by words from the decision content", () => {
  ...
  assert.ok(results.every((item) => [item.title, item.plainTitle, item.summary, item.whatIsDecided, item.intendedGoal, item.parliamentaryStatus, item.analysisStatus, ...item.impactPath, ...item.affectedGroups, ...item.questions, ...item.sources.map((source) => `${source.title} ${source.publisher}`)].join(" ").toLocaleLowerCase("de-DE").normalize("NFD").replace(/[\u0300-\u036f]/g, "").includes("zugang")));
});
