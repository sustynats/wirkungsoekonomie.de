...
test('P55 cannot silently omit an exact-span child or revive a programme terminal', () => {
  ...
  assert.deepEqual(residual.summary.materialised_terminal_pages,[1,...Array.from({length:51},(_,index)=>index+5)]);
  ...
  assert.equal(residual.summary.protected_authored_unresolved_source_unit_ids.length,15);
  assert.equal(residual.summary.protected_authored_role_binding_required_ids.length,4);
