...
function header() {
  return `<tr><td class="pad" style="background-color:#0a1730; padding:26px 32px;">
    ...
    <div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Arial,Helvetica,sans-serif; font-size:14px; line-height:20px; color:#c8d2e0; padding-top:5px;">Herausgegeben vom Institut f&uuml;r Wirkungs&ouml;konomie &middot; Verantwortlich: Natalie Weber</div>
...
function footer(links: TemplateLinks) {
  ...
  return `<tr><td class="pad" style="background-color:#0a1730; padding:24px 32px;">
    <p style="margin:0 0 10px 0; font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Arial,Helvetica,sans-serif; font-size:15px; line-height:23px; color:#e8e2d5;"><strong style="color:#fffdf8;">Wirkungsportal Parlament</strong><br>Herausgegeben vom Institut f&uuml;r Wirkungs&ouml;konomie<br>Inhaltlich verantwortlich: Natalie Weber</p>
...
function textFooter(links: TemplateLinks) {
  return [
    ...
    "Inhaltlich verantwortlich: Natalie Weber",
    "",
...
const digestSectionLabels: Record<DailyDigestItem["section"], string> = {
  WIRKUNGSANALYSE: "Neue oder aktualisierte Wirkungsanalyse",
  REALITY_CHECK: "Neuer Reality Check",
  ABSTIMMUNG: "Amtliche Abstimmung",
  LEBENSZYKLUS: "Neuer politischer Verfahrensstand",
  KORREKTUR: "Korrektur",
};

export function dailyDigestEmail(input: TemplateLinks & { dateLabel: string; items: DailyDigestItem[] }): EmailTemplate {
  const count = input.items.length;
  const subject = count === 1 ? `Wirkungsportal: 1 Neuerung am ${input.dateLabel}` : `Wirkungsportal: ${count} Neuerungen am ${input.dateLabel}`;
  const textItems = input.items.flatMap((item, index) => [
    `${index + 1}. ${digestSectionLabels[item.section]}`,
    item.title,
    item.summary,
    item.url,
    "",
  ...
  const text = [
    ...
    "Herausgegeben vom Institut für Wirkungsökonomie",
    "",
    `DIE NEUERUNGEN VOM ${input.dateLabel.toUpperCase()}`,
    "",
    "Diese Übersicht enthält ausschließlich Änderungen, die fachlich freigegeben, technisch geprüft und anschließend auf dem öffentlichen Portal verifiziert wurden.",
    "",
    ...textItems,
    "WICHTIG ZUR EINORDNUNG",
    "Wirkungspotenzial ist keine bereits eingetretene Wirkung. Beobachtung ist keine automatische Kausalität. Offene Daten bleiben offen.",
    "",
    "Wir messen weder Öffnungen noch Klicks.",
    "",
    textFooter(input),
    `Hinweise abbestellen: ${input.unsubscribeUrl}`,
  ].join("\n");
  const htmlItems = input.items.map((item) => `<tr><td class="pad" style="padding:0 32px 18px 32px;">
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #ded8ca;background:#fffdf8;">
      <tr><td style="padding:18px 20px 5px 20px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Arial,Helvetica,sans-serif;font-size:13px;line-height:20px;font-weight:700;letter-spacing:.04em;text-transform:uppercase;color:#6b5730;">${escapeHtml(digestSectionLabels[item.section])}</td></tr>
      <tr><td style="padding:0 20px 8px 20px;font-family:Georgia,'Times New Roman',serif;font-size:22px;line-height:30px;color:#0a1730;">${escapeHtml(item.title)}</td></tr>
      <tr><td style="padding:0 20px 14px 20px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Arial,Helvetica,sans-serif;font-size:16px;line-height:25px;color:#343a45;">${escapeHtml(item.summary)}</td></tr>
      <tr><td style="padding:0 20px 20px 20px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Arial,Helvetica,sans-serif;font-size:16px;line-height:24px;"><a href="${escapeHtml(item.url)}" style="color:#167568;text-decoration:underline;font-weight:700;">Einordnung und Quellen ansehen</a></td></tr>
    </table>
  </td></tr>`).join("");
  const body = `${header()}
    <tr><td class="pad" style="padding:34px 32px 12px 32px;"><h1 class="title" style="margin:0 0 16px 0;font-family:Georgia,'Times New Roman',serif;font-size:29px;line-height:38px;color:#0a1730;font-weight:normal;">Die Neuerungen vom ${escapeHtml(input.dateLabel)}</h1><p style="margin:0;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Arial,Helvetica,sans-serif;font-size:17px;line-height:27px;color:#20201c;">Diese Übersicht enthält ausschließlich Änderungen, die fachlich freigegeben, technisch geprüft und anschließend auf dem öffentlichen Portal verifiziert wurden.</p></td></tr>
    <tr><td class="pad" style="padding:0 32px 24px 32px;"><div style="padding:14px 18px;border-left:3px solid #a57a2e;background:#f6f1e8;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Arial,Helvetica,sans-serif;font-size:15px;line-height:24px;color:#3d3d38;"><strong style="color:#0a1730;">${count} ${count === 1 ? "Änderung" : "Änderungen"}</strong> seit der letzten versendeten Tagesübersicht.</div></td></tr>
    ${htmlItems}
    <tr><td class="pad" style="padding:4px 32px 28px 32px;"><div style="padding-top:18px;border-top:1px solid #ded8ca;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Arial,Helvetica,sans-serif;font-size:15px;line-height:24px;color:#4a4a44;"><strong style="color:#0a1730;">Wichtig zur Einordnung:</strong> Wirkungspotenzial ist keine bereits eingetretene Wirkung. Beobachtung ist keine automatische Kausalität. Offene Daten bleiben offen.<br><br>Wir messen weder Öffnungen noch Klicks.</div></td></tr>
    ${footer(input)}`;
  return {
    subject,
    text,
    html: htmlDocument(subject, `${count} fachlich freigegebene ${count === 1 ? "Neuerung" : "Neuerungen"} im Wirkungsportal.`, body),
  };
}
