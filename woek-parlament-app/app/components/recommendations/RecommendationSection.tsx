...
export function RecommendationSection({ impactCaseId }: { impactCaseId: string }) {
  ...
  return <section className="government-recommendation" aria-labelledby={`recommendation-${impactCaseId}`} data-woek-recommendation-layer="published">
    ...
    {recommendation ? <>
      ...
      {recommendation.analysis_mode === "RETROSPECTIVE_DECISION_REVIEW" && <div className="notice"><strong>Schutz vor Rückschaufehlern</strong><p>Entscheidungsdatum: {recommendation.decision_date} · Wissensgrenze: {recommendation.knowledge_cutoff_date}</p><p>{hindsightLimitations ?? "Die fachlich freigegebene Abgrenzung des damaligen Wissensstands ist noch nicht öffentlich lesbar aufbereitet."}</p><h4>Damals verfügbar</h4><ul>{recommendation.evidence_available_at_decision_time?.map((item) => <li key={item}>{item}</li>)}</ul><h4>Erst später verfügbar</h4><ul>{recommendation.evidence_only_available_later?.map((item) => <li key={item}>{item}</li>)}</ul></div>}
