...
/** P4: exact former home passages relocated, never rewritten or discarded. */
function OriginalHeroExplanation({ parliamentMode = false }: { parliamentMode?: boolean }) {
...
return (<section className="shell hero-shell">
        <div className="hero">
          <div className="hero-inner">
            ...
            <aside className="hero-education" aria-labelledby={parliamentMode ? "hero-education-parliament" : "hero-education-title"}>
              <p className="eyebrow">{parliamentMode ? "Für die parlamentarische Vorbereitung" : "Was ist Wirkungsökonomie?"}</p>
              <h2 id={parliamentMode ? "hero-education-parliament" : "hero-education-title"}>{parliamentMode ? "Wirkungspotenzial, Risiken und veränderbare Stellschrauben." : "Die WÖk ergänzt vorhandene Prüfung – und führt sie weiter."}</h2>
...
export function HomeMethodology() { return <><OriginalHeroExplanation /><details className="shell relocated-parliament-mode"><summary>Erklärung für die parlamentarische Vorbereitung</summary><OriginalHeroExplanation parliamentMode /></details><section className="shell quick-orientation" aria-labelledby="orientation-title">
...
export function HomeTrust() { return (<section className="shell section trust-section" aria-labelledby="trust-title">
...
export function HomeStatesContext() { return <details className="shell relocated-context"><summary>Startpunkt des Länderportals – bisheriger Einführungstext</summary><section className="shell section section-compact federal-to-states" aria-labelledby="states-teaser-title">
        <div>
...
export function HomeMonitorContext() {
...
return <><section className="shell section section-compact mandate-teaser" aria-labelledby="mandate-teaser-title">
...
export function HomeGovernmentContext() {
const governmentActions = process.env.GOVERNMENT_STAGING === "1" ? getGovernmentPublicData().actions.slice(0,3) : [];
return <>{governmentActions.length > 0 ? (
...
export function HomeRadarScope() { return <section className="shell section"><h2>Geltungsbereich des Parlamentsradars</h2><div className="notice notice-neutral"><strong>Der Parlamentsradar zeigt nur prüfrelevante Vorgänge mit belegtem Verfahrensstand.</strong> Er ist kein vollständiger parlamentarischer Kalender. Aufgenommen werden Entscheidungen, deren Reichweite, Dauer, finanzielle Größenordnung, mögliche Irreversibilität oder systemische Folgen eine frühe Prüfung besonders sinnvoll machen.</div></section>; }
