  ...
  return <main className="shell content-page">
    ...
    <section className="section section-compact" aria-labelledby="state-observations">
      ...
      <div className="source-register">{observations.map((observation) => {
        ...
        return <article key={observation.observation_id}>
          <p className="source-register-label">{humanizeSystemValue(observation.revision_status)} · Datenqualität {humanizeSystemValue(observation.data_quality)}</p>
          ...
          <p><strong>Serie:</strong> {outcome ? `${outcome.outcome_series_id} · ${humanizeSystemValue(outcome.status)}` : "keine Serie verknüpft"}</p>
          <p><strong>Außergewöhnliches externes Ereignis:</strong> {shock ? `${shock.title} · ${humanizeSystemValue(shock.attribution_status)}` : "kein außergewöhnliches Ereignis verknüpft"}</p>
    ...
    <section className="section section-compact" aria-labelledby="evidence-events">
      ...
      {events.length ? <div className="source-register">{events.map((event) => <article id={event.evidence_event_id} key={event.evidence_event_id}>
        <p className="source-register-label">Beobachtung {new Date(event.observation_date).toLocaleString("de-DE")} · {humanizeSystemValue(event.materiality)}</p>
        <h3>{event.title}</h3>
        <p>{event.concise_public_summary}</p>
        <p><strong>Was sich für die Prüfung ändert:</strong> {event.what_changed_or_may_change}</p>
        <p><strong>Zurechnung:</strong> {humanizeSystemValue(event.attribution_status)} · <strong>Datenqualität:</strong> {humanizeSystemValue(qualityLabel(event.data_quality))}</p>
    ...
    <section className="section section-compact" aria-labelledby="reality-candidates">
      ...
      {candidates.length ? <div className="source-register">{candidates.map((candidate) => <article key={candidate.reality_candidate_id}>
        <p className="source-register-label">{humanizeSystemValue(candidate.priority)} · {humanizeSystemValue(candidate.attribution_status)}</p>
