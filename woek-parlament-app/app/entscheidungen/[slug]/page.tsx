...
import { DecisionPageNavigation, type DecisionNavigationItem } from "@/app/components/DecisionPageNavigation";
...
export default async function DecisionPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const reviewDetail = item.publicWorkingAct?.reviewDetail;
  const mappingCount = normativeMapping ? (normativeMapping.sdgItems?.length ?? 0) + (normativeMapping.sdgPlusItems?.length ?? 0) + (normativeMapping.constitutionalAnchorItems?.length ?? 0) : 0;
  const navigationItems: DecisionNavigationItem[] = [
    { id: "case-overview", label: "Worum geht es?" },
    ...(normativeMapping ? [{ id: "normative-impact-tiles-title", label: "Ziele & Schutzgüter", count: mappingCount }] : []),
    ...(item.publicAssessment ? [{ id: "assessment-title", label: "WÖk-Einordnung" }] : item.publicWorkingAct ? [{ id: "working-act-title", label: "Wirkungspotenzial" }] : []),
    ...(item.publicWorkingAct?.voteLayer ? [{ id: "public-vote-title", label: "Abstimmung" }] : []),
    ...(reviewDetail ? [{ id: "review-deep-dive-title", label: "Vollständige Wirkungslogik", count: reviewDetail.impactPaths.length }] : []),
    ...(reviewDetail?.calculations.length ? [{ id: "calculation-title", label: "Berechnungen", count: reviewDetail.calculations.length }] : []),
    ...(reviewDetail && (reviewDetail.risks.length || reviewDetail.boundaries.length) ? [{ id: "review-protection", label: "Risiken & Grenzen", count: reviewDetail.risks.length + reviewDetail.boundaries.length }] : []),
    ...(item.publicWorkingAct?.dataGaps.length ? [{ id: "working-act-open-items", label: "Fehlende Daten", count: item.publicWorkingAct.dataGaps.length }] : []),
    ...(reviewDetail?.counterarguments.length ? [{ id: "review-counterarguments", label: "Gegenprüfung", count: reviewDetail.counterarguments.length }] : []),
    ...(reviewDetail?.feedback ? [{ id: "feedback-title", label: "Rückkopplung" }] : []),
    { id: "case-version", label: "Geprüfte Fassung" },
    { id: "source-basis", label: "Quellen", count: item.sources.length },
    ...(item.fachakteId ? [{ id: "case-full-dossier", label: "Vollständige Fachakte" }] : [])
  ];
  return (
    <div className="shell decision-page" id="case-top">
      ...
      <div className="decision-reading-layout">
      <DecisionPageNavigation items={navigationItems} />
      <div className="decision-page-flow">

      <section className="sixty-second" id="case-overview" aria-labelledby="sixty-second-title"><div><p className="eyebrow">60 Sekunden</p><h2 id="sixty-second-title">Worum geht es?</h2></div><dl><div><dt>Was wird entschieden?</dt><dd>{humanizeSystemValue(item.whatIsDecided)}</dd></div><div><dt>Stand der WÖk-Analyse</dt><dd>{humanizeSystemValue(item.analysisStatus)}</dd></div><div><dt>Was soll erreicht werden?</dt><dd>{item.intendedGoal}</dd></div></dl></section>
      ...
      {"fachakteId" in item && typeof item.fachakteId === "string" && <section className="decision-section full-dossier-link" id="case-full-dossier"><p className="eyebrow">Vollständige Fachakte</p><h2>Die vollständige Analyse einsehen</h2><p>Die ausführliche Fachakte enthält den vollständigen quellengebundenen Prüfbestand für diesen Fall. Die gegliederte Seite oben ist die Leseansicht; die Fachakte bewahrt die vollständige Herleitung.</p><Link className="button button-secondary" href={`/fachakten/${item.fachakteId}`}>Vollständige Fachakte öffnen</Link></section>}
