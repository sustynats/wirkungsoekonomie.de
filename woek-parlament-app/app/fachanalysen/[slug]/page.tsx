...
export default async function FachanalyseDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <div className="shell content-page fachanalyse-detail">
      ...
      {analysis.fullPublicationSource ? <section className="decision-section full-dossier-link" aria-labelledby="full-source-title"><p className="eyebrow">Vollständige Fachakte</p><h2 id="full-source-title">Alle Befunde und Quellenbezüge nachlesen</h2><p>{analysis.fullPublicationSource.description}</p><a className="button button-secondary" href={analysis.fullPublicationSource.href}>{analysis.fullPublicationSource.label}</a></section> : null}
