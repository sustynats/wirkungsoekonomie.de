...
export const metadata = { title: "Wirkungsakten" };

/** P1 retains each complete collection. P3 adds common object facets. */
export default async function RegisterPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <>
    <div className="shell"><p className="eyebrow">Wirkungsakten</p><nav className="portal-register-views" aria-label="Bestandsansichten">{registerViews.map((view) => <SamePageStateLink key={view.key} href={{ pathname: "/wirkungsakten", query: { ...query, bestand: view.key } }} aria-current={selected === view.key ? "page" : undefined}>{view.label}</SamePageStateLink>)}</nav></div>
