...
### 1. Current legal/current-policy baseline guards

**Federal/Länder financing is not a blanket `Bund orders, Bund always pays` rule.** Art. 104a GG states the default that Bund and Länder separately bear expenditure arising from their own tasks, with specified exceptions; federal laws creating certain third-party benefit obligations and costs borne by Länder require Bundesrat consent. Art. 84(1) GG also bars federal law from directly assigning tasks to municipalities/municipal associations. Therefore any full-reimbursement proposal must be assessed as a prospective federal constitutional/statutory/fiscal rule change, not as a description of the current universal legal baseline. Official GG: `https://www.gesetze-im-internet.de/gg/art_104a.html`, `https://www.gesetze-im-internet.de/gg/art_84.html`.

Berlin's tax-enforcement baseline has already moved in 2026: on **24.02.2026** the Senate stated that the Finanzamt für Fahndung und Strafsachen Berlin should be strengthened **personally and structurally** after evaluation. This means the P53 tax-office-strengthening proposal is not a zero-to-one policy invention; any WÖk attribution is incremental/additionality-based. `https://www.berlin.de/rbmskzl/aktuelles/pressemitteilungen/2026/pressemitteilung.1646345.php`.
...
- `evidence_level = MEDIUM_FOR_ASSIGNMENT_TO_BUDGET_RISK_MECHANISM / NET_FEDERAL_SYSTEM_OUTCOME_PENDING`
- Competence: `FEDERAL_CONSTITUTIONAL_OR_STATUTORY_FINANCING_RULE / BERLIN_BUNDESRAT_AND_INTERGOVERNMENTAL_ADVOCACY_ROLE`; Berlin cannot enact a universal federal reimbursement rule alone.
- `A→M→ΔZ`: clearer ex-ante rule tying new federal obligations to defined cost coverage -> less unfunded/underfunded vertical fiscal risk and more predictable Länder/local budgeting -> potentially fewer forced offsetting cuts or emergency reallocations.
- Material omissions: which task classes, `full cost` definition, standard/actual cost, dynamic indexation, administrative vs transfer expenditure, co-financing, audit/settlement, legacy tasks and municipal pass-through.
- Risks/trade-offs: moral hazard/overstated standard costs, reduced cost discipline, negotiation delay, rigid cost assignment, federal incentive to avoid useful legislation or shift instrument form.
...
- Current-law guard: Art. 104a(4) already creates Bundesrat-consent requirements for defined federal laws imposing certain third-party benefit obligations/costs on Länder. The proposal implies **broader/stronger** rights beyond current scope; do not render current law as having no Länder voice.
...
- Competence: federal constitutional/statutory/intergovernmental design; Berlin advocacy/Bundesrat role.
- Mechanism: mandatory financing settlement before enactment can align mandate and resources and improve Länder/municipal budgetability.
- Omissions: trigger threshold, covered cost types, dynamic costs, dispute method, emergency exceptions, treatment of mixed-benefit policies and sanctions if forecasts miss.
...
1. `verbesserte bundeseinheitliche Abstimmung im Bereich der Digitalisierung`
   - `terminal_fach_state = EXPLICIT_FACH_APPROVED`
   - `impact_direction = POSITIVE_TAX_ADMIN_INTEROPERABILITY_AND_PROCESS_EFFICIENCY_POTENTIAL / FEDERAL_COORDINATION_AND_DELIVERY_DEPENDENT`
   - `evidence_level = MEDIUM_FOR_STANDARDISATION_INTEROPERABILITY_MECHANISM / SAVINGS_AND_REVENUE_OUTCOME_PENDING`
   - Current-baseline guard: KONSENS/digital tax administration already exists; assess only incremental standardisation/interoperability, not a zero-to-digital claim.
   - Risks: migration cost, vendor/architecture lock-in, cybersecurity/data protection, process standardisation that codifies poor workflows.
   - Reality check: processing time/error rates, duplicate systems/manual handoffs, service quality, staff effort, IT lifecycle cost and compliance/revenue effects.

2. `Priorisierung verschiedener Fachbereiche`
   - `terminal_fach_state = REVIEWED_NOT_ASSESSABLE_WITH_EXACT_REASON`
   - Exact reason: no `Fachbereiche`, prioritisation criterion, resource shift, counterfactual or protected lower-priority function is identified; cannot determine whether resources move toward higher-risk work or away from essential control/service.

3. `einen effizienteren Steuervollzug`
   - `terminal_fach_state = EXPLICIT_FACH_APPROVED`
   - `impact_direction = POSITIVE_TAX_COMPLIANCE_AND_REVENUE_PROTECTION_POTENTIAL / TARGETING_LEGALITY_AND_CAPACITY_DEPENDENT`
   - `evidence_level = MEDIUM_HIGH_FOR_ENFORCEMENT_TO_COMPLIANCE_MECHANISM / MARGINAL_REVENUE_PENDING`
   - Competence: mixed federal tax law/administration cooperation and Berlin tax administration.
   - Rights guard: legality, equal treatment, proportionality, taxpayer procedural rights and data protection cannot be traded for gross assessment yield.
   - Reality check: risk-adjusted audit/fraud yield net of staff/IT cost, compliance gap, processing time, litigation/error rate and distribution across taxpayer types.
...
- Competence: `LAND_BERLIN_FUNDING_PROGRAMME_DESIGN_WITH_EU_STATE_AID_PROCUREMENT_AND_PROGRAMME_SPECIFIC_CONSTRAINTS`.
- Mechanism: eligibility/scoring/conditions can shift public support toward firms/projects meeting defined labour/regional-benefit standards and create compliance incentives.
- Omissions: programmes/amounts, legal form, exact `fair wages`/regional value criteria, verification, grandfathering and sanction/recovery rules.
- Risks: SME/start-up exclusion, administrative burden, gaming/formal compliance, reduced applicant competition, relocation/substitution and weak additionality.
- Distribution: workers/wages, SMEs/large firms, sectors, districts/regional supply chains, taxpayers.
- Reality check: recipient mix, compliance/additionality, wage/tariff outcomes, jobs, regional procurement/value, programme take-up, admin cost and displaced otherwise-beneficial projects.

**`BE-BSW-P53-U07-A03-349b35196172`** is compound and becomes versioned/non-counting with two deterministic semantic children:

1. `ineffektive Subventionen gestrichen`
   - `terminal_fach_state = REVIEWED_NOT_ASSESSABLE_WITH_EXACT_REASON`
   - Exact reason: no subsidy/programme, fiscal amount, `ineffective` outcome criterion, beneficiary/incidence, transition or spillover is named. Same guard as P52: no generic `subsidy cut = positive` inference.

2. `die freiwerdenden Mittel zum Beispiel in arbeitsmarktpolitische Programme für Geringqualifizierte investiert werden`
   - `terminal_fach_state = EXPLICIT_FACH_APPROVED`
   - `impact_direction = POSITIVE_SKILLS_EMPLOYMENT_AND_PARTICIPATION_POTENTIAL / PROGRAMME_QUALITY_LABOUR_DEMAND_AND_ADDITIONALITY_DEPENDENT`
   - `evidence_level = MEDIUM_LOW_FOR_ACTIVE_LABOUR_POLICY_MECHANISM / DURABLE_EMPLOYMENT_INCOME_OUTCOME_PENDING`
   - Competence: mixed Berlin labour/economic/social funding with federal employment-system interfaces.
...
- Current-policy/version guard: Senate already decided on 24.02.2026 to strengthen FA FuSt personally/structurally after evaluation. The election object is therefore partly `CONTINUE_SCALE_OR_BROADEN_EXISTING_2026_PATH`, not a zero-to-one invention; exact additionality must be versioned.
- Mechanism: additional qualified audit/investigation/service capacity, if targeted to risk/bottlenecks, can improve case throughput, compliance and revenue protection.
- Risks: staffing without process/data improvement; displacement from taxpayer service; excessive enforcement burden; recruitment/training bottlenecks.
...
`Eine Umschichtung zugunsten dieser Bereiche verbessert die Lebensqualität breiter Bevölkerungsschichten unmittelbar.`

- repaired child `terminal_fach_state = NON_EFFECT_EXPECTED_DISTRIBUTIONAL_OUTCOME_CLAIM_REVIEWED`
- Exact reason: claimed outcome of U10-A03/A04 reallocation/priority choices, not an independent intervention and not evidence that life quality improves immediately.
- Preserve both source IDs/SHA/cross-page lineage; P54-U01 is consumed only for this repair and **does not close P54**.

### 10. Page terminality

P53 physical source scope is terminal once the two authorised compound repairs and the single P53→P54 cross-page repair are materialised exactly as above. The page has 10 source units and 21 stored atoms, but stored punctuation atoms are not the authoritative effect count where superseded by the explicitly authorised repairs.

`BE_BSW_P53_FACH_COMPLETE = PASS_SOURCE_BOUND_AFTER_COMPOUND_AND_CROSS_PAGE_REPAIR`

After P53 Fach closure, the **next open physical page remains P54**; its U01 is already consumed only as the cross-page P53 repair component, while all other P54 source units/atoms stay fail-closed until a dedicated P54 Fach batch. BSW remains programme-open; Berlin remains `3/12 programme-terminal / 9/12 programme-open`; MV and combined Parliament Golden State remain fail-closed.

### 11. CodeX handoff

Consume P53 only after idempotent source-order reconciliation/materialisation of all authoritative prior BSW batches not yet on exact main. Generate only the explicitly authorised U06-A04 3-child repair, U07-A03 2-child repair, and P53-U10-A05/P54-U01 cross-page repaired child; stable IDs mechanically from exact clause/full-sentence text/hash. Preserve original parents as versioned/non-counting with exact source/SHA/supersession/cross-page lineage. No additional technical Fach/DNS/Recommendation/score synthesis.

Recompute Berlin/#241 residual SET-WISE; P54 page envelope must remain open after consuming only U01 as cross-page repair. Run BSW/Berlin residual, parent-child/cross-page integrity, #241, Golden-readiness, Source-vs-View, SamePage, tests/typecheck/lint/local production build/privacy/link/accessibility/responsive gates. GitHub-only; no Vercel Preview/build/deploy/promotion and no owner-RC request.
