...
- Berlin administration is **already** in a major reform/digitalisation path. The new Landesorganisationsgesetz entered into force 01.01.2026; the Zuständigkeitsverordnung/catalog became effective in July 2026 and the public responsibility database went online 07.08.2026. `https://www.berlin.de/rbmskzl/aktuelles/pressemitteilungen/2025/pressemitteilung.1604357.php`, `https://www.berlin.de/rbmskzl/aktuelles/pressemitteilungen/2026/pressemitteilung.1700630.php`.
- Digitalisation is not a zero baseline: Berlin has active digital services/NOOTS pilots, a binding Digitalcheck since 11.08.2026 and an active CDO/administrative-modernisation programme. `https://www.berlin.de/rbmskzl/aktuelles/pressemitteilungen/2026/pressemitteilung.1701600.php`, `https://www.berlin.de/sen/web/presse/pressemitteilungen/2026/pressemitteilung.1694690.php`.
- Personal councils already have statutory participation/co-determination rights under Berlin PersVG, including in hiring processes; stronger rights would be a **delta to an existing framework**, not creation from zero. `https://www.berlin.de/gpr/oertliche-personalraete/spandau/pr-arbeit/artikel.1241936.php`.
- For Beamten appointments, health suitability is already an individual legal criterion; Berlin's current §8 LBG guidance requires an individual prognosis. A past psychotherapy episode must therefore not be rendered as if current law itself automatically creates a blanket exclusion. `https://www.berlin.de/sen/finanzen/ueber-uns/karriere/ausbildung-duales-studium/merkblatt-8-lbg-aerztliche-untersuchung-endfassung.pdf`.
- Promotion for Beamte remains bound by the performance principle (Eignung, Befähigung, fachliche Leistung). `https://gesetze.berlin.de/bsbe/document/jlr-LbGBE2011V15P31/format/xsl/part/X`.

## A. Structural / diagnosis / goal records

Keep exact source IDs/text/hashes. Terminal non-counting:
- `BE-BSW-P62-U01-b026f0a67b43` → `NON_EFFECT_CONTEXT_AND_HISTORICAL_CAUSAL_CLAIM_REVIEWED`; historical attribution of today's administrative condition is a programme claim, not accepted as causal proof.
- `BE-BSW-P62-U03-e3ebf75c103f`, `U05-d9f9e2e9aa18`, `U07-092ddff2b39a`, `U09-95f24170b956` → `NON_EFFECT_CONTEXT_REVIEWED / STRUCTURAL_HEADING`.

U02:
- `A01-998b9a76a0d6` coordination Senat/Bezirke → `NON_EFFECT_COORDINATION_GOAL_REVIEWED`; no distinct governance instrument beyond coordination.
...
- `A03-6e7c271987d7` stronger statutory staff-council co-determination rights → `EXPLICIT_FACH_APPROVED`.
  - `impact_direction = AMBIVALENT_EMPLOYEE_VOICE_AND_PROCEDURAL_PROTECTION_POTENTIAL / SCOPE_AND_DECISION_SPEED_DEPENDENT`
  - `evidence_level = HIGH_FOR_FORMAL_DECISION_RIGHTS_TO_EMPLOYEE_VOICE_MECHANISM / ORGANISATIONAL_AND_SERVICE_OUTCOME_DESIGN_DEPENDENT`
  - Potential: stronger rights can improve worker voice, information quality, acceptance and protection in organisational/personnel changes.
  - Risks: poorly scoped veto/approval layers can increase transaction time or conflict; stronger participation cannot substitute for management quality or staffing.
  - Reality check: case types covered, agreement/dispute time, reversals/grievances, worker trust/retention and service-time effects.
...
- `A02-508b4843a227` replace paper files with digitally managed files → `EXPLICIT_FACH_APPROVED`.
  - `impact_direction = POSITIVE_PROCESS_TRACEABILITY_AND_DIGITAL_WORKFLOW_POTENTIAL / MIGRATION_SECURITY_INTEROPERABILITY_AND_PROCESS_REDESIGN_DEPENDENT`
  - `evidence_level = HIGH_FOR_DIGITAL_WORKFLOW_MECHANISM / NET_TIME_AND_QUALITY_OUTCOME_DEPENDENT`
  - Current baseline: Berlin is already digitalising; treat as acceleration/completion of a current pathway, not `paper → digital from zero`.
  - Risks: digitising bad processes, migration/data-quality failure, access/cybersecurity, vendor lock-in, staff burden and exclusion of non-digital users.
  - Reality check: end-to-end processing time, handoffs/re-entry, error/rework, availability, security incidents, accessibility and user/staff experience; digital-file count is output.

### Cross-page repair
Old `BE-BSW-P62-U10-A03-0cca4a571f8d` (`Auch die Zeit des Faxgeräts in den Behörden soll`) and P63 `BE-BSW-P63-U01-A01-2cc3c71a4c3f` (`ablaufen.`) are one sentence across the physical page break:

`Auch die Zeit des Faxgeräts in den Behörden soll ablaufen.`

Create one deterministic cross-page source-bound replacement from that exact full sentence; both old fragments become versioned/non-counting parents/parts. The repaired full-sentence object is `REVIEWED_NOT_ASSESSABLE_WITH_EXACT_REASON`: eliminating fax names a channel to retire but not its secure/interoperable replacement, affected procedures, legal form requirements, transition/fallback or accessibility. Fax retirement itself is output, not service quality.

Do **not** consume P63 U01 A02/A03 in this batch; they remain for the P63 Fach review.

## F. P62 terminal result

After the U02-A03 deterministic 3-child split and P62→P63 fax-fragment repair above:
...
Re-read exact current main/newest #240/#241 before write. Consume all queued P38→P62 in source order/idempotently. Generate only the exact children/repairs authorised by each handoff, including P62 U02-A03 and the P62→P63 fax sentence. Preserve all original IDs/text/hashes and lineage. Recompute Berlin/BSW residual SET-WISE; run established residual/BSW truth/#241/Golden-readiness/Source-vs-View/SamePage/tests/typecheck/lint/local-build/privacy/link/accessibility/responsive gates. **GitHub only; `NO_NEW_VERCEL_BUILD=true`; no Vercel action or owner RC.**
