...
- Berlin already operates digital/connected traffic-signal infrastructure: on 10.07.2026 the first C-ITS-based signal systems for public-transport priority entered operation; Berlin explicitly keeps traffic control as a sovereign city task. `https://www.berlin.de/sen/uvk/presse/pressemitteilungen/2026/pressemitteilung.1691990.php`, `https://www.berlin.de/sen/uvk/mobilitaet-und-verkehr/verkehrsmanagement/ampeln-und-co/generaluebernehmer/`.
- Berlin already has an integrated digital construction-coordination path. The Senate's 21.04.2026 concept includes a digital coordination platform and the City Challenge project `Smarte Baustelleninformationen für weniger Stau`, using AI to make traffic-sign plans machine-readable. `https://www.berlin.de/rbmskzl/aktuelles/pressemitteilungen/2026/pressemitteilung.1662846.php`; current VIZ expansion followed on 14.08.2026: `https://www.berlin.de/sen/uvk/presse/pressemitteilungen/2026/pressemitteilung.1703029.php`.
...
- Mechanism: changing classification constraints can raise attainable compensation for scarce IT profiles and potentially improve hiring/retention for digital-delivery capacity.
- Omissions: exact criteria/pay bands/roles, tariff/beamtenrecht pathway, internal equity, budget, market benchmark and whether pay is the binding vacancy cause.
- Risks/trade-offs: wage compression/reclassification disputes, inequity with comparable occupations, fiscal cost, hiring without retention/management improvement, salary competition among public employers.
- Reality check: qualified applicants, vacancy duration, accepted offers, retention/turnover, pay distribution/equity and actual digital-service capacity; salary level alone is input.
...
- `terminal_fach_state = EXPLICIT_FACH_APPROVED`
- `impact_direction = AMBIVALENT_ADAPTIVE_TRAFFIC_CONTROL_PILOT_POTENTIAL / MULTIMODAL_SAFETY_EMISSIONS_AND_OBJECTIVE_FUNCTION_DEPENDENT`
- `evidence_level = HIGH_FOR_SIGNAL_TIMING_TO_TRAFFIC_DELAY_MECHANISM / CITYWIDE_NET_OUTCOME_PILOT_DEPENDENT`
- Current-path guard: Berlin is already digitalising signals/C-ITS; additionality is the adaptive/AI pilot and its objective function, not digital signal control from zero.
- Mechanism: real-time/forecast traffic information can alter phase/timing logic, potentially reducing selected delay/queues.
- Hard noncompensation: vehicle-flow gains cannot compensate for worse pedestrian/cyclist crossing time, public-transport reliability, safety, accessibility or induced traffic/emissions.
...
- `terminal_fach_state = EXPLICIT_FACH_APPROVED`
- `impact_direction = POSITIVE_CONSTRUCTION_COORDINATION_AND_TRAFFIC_DISRUPTION_REDUCTION_POTENTIAL / CURRENT_PATH_ADDITIONALITY_DATA_QUALITY_AND_DELIVERY_DEPENDENT`
- `evidence_level = HIGH_FOR_SHARED_DATA_COORDINATION_TO_CONFLICT_REDUCTION_MECHANISM / NET_TRAVEL_AND_PROJECT_OUTCOME_PENDING`
- Current-path guard: Berlin's April/August 2026 construction-coordination programme already includes digital platform/data/AI elements. Treat this as continuation/acceleration unless a future implementation delta is proven.
...
- `terminal_fach_state = EXPLICIT_FACH_APPROVED`
- `impact_direction = POSITIVE_SERVICE_MATCHING_AND_FLEET_UTILISATION_POTENTIAL / EQUITY_RELIABILITY_LABOUR_EMISSIONS_AND_DATA_DEPENDENT`
- `evidence_level = MEDIUM_HIGH_FOR_DEMAND_FORECASTING_TO_DISPATCH_MECHANISM / RIDERSHIP_AND_SYSTEM_OUTCOME_PILOT_DEPENDENT`
- Mechanism: demand forecasting/dynamic deployment can better align buses with observed demand, potentially lowering overload/empty capacity and improving wait/reliability.
- Risks: disadvantaging low-demand but essential routes/times, algorithmic feedback loops, instability/uncertainty for riders/drivers, labour scheduling, privacy, fleet repositioning emissions and exclusion of non-digital demand signals.
...
- `terminal_fach_state = EXPLICIT_FACH_APPROVED`
- `impact_direction = AMBIVALENT_LEGAL_CLARITY_FOR_LEGITIMATE_SECURITY_RESEARCH_POTENTIAL / OFFENCE_BOUNDARY_AND_ABUSE_SAFEGUARDS_DEPENDENT`
- `evidence_level = HIGH_FOR_CRIMINAL_LAW_BOUNDARY_TO_LEGAL_RISK_INCENTIVE_MECHANISM / CYBERSECURITY_NET_EFFECT_DESIGN_DEPENDENT`
- Competence: `FEDERAL_CRIMINAL_LAW / BERLIN_BUNDESRAT_INITIATIVE_AND_ADVOCACY_PATH`; do not render as direct Berlin amendment.
- Mechanism: a clearer legitimate-research carve-out/intent boundary can reduce legal uncertainty/chilling effects for responsible vulnerability research and disclosure.
- Risks/trade-offs: overbroad exemption can weaken enforcement against malicious tooling/preparation; underbroad reform leaves research uncertainty. `nützliche Hinweisgeber` must not be conflated with all tool possession/use.
...
## I. Page / full-programme Fach result

P66 requires no deterministic child split. Exact terminal effect-role summary:
- `5 EXPLICIT_FACH_APPROVED`: U01, U03, U04, U05, U06
- `2 REVIEWED_NOT_ASSESSABLE_WITH_EXACT_REASON`: U07, U08-A01
- `1 NON_EFFECT_*`: U08-A02
- U02/U09 non-effect context.

`BE_BSW_P66_FACH_COMPLETE = PASS_SOURCE_BOUND_OBJECT_LEVEL`

With the already-authoritative #240 chain P1→P65 plus this P66 batch, the **human/WÖk Fach handoff now covers the complete frozen BSW 66-page source artefact**. This is not yet a claim that current `main` has materialised it.

`BE_BSW_FULL_PROGRAMME_FACH_HANDOFF_COMPLETE = PASS_SOURCE_BOUND_66_OF_66`

### Required technical completion semantics

CodeX must now consume every unmaterialised authoritative BSW handoff from current technical frontier through P66, in source order, idempotently/losslessly, applying only explicit deterministic repairs/children. After a green exact-head materialisation and set-wise recomputation, BSW may become the **fourth** Berlin terminal programme:

`BE_PROGRAMME_ANALYSIS_COMPLETE = 4/12` only if and when BSW physical/page/child residual is exactly zero and all BSW source-vs-view/truth/golden gates pass.

The other Berlin eight genuine programmes remain open and must not be affected: AfD, GRÜNE, FDP, Tierschutzpartei, Volt, SPD, CDU, Die Linke. SPD fragmentation defects remain a separate finite technical/Fach-neutral segmentation issue.
