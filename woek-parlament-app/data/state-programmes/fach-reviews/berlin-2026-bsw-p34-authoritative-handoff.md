...
P34 must not be reviewed against a pre-GEAS or zero-integration baseline.

- Berlin already implemented the Land-side GEAS screening architecture before the 12.06.2026 applicability date: the Senate assigned the inland screening to a joint structure involving LAF, Berlin Police and, for unaccompanied minors, the youth authority; on 31.03.2026 it also approved additional staffing and the operational screening architecture at the central arrival centre. Official sources: `https://www.berlin.de/rbmskzl/aktuelles/pressemitteilungen/2025/pressemitteilung.1622116.php` and `https://www.berlin.de/rbmskzl/aktuelles/pressemitteilungen/2026/pressemitteilung.1658072.php`.
- The EU safe-third-country amendments and the wider Pact framework apply from 12.06.2026; the safe-third-country concept is therefore an EU/federal legal field rather than a Berlin-only lever. Council final-greenlight source: `https://www.consilium.europa.eu/en/press/press-releases/2026/02/23/council-gives-final-greenlight-to-measures-to-make-the-eu-s-asylum-system-more-efficient-and-robust/`.
- Integration-course access is currently a real delivery constraint. The Berlin Integration Commissioner reported on 13.02.2026 that federal access was narrowed to people with a statutory entitlement or an authority-imposed obligation and estimated roughly 8,000 fewer course places in Berlin. Source: `https://www.berlin.de/lb/intmig/service/presse/pressemitteilungen/2026/pressemitteilung.1643181.php`.
...
- `terminal_fach_state = NON_EFFECT_POLICY_FRAME_REVIEWED`
- Exact reason: broad normative policy frame with no independent legal, budgetary or administrative intervention in this atom. Do not count it as an effect object.

#### `BE-BSW-P34-U01-A02-013252e913d2`
Source: equal educational/career opportunities for German citizens regardless of ethnic origin/religious affiliation and strengthening cultural diversity.

- `terminal_fach_state = NON_EFFECT_EQUAL_OPPORTUNITY_AND_DIVERSITY_GOAL_REVIEWED`
- Exact reason: target/equity state, not an independently specified intervention. Preserve the source scope `deutsche Staatsbürger` exactly; do not silently generalise it to all residents and do not infer a separate exclusion effect from wording alone.

#### `BE-BSW-P34-U01-A03-4a7c7a21e81f`
This stored atom combines at least two distinct policy mechanisms: stricter regulation of immigration and an obligation for lawful asylum applicants to participate in integration measures. They differ in competence, mechanism, target group, safeguards and delivery dependencies.

Mark parent:
`SOURCE_UNIT_RECLASSIFIED_VERSIONED / NONCOUNTING_COMPOUND_PARENT_PENDING_CHILD_IDS`.

Generate deterministic source-bound children from the **exact stored text**, without paraphrasing:
1. strict/regulatory immigration-policy clause;
2. obligation-to-integration-measures clause, including its language/social-integration purpose as part of that child unless the exact syntax creates another independent intervention.
...
Generate deterministic children from the exact stored text, preserving punctuation and source order. At minimum separate:
1. the federal/German implementation demand (`notwendige Regelungen ... zügig umzusetzen`);
2. the `regelbasiert` target/system-state clause if independently tokenisable;
3. the flight-route-deaths / smuggling-crime claimed outcome clause(s) if independently tokenisable.

Do **not** force a child count if exact syntax yields a different smallest valid source-bound decomposition. Return exact IDs/text spans to #240. No child Fach synthesis by CodeX.
...
### 8. P34 exact Fach result

Original frozen P34 object set:
- **17 clean records** are now terminal reviewed/non-counting under the decisions above;
- **2 compound parents** (`U01-A03`, `U06-A01`) are versioned non-counting and await only deterministic child IDs/text spans;
- generic #313 RNAA does not survive as Fach authority for this scope.
...
On the existing exact-current-main GitHub-only materialisation lane, extend the authoritative source-order queue through **P34**. Consume P22→P33 idempotently first if not yet materialised, then this P34 batch.

For the two P34 compound parents generate only deterministic IDs/hashes/exact text spans; preserve parents as versioned zero-count records; post generated children + SET-WISE residual to #240/#241. No technical Fach, DNS, Recommendation, score or party-wide synthesis.

Run Berlin residual integrity, BSW full-programme truth, #241 residual, current Golden-readiness, Source-vs-View, SamePage navigation, tests/typecheck/lint/build, privacy/link/accessibility/responsive gates as applicable. GitHub-only. **No Vercel Preview/build/deploy/promotion and no owner RC request.**
