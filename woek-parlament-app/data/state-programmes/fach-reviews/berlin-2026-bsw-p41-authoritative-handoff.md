...
### 1. Current legal / delivery baseline

- Berlin already has current organised-crime and asset-recovery practice; the 2024 OK Lagebild reported 55 OK investigation complexes, 371 suspects and €2.9m provisionally secured assets. Official source: `https://www.berlin.de/sen/inneres/presse/pressemitteilungen/2026/pressemitteilung.1641833.php`.
- Asset recovery is therefore not a zero baseline. Berlin also launched a Bundesrat initiative in March 2026 to modify the burden-of-proof rule for extended independent confiscation. Source: `https://www.berlin.de/sen/justv/presse/pressemitteilungen/2026/pressemitteilung.1648810.php`.
- Joint investigation structures also already exist, including a police/customs joint group on labour exploitation. Source: `https://www.berlin.de/polizei/polizeimeldungen/2025/pressemitteilung.1523671.php`.
- Under the current ASOG/kbO framework, Berlin Police can already make/transfer/record video images at designated kriminalitätsbelastete Orte; seven kbO apply from 01.07.2026. Current-law source: `https://www.berlin.de/polizei/polizeimeldungen/fakten-hintergruende/artikel.1078268.php`.
...
- `BE-BSW-P41-U06-16a910033412` → `NON_EFFECT_SECURITY_AND_SUBJECTIVE_SAFETY_GOAL_REVIEWED`.
...
- `evidence_level = HIGH_FOR_ADDITIONAL_ADJUDICATION_CAPACITY_MECHANISM / NET_TIMELINESS_AND_QUALITY_OUTCOME_PENDING`
- Competence: `LAND_BERLIN_JUSTICE_BUDGET_AND_PERSONNEL_WITH_JUDICIAL_INDEPENDENCE_GUARD`.
- Mechanism: genuinely additional filled judicial/prosecutorial positions can reduce caseload pressure and improve throughput/timeliness if supporting registry, IT, experts, detention/defence and court capacity do not bind.
- Risks: authorised-but-unfilled posts, shifting legal staff from other jurisdictions, support-staff bottlenecks, recurring fiscal cost, using throughput metrics that pressure independent decision quality.
...
- `evidence_level = HIGH_FOR_PROCESS_TIME_MECHANISM / CRIME_AND_REHABILITATION_OUTCOME_DEPENDENT`
- Mechanism: prioritised/simplified scheduling and sufficient capacity can shorten offence-to-decision time, potentially improving certainty and relevance of response, especially for youth.
- Hard guards: appropriate case eligibility, defence/access to evidence, judicial independence, youth-justice educational/rehabilitative purpose, victim participation and no plea/processing pressure caused by speed targets.
...
- `evidence_level = HIGH_FOR_DIRECT_ASSET_DEPRIVATION_MECHANISM / CRIME_NETWORK_OUTCOME_PENDING`
- Baseline/additionality: asset recovery already occurs; the assessable delta is enforcement intensity/capacity/legal effectiveness, not creation of confiscation from zero.
- Mechanism: lawfully identifying/freezing/confiscating crime-linked assets can reduce available criminal capital and recover value for public budgets.
- Risks: wrongful seizure, weak beneficial-ownership tracing, asset depreciation/administration costs, displacement to harder-to-trace assets, litigation and investigative opportunity cost.
...
- `evidence_level = HIGH_FOR_BUDGET_TRANSFER_MECHANISM / SERVICE_OUTCOME_PENDING`
- Mechanism: if legally budgeted as additional revenue, recovered assets can finance police/fire capacity.
- Risks: volatile/nonforecastable revenue, substitution for ordinary appropriations, perverse institutional incentives if enforcement benefits directly from seizure volume, unclear allocation between police/fire and high-value assets distorting priorities.
...
- `evidence_level = MEDIUM_HIGH_FOR_COORDINATION_AND_INFORMATION_MECHANISM / NET_CASE_OUTCOME_PENDING`
- Baseline: Berlin already has joint investigation structures; source does not prove which `GERen` are meant or current gaps. Treat as expansion of joint capacity, not creation from zero.
- Mechanism: stable multi-agency teams can reduce handoff/information friction and combine legal/operational competences for complex crime.
...
- `evidence_level = MEDIUM_FOR_DETECTION_RESPONSE_MECHANISM / PREVENTION_EFFECT_CONTEXT_DEPENDENT`
- Baseline/additionality: ASOG already permits video at kbO. The source adds/conditions a broader or more explicit parliamentary/staffed-real-time use; exact delta to current deployment must be documented before claiming added coverage.
- Potential mechanism: real-time observation can improve incident detection/dispatch and recordings can support evidence; deterrence is offence/place dependent and not assumed.
- Rights guards: demonstrable place-specific threshold, necessity/proportionality, limited field/use, access logging, retention/deletion, oversight, transparency and no automated identity inference unless separately lawful/reviewed.
...
Version both existing atoms:
- `BE-BSW-P41-U08-A01-f7dc6a88c4d6`
- `BE-BSW-P41-U08-A02-ab18a7d736ec`

to `SOURCE_UNIT_RECLASSIFIED_VERSIONED / MALFORMED_SENTENCE_SEGMENTATION_SUPERSEDED / counts_as_effect_object=false`.

Re-segment complete exact source unit `BE-BSW-P41-U08-9f297781fd9e` into the smallest coherent technology-specific interventions, preserving the exact limiting scope `ausschließlich ... organisierte Kriminalität und Straftaten mit Extremismus- bzw. Terrorismusbezug`. At minimum separate Quellen-TKÜ from KI if exact grammar permits; do not invent specific AI functions (face recognition, prediction, biometrics etc.) not in source.
...
- `BE-BSW-P41-U13-A03-a2830e9966f7` → `NON_EFFECT_SECURITY_FREEDOM_BALANCE_OUTCOME_CLAIM_REVIEWED`.

### 11. P41 result

After this handoff:
- all clean P41 originals are terminal reviewed/noncounting, explicitly Fach-approved or exact RNAA;
- remaining finite technical residuals on P41 are:
  - malformed/compound resegmentation of `BE-BSW-P41-U08-9f297781fd9e` (Quellen-TKÜ vs KI, preserving target limitations);
...
Extend the GitHub-only source-order queue through **P41**. Current exact main remains `b0209e26bbc93d89070bc89e0c83df7d4ab0269f` materialised through P22; consume P23→P40 idempotently, then this batch. Generate only deterministic child/repair IDs, hashes and exact text spans explicitly required by Fach. Preserve current-law/additionality, privacy, due-process and source-claim guards. Post generated exact children + fresh SET-WISE residual to #240/#241. No technical Fach/DNS/Recommendation/score synthesis. Run established Berlin/BSW/#241/Golden-readiness/Source-vs-View/SamePage/tests/typecheck/lint/build/privacy/link/accessibility/responsive gates. **No Vercel action and no owner RC request.**
