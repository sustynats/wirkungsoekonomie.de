...
### 1. Current baseline / evidence guards

- Berlin's 2025 PKS is mixed, not a blanket rising-violence baseline. Recorded `Messerangriffe` were 3,599 in 2025 (+5.5%), but the police explicitly notes a 2025 recording-method change and that the significant increase was especially in threats. Use offence-/context-specific data. Official source: `https://www.berlin.de/sen/inneres/presse/pressemitteilungen/2026/pressemitteilung.1651060.php` and PKS short report.
- Berlin already operates violence-prevention structures through the Landeskommission Berlin gegen Gewalt and district/school programmes; expansion proposals therefore require programme-specific additionality rather than a zero baseline. Official source: `https://www.berlin.de/lb/lkbgg/`.
- Current public-space policing already includes geographically targeted instruments; seven areas have been designated kriminalitätsbelastete Orte since 01.07.2026. Source: `https://www.berlin.de/polizei/polizeimeldungen/fakten-hintergruende/artikel.1078268.php`.
...
- `evidence_level = MEDIUM_FOR_EARLY_SUPPORT_AND_SCHOOL_LINKAGE_MECHANISM / VIOLENCE_AND_EDUCATIONAL_OUTCOME_PENDING`
- Competence: `LAND_BERLIN_SCHOOL_YOUTH_AND_PERSONNEL_POLICY_WITH_DISTRICT_PROVIDER_DELIVERY`.
- Mechanism: additional qualified school-social-work capacity can identify psychosocial/conflict needs earlier, mediate, connect families/youth to services and reduce escalation where adequate referral capacity exists.
- Risks: role substitution for teachers/psychologists/youth welfare; high caseloads; insufficient specialist services; documentation burden; school-level inequality; treating social work as police adjunct rather than youth support.
...
- `terminal_fach_state = EXPLICIT_FACH_APPROVED`
- `impact_direction = AMBIVALENT_IMMEDIATE_SCHOOL_ACCESS_AND_INCIDENT_DETERRENCE_POTENTIAL / RISK_THRESHOLD_ROLE_SAFEGUARD_AND_SCHOOL_CLIMATE_DEPENDENT`
- `evidence_level = MEDIUM_FOR_PRESENCE_AND_ACCESS_CONTROL_MECHANISM / NET_SAFETY_AND_EDUCATIONAL_OUTCOME_CONTEXT_DEPENDENT`
- Competence: `LAND_BERLIN_SCHOOL_SAFETY_AND_PROCUREMENT_WITH_SCHOOL_DISTRICT_POLICE_DEPENDENCIES`.
- Strong design conditions already present in source: proven high risk, temporary use, civilian appearance, financed provision. These reduce but do not remove risks.
- Mechanism: temporary trained security presence can improve entry monitoring, incident detection/response and perceived safety during acute risk periods.
- Risks: unclear authority/use-of-force role, private-security quality, discriminatory profiling, escalation, normalisation of securitisation, displacement around school perimeter, stigma and adverse school climate.
...
- `evidence_level = MEDIUM_FOR_TRAINING_TO_CAPABILITY_MECHANISM / HARM_OUTCOME_PENDING`
- Mechanism: role-specific training can improve recognition, evidence preservation, safeguarding/referral and appropriate response to cyberbullying/online offences.
...
- `evidence_level = MEDIUM_FOR_INTENSIVE_CASE_SUPPORT_MECHANISM / RECIDIVISM_AND_WELLBEING_OUTCOME_PENDING`
- Mechanism: intensive stable casework addressing family, school, mental health, peer, addiction/housing and service needs can reduce unresolved drivers and improve reintegration when services exist.
- Risks: `Mehrfachtäter` label/stigma; coercive support; privacy/data sharing; net-widening; insufficient specialist services; selection bias.
...
- `evidence_level = MEDIUM_HIGH_FOR_VISIBILITY_RESPONSE_MECHANISM / NET_CRIME_AND_SAFETY_OUTCOME_PLACE_DEPENDENT`
- Baseline: Berlin already uses geographically targeted policing/kbO; additionality requires actual marginal patrol hours/deployment.
- Mechanism: visible targeted patrol can shorten response/deter some place-based offending and increase reassurance; benefits depend on problem type/time and may displace activity.
...
Split deterministically from exact text into independently governable levers, at minimum:
1. city/public-space lighting expansion / removal of dark locations;
2. urban-design crime-prevention/CPTED intervention.
Preserve place examples with the appropriate child. No child Fach by CodeX.

Later child guards: actual location-specific safety problem; pedestrian/accessibility benefit; energy/lifecycle cost; light pollution/biodiversity/sleep effects; glare/visibility quality; design accessibility; displacement; usage and crime/safety outcomes. More light must not automatically equal more safety.
...
Split deterministic children from exact text:
1. expand women-shelter offer/capacity;
2. multilingual information/signposting on help offers via doctors/authorities/schools.
No child Fach by CodeX.

Later Fach for shelter child: distinguish already planned/current capacity from additional places; occupancy/turn-away, geographic/accessibility/special-needs capacity, staffing, security, children, move-on housing and lifecycle funding. For information child: reach, languages, confidentiality/safe communication, staff training, referral completion; information is not shelter capacity.

### 11. P40 exact result

After this handoff:
- all clean original P40 records are terminal reviewed/noncounting, explicitly Fach-approved or exact RNAA;
- compound/versioned residual parents requiring deterministic children are:
  - `BE-BSW-P40-U02-A01-fe9936e8a91c`
  - `BE-BSW-P40-U08-A02-9237737a437e`
  - `BE-BSW-P40-U10-A02-8a8f6d0d9348`
  - `BE-BSW-P40-U12-A02-b6563e374ce6`
...
Extend the GitHub-only source-order queue through **P40**. Current exact main already materialises P22 as `b0209e26bbc93d89070bc89e0c83df7d4ab0269f`; consume P23→P39 idempotently from authoritative #240 comments, then this batch. Generate only deterministic child/repair IDs, hashes and exact text spans explicitly required by Fach. Preserve every cross-page/parent/supersession relation and current-law/additionality guard; post generated exact children + fresh SET-WISE residual to #240/#241. No technical Fach/DNS/Recommendation/score synthesis. Run established Berlin/BSW/#241/Golden-readiness/Source-vs-View/SamePage/tests/typecheck/lint/build/privacy/link/accessibility/responsive gates. **No Vercel Preview/build/deploy/promotion and no owner RC request.**
