...
A further First rbb Amendment Treaty was approved by the Berlin Senate on 02.06.2026 to carry forward the broader public-broadcasting reform:
`https://www.berlin.de/rbmskzl/aktuelles/pressemitteilungen/2026/pressemitteilung.1676514.php`

Berlin also approved the broader Länder Reformstaatsvertrag path for public broadcasting; this is an **interstate** media-governance architecture, not a field Berlin can redesign unilaterally:
`https://www.berlin.de/rbmskzl/aktuelles/pressemitteilungen/2025/pressemitteilung.1525304.php`

The current household broadcasting contribution remains **€18.36/month** in 2026; contribution level changes require the Länder legal process and are tied to the constitutionally protected functional-financing framework:
`https://www.rundfunkbeitrag.de/presse-und-aktuelles/presse/neues-sommersemester-was-studierende-und-auszubildende-jetzt-zum-rundfunkbeitrag-wissen-sollten`

Current rbb practice also already expands regional live reporting in 2026, so P30's regionality demand must not be assessed against a zero baseline:
`https://www.rbb24.de/panorama/beitrag/2026/05/rbb-staerkt-regionale-live-berichterstattung.html`

The Electronic Media School (ems) is active and 100% owned by rbb; its 15th journalistic traineeship cohort is running and the school reports established training/placement structures:
...
- `terminal_fach_state = NON_EFFECT_STRUCTURAL_AND_NORMATIVE_HEADING_REVIEWED`
...
- `A02-ca0e26c41b8a` → `NON_EFFECT_NORMATIVE_ENTITLEMENT_CLAIM_REVIEWED`
- `A03-b701082eed6f` → `NON_EFFECT_RBB_PERFORMANCE_SOURCE_CLAIM_REVIEWED`
- `A04-5f83346503d8` → `NON_EFFECT_REFORM_VS_CONTRIBUTION_FRAME_REVIEWED`
...
- `A01-b8ff29d82a38` (`Noch höhere Beiträge?`) → `NON_EFFECT_RHETORICAL_QUESTION_REVIEWED`
- `A02-b4b8fa8c6f1d` (`Das kommt für uns nicht in Frage.`) → `NON_EFFECT_POSITION_RESTATEMENT_REVIEWED`
- `A03-57e28a19cb31` (`wirtschaftlicheres Handeln ... Spielräume`) → `NON_EFFECT_EFFICIENCY_OUTCOME_SOURCE_CLAIM_REVIEWED`
...
`A01-1fc0955a0d88` is the reform/distribution goal and is non-counting context for A02:
...
- `evidence_level = HIGH_FOR_DIRECT_HOUSEHOLD_PAYMENT_EFFECT / LOW_FOR_NET_MEDIA_AND_DEMOCRATIC_OUTCOME_WITHOUT_FINANCING_DESIGN`
- Competence: `INTERSTATE_LAENDER_RUNDFUNKBEITRAG_ARCHITECTURE / BERLIN_NEGOTIATION_RATIFICATION_ROLE_NOT_UNILATERAL_RATE_SETTING`.
- Direct mechanism: reducing €18.36 toward €10 for an eligible household lowers its mandatory cash outflow; the same reform creates a revenue gap unless eligibility, other rates, transfers, efficiency savings or another financing source compensates it.
- Missing: exact definition of `untere und mittlere Haushaltseinkommen`, household/means-testing rule, take-up/admin design, revenue loss, compensation/savings plan, constitutional functional-financing check.
- Distribution: strongest direct benefit for eligible lower-income households relative to disposable income; administrative exclusion/non-take-up risk.
...
`A02-fe45488c6b16` and `A03-2c0077125967`
...
- Current-baseline guard: the 2024 rbb treaty already caps the Intendant salary; this proposal's additional effect therefore depends on which other leadership posts/rates are targeted.
...
- Mechanism: role/function/pay review can identify duplication or unjustified cost; review itself is output and may equally conclude a role is necessary.
...
- Competence: mixed treaty/governance/employer/collective-bargaining domain; do not render as a unilateral Berlin payroll order.
...
U10:
- `A01-ff3f36d8fbd5` → `NON_EFFECT_REGIONAL_PUBLIC_SERVICE_ROLE_CLAIM_REVIEWED`
- `A02-e805935e4472` → `NON_EFFECT_REGIONALITY_GOAL_REVIEWED`
- `A03-78fbfd88341f` → `NON_EFFECT_CONTESTED_RBB_LITIGATION_SOURCE_CLAIM_REVIEWED`
- `A04-8850e5fd3081` → `NON_EFFECT_CURRENT_LEGAL_IMPLEMENTATION_LIFECYCLE_GOAL_REVIEWED`
...
- Baseline: EMS is already active, rbb-owned and training cohorts; `stärken` needs an additional capacity/access delta to create impact.
- Risks: resources without wider access, selection capture, training quantity over quality.
- Reality check: additional funded places/courses, applicant/admit diversity, completion, placements, quality and access barriers.
...
- Trade-off: fewer total placements if budgets are fixed; monitor placement count and quality as well as pay.

`BE-BSW-P30-U11-A03-a20cbfd937a5` (`Ausbildungsangebot stärken`)
- `NON_EFFECT_TRAINING_CAPACITY_GOAL_RESTATEMENT_REVIEWED`; covered by A01/A02 concrete instruments.

### 10. Cross-page repair: P30 U11 A04 + P31 U01 A01

P30 ends with:
`BE-BSW-P30-U11-A04-0fe6b1f98458` = `Das sorgt für mehr soziale Vielfalt in den Redaktionen`

P31 begins with:
`BE-BSW-P31-U01-A01-56f788d33d29` = `und damit für mehr Repräsentation der Bevölkerung und weniger Bubble-Effekte in den Redaktionen.`

Version both fragments to zero-count superseded states and generate one deterministic cross-page semantic record with exact combined text:
...
- `terminal_fach_state = NON_EFFECT_EXPECTED_DIVERSITY_REPRESENTATION_OUTCOME_CLAIM_REVIEWED`
- `counts_as_effect_object = false`
- Guard: widening training access can plausibly alter recruitment composition, but the claimed chain to population representation/editorial perspective and fewer `bubble effects` is not automatically established. Measure outcomes separately.
- Structural use of P31 A01 does **not** close P31.

### 11. P30 terminality

After all above plus the P30→P31 repair:

`BE_BSW_P30_FACH_COMPLETE = PASS_SOURCE_BOUND`

P30 active effect-review set is restricted to:
- €10 contribution target for defined lower/middle-income households (ambivalent system/distribution effect);
- leadership-pay reduction;
- review of highly paid roles;
- TVöD-oriented pay-structure proposal;
- EMS strengthening;
- three separated early-career access/work instruments.

Broad values, diagnoses, rhetorical questions, generic slimming, regionality goals, treaty-compliance lifecycle text and claimed outcomes count zero or RNAA exactly as above.

If P22→P29 are correctly materialised first, protected BSW Fach scope can extend through P30. Next untouched Fach page is **P31**, excluding only its first cross-page continuation fragment. Recompute residual SET-WISE. BSW remains programme-open; Berlin remains `3/12 terminal / 9/12 open`; MV/combined Golden State remain fail-closed.

### CodeX handoff

Extend the pending source-order queue to `P22 → P23 → P24 → P25 → P26 → P27 → P28 → P29 → P30`. Generate only deterministic child/merge/cross-page IDs/hashes, preserve current-law/additionality guards and materialise Fach verbatim. Post exact generated IDs/text hashes + SET-WISE residual to #240/#241. No technical Fach/DNS/Recommendation/score synthesis. GitHub-only; no Vercel Preview/build/deploy/promotion and no owner RC request.
