...
### 1. Current baseline / evidence guard
...
### 2. Mandatory P38→P39 context repair

P38 `BE-BSW-P38-U10-6768b5e7c7cb` ends mid-diagnostic sentence and P39 `BE-BSW-P39-U01-8cc1f9a0fa79` continues it (`... Beamtinnen und Beamten in Pension ...`). These are programme diagnosis/context, not an intervention.

Version both physical fragments zero-count and generate one deterministic cross-page diagnostic record from their exact concatenated stored text:
- `terminal_fach_state = NON_EFFECT_STAFFING_CAPACITY_DIAGNOSIS_REVIEWED`
- `source_span = physical PDF P38→P39`
...
- `evidence_level = HIGH_FOR_HIRING_TO_HEADCOUNT_MECHANISM / OPERATIONAL_SERVICE_OUTCOME_PENDING`
- Competence: `LAND_BERLIN_PUBLIC_EMPLOYER_POLICE_AND_FIRE_SERVICE_RECRUITMENT_WITH_BEAMTENRECHT_AND_SERVICE_REQUIREMENTS`.
- Baseline/additionality: recruitment/career rules already changed in 2026; this proposal is assessable as additional hiring volume and selection design.
- Mechanism: more successfully recruited and trained personnel can replace retirements/attrition and increase staffed operational capacity.
- Hard guards: job-related transparent aptitude criteria; equal opportunity/non-discrimination; selection validity; training-school capacity; retention after hiring; no false trade-off between merit and lawful diversity/equal-opportunity measures.
...
- `evidence_level = HIGH_FOR_DIRECT_COMPENSATION_INCENTIVE_MECHANISM / RETENTION_MAGNITUDE_PENDING`
- Baseline: Berlin already enacted 2026/27 pay increases; Fach does not assume a current federal-vs-Berlin gap amount from the programme. Exact comparison by grade/allowance is required before costing.
- Mechanism: if a material pay gap exists, narrowing it can reduce financial incentives to leave and improve recruitment competitiveness.
...
- `terminal_fach_state = EXPLICIT_FACH_APPROVED`
- `impact_direction = AMBIVALENT_FRONTLINE_CAPACITY_REALLOCATION_POTENTIAL / SUPPORT_BOTTLENECK_AND_FUNCTIONAL_NEED_DEPENDENT`
- `evidence_level = HIGH_FOR_DIRECT_FTE_ALLOCATION_MECHANISM / NET_SAFETY_AND_CASE_OUTCOME_DEPENDENT`
- Mechanism: directing genuinely additional deployable FTE toward patrol/investigation/prevention can increase those functions' capacity.
- Risks: crowding essential dispatch, analysis, HR, IT, procurement, training or case administration; prevention/investigation/patrol compete for different skills; `additional` must be net of attrition.
...
Version **both** existing atoms zero-count:
`SOURCE_UNIT_RECLASSIFIED_VERSIONED / MALFORMED_SENTENCE_SEGMENTATION_SUPERSEDED`.

Re-segment the complete exact source unit `BE-BSW-P39-U06-302b35e885e2` deterministically into smallest coherent interventions. At minimum the semantic domains are:
1. higher allowances for especially burdensome duties, preserving examples as examples rather than effects;
2. mandatory/effective psychosocial follow-up/support;
3. flexible work models / home office where suitable for admin/analysis work.

Use the exact source grammar, not this paraphrase, to establish child boundaries. Return exact child IDs/text spans to #240. No child Fach by CodeX.
...
- `terminal_fach_state = EXPLICIT_FACH_APPROVED`
- `impact_direction = POSITIVE_ADMINISTRATIVE_PRODUCTIVITY_AND_MOBILE_WORK_POTENTIAL / PROCESS_REDESIGN_INTEROPERABILITY_CYBER_AND_VENDOR_DEPENDENT`
- `evidence_level = MEDIUM_HIGH_FOR_DIGITAL_PROCESS_FRICTION_MECHANISM / REALISED_TIME_SAVING_PENDING`
- Mechanism: digitised end-to-end processes can reduce duplicate entry/wait/hand-offs and make case/procurement processing faster **if** process rules are redesigned rather than merely digitised.
- Risks: vendor lock-in, cyber/privacy, parallel paper/digital burden, bad automation, procurement-law constraints, frontline technology stealing time rather than saving it.
...
- `evidence_level = MEDIUM_FOR_TRAINING_AND_TACTICAL_BEHAVIOUR_MECHANISM / FIELD_OUTCOME_PENDING`
- Baseline: Berlin Police says de-escalation is already a long-developed assembly strategy. Proposal is additional training/consistency, not invention from zero.
- Mechanism: scenario-based training + operational doctrine/supervision can increase appropriate communication/tactical de-escalation and reduce avoidable escalation/use-of-force where safe and lawful.
...
- `evidence_level = HIGH_FOR_PROCESS_AND_EVALUATION_MECHANISM / NET_FIELD_OUTCOME_PENDING`
- Mechanism: documented consistent application + after-action evaluation can expose differential practice, identify failures and feed operational learning.
...
- `evidence_level = HIGH_FOR_ACCOUNTABILITY_INCENTIVE_MECHANISM / MISCONDUCT_AND_TRUST_OUTCOME_PENDING`
- Competence: `LAND_BERLIN_DISCIPLINARY_AND_POLICE_EMPLOYER_GOVERNANCE_SUBJECT_TO_BEAMTEN_DISCIPLINARY_LAW_AND_JUDICIAL_REVIEW`.
- Mechanism: timely credible investigation and proportionate sanctions for proven disproportionate force can strengthen accountability/deterrence and institutional learning.
...
After the P38→P39 repair and this handoff:
- clean P39 records are terminal reviewed/noncounting, explicitly Fach-approved or exact RNAA;
- `U04-A02` and `U08-A01` are compound parents pending deterministic child IDs;
...
Extend the GitHub-only source-order queue through **P39**. Consume P22→P38 idempotently, then this batch. Generate only the deterministic child/repair IDs, hashes and exact spans explicitly required above. Preserve P38→P39 lineage, legal/rights guards and zero-count rationale records. Post exact generated set + fresh SET-WISE residual to #240/#241. No technical Fach/DNS/Recommendation/score synthesis. Run established Berlin/BSW/#241/Golden-readiness/Source-vs-View/SamePage/tests/typecheck/lint/build/privacy/link/accessibility/responsive gates. **No Vercel action and no owner RC request.**
