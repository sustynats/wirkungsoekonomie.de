...
```bash
python3 -m venv .wwk-venv
.wwk-venv/bin/pip install -r content/wirkungswahl-kompass/requirements-content.txt
.wwk-venv/bin/python content/wirkungswahl-kompass/validate_content.py content/wirkungswahl-kompass/real-content.json
...
.wwk-venv/bin/python scripts/wirkungswahl-kompass/build-preview.py
```

Die optionale Browserprüfung braucht zusätzlich Playwright und Chromium:

```bash
.wwk-venv/bin/pip install -r tests/wirkungswahl-kompass/requirements-test.txt
.wwk-venv/bin/playwright install chromium
.wwk-venv/bin/python tests/wirkungswahl-kompass/browser_audit.py
```
