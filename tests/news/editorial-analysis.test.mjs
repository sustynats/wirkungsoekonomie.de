...
test("Backfill publiziert jeden relevanten Kandidaten bis zur technischen Batchgrenze und ist idempotent", async () => {
  ...
  assert.match(stored.analyses[0].transparency_note, /Automatisiert/);
...
test("Generator bindet Portrait, eigenständige Route, Rücklink, RSS und gemischten Feed ein", () => {
  ...
  const analysis = { analysis_id: "analysis-render", story_id: story.story_id, slug: "render-analysis", published_at: "2026-09-05T10:00:00Z", updated_at: "2026-09-05T10:00:00Z", reading_time_minutes: 8, transparency_note: "Automatisiert nach der von Natalie Weber entwickelten Methodik der Wirkungsökonomie erstellt.", source_snapshot: story.sources.map((source) => ({ ...source, source_id: editorialSourceRef(source) })), ...validEditorial(story) };
