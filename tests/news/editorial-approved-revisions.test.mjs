...
test('personal factual correction previews and imports only after fresh approval, preserving the published edition',async t=>{
 ...
 const patch={body_markdown:base.body_markdown.replace('Gesellschaftliche Wirkung','Kommunikatives Wirkungspotenzial'),correction_note:'Die kommunikative Resonanz wird als Potenzial bezeichnet.',subtitle:'Der von der Autorin korrigierte Vorspann ersetzt auch die alte Vorschau.'};
 const target={analysis_id:base.analysis_id,slug:base.slug,base_hash:editorialRevisionBaseHash(base)};
 const preview={format:base.subtype,title:base.title,subtitle:patch.subtitle,markdown:patch.body_markdown,sources:base.sources,checks:{source_binding:true,editorial_validation:true,personal_experiences_invented:false},editorial_revision:{base,target,patch}};
 ...
 const mismatched=structuredClone(preview);mismatched.subtitle=base.subtitle;
 assert.throws(()=>s.stage(job,mismatched),/REVISION_INVALID/);
 const forged=structuredClone(preview);forged.editorial_revision.patch.sources=[];
 assert.throws(()=>s.stage(job,forged),/REVISION_INVALID/);
 const staged=s.stage(job,preview);assert.equal(s.claimPublications().length,0);
 const altered=structuredClone(preview);altered.subtitle+=' Unfreigegebene Änderung.';
 altered.editorial_revision.patch.subtitle=altered.subtitle;
 assert.notEqual(editorialPreviewHash(altered),staged.preview_hash);
 ...
 assert.equal(updated.subtitle,patch.subtitle);assert.equal(updated.teaser,patch.subtitle);
 assert.ok(html.includes('<p class="hero-subtitle">'+patch.subtitle+'</p>'));
 assert.ok(html.includes('name="description" content="'+patch.subtitle+'"'));
