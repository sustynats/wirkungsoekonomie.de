...
test('the ticker workflow is a single serialized lane without bridge phases or clock triggers', () => {
  ...
  // The Oracle clock branch is the documented fallback for GitHub's unreliable cron;
  // the bridge-era import clock (five-minute Dropbox poll) stays retired.
  assert.match(ticker, /^  push:\n(?:    #[^\n]*\n)*    branches:\n      - codex\/wirkungsticker-clock\n/m, 'Oracle clock as fallback trigger');
  assert.doesNotMatch(ticker, /codex\/wirkungsticker-import-clock/, 'no bridge import clock');
