...
## 2026-09-16 - Claude: Zwei gemessene Ursachen: halbe Nachlieferung und der Verlagsname als Zahl

- **Was:** Die ersten Läufe mit den neuen Vorgaben (11:50 und 12:05 UTC) brachten zwei von drei und eine von drei Meldungen live, bei 2,3 bzw. 4,3 Cent je veröffentlichte Meldung. Die Halte-Gründe habe ich in den bezahlten Antworten nachgelesen, nicht geraten, und zwei Ursachen gefunden. (1) Die eine erlaubte Nachlieferung gab dem Bewertungsobjekt Vorrang; ein Textbefund kam nur dran, wenn die Bewertung vollständig war. Fehlte beides, bekam der Text nie einen Versuch, obwohl der Aufruf noch nicht verbraucht war. `combinedRepairFormat` fragt jetzt beides in einem Aufruf nach, schemagebunden auf story_id, impact_assessment und die betroffenen Textfelder (70 kB, innerhalb der Anbietergrenzen); für ein offenes Objekt wie `media_impact` bleibt der freie Modus. (2) Die beanstandete Zahl in `CLAIM_NUMBER_NOT_IN_EVIDENCE` war die 24 in „laut France 24" — in drei Aussagen derselben Meldung, eine davon „Die Überschrift des France-24-Berichts". Die Prüfung der Lesetexte nimmt Verlagsnamen längst heraus, die Prüfung der Aussagen tat es nicht; jetzt teilen beide `withoutPublisherNames` in `numeric-evidence.mjs`, auch für Schreibweisen mit Bindestrich, und nur Namen mit Ziffer werden betrachtet. Eine echte Zahl daneben bleibt beweispflichtig.
- **Beobachtet:** Sechs persönliche Ausgaben sind heute live gegangen, alle durch Natalies Freigabe in der App. Ich habe den Weg geprüft, weil die Lanz-Ausgabe 56 Sekunden nach Auslieferung des Entwurfs live war: `APPROVED_FOR_PUBLICATION` setzt ausschließlich `decide(APPROVE)` mit ihrem Konto und dem Hashwert genau dieser Fassung, und `publishable()` verlangt zusätzlich `manual_only`. Der Weg ist dicht.
- **Geprüft:** `tests/news/direct-operation.test.mjs` 35/35, `tests/news/newsroom.test.mjs` und `tests/news/numeric-evidence.test.mjs` 38/38, `npm run news:test` ohne neue Fehler. PRs #826, #827.
- **Lehre:** Dieselbe Fehlerklasse zweimal am selben Tag: ein Medienname in einer Prüfung, die über Inhalte urteilt (morgens „Deutschlandfunk" als Ländername, mittags „France 24" als Zahl). Wo eine Prüfung Namen von Quellen sieht, muss sie sie vorher herausnehmen — und zwar an einer Stelle, die alle Prüfungen teilen.
