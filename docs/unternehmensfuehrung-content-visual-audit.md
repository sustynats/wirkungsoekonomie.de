# Wirkungsorientierte Unternehmensführung: Content- und Visual-Audit

Stand: 22. Mai 2026

## Anlass

Die Seite `/fuer/unternehmen.html` wirkte noch zu kurz, zu ESG-/Reporting-nah und zu wenig aus der Führungslogik der Wirkungsökonomie entwickelt. Außerdem hatte das bisher eingebundene Roadmap-PNG `woek_08_unternehmen_change_roadmap` ein sichtbares Headline-Layoutproblem.

## Eingearbeitete Grundlagen

- `Die neue Ordnung des Wohlstands`, insbesondere Teil V: Unternehmen, Märkte und Kapital
- Führender Begriffsleitfaden der Wirkungsökonomie
- Systemmodell der Wirkungsökonomie
- LinkedIn-Grundartikel `Wirkungsorientierte Unternehmensführung` von Natalie Weber
- bestehende Website-Inhalte zu Resilienz, Wertschöpfung, Wirkungskapital, T-SROI, Wirkungslast und Wirkungsarchitektur

## Inhaltliche Korrekturen

- Die Seite beginnt jetzt bei der Maßstabskrise der klassischen BWL: Umsatz, Marge, Kapitalrendite, Effizienz und KPI messen Bewegung, aber nicht Richtung.
- Der Text erklärt klarer, dass wirkungsorientierte Unternehmensführung die betriebswirtschaftliche Konsequenz einer wirkungsökonomischen Ordnung ist.
- ESG, CSRD und Reporting werden als Anschlussräume behandelt, nicht als Primärlogik.
- Führung wird als Systemverantwortung beschrieben: Wirkungstransparenz, Regeln, Lernschleifen, Mitarbeitende, Kultur und Entscheidungsarchitektur.
- Resilienz in der Wertschöpfung wurde vertieft: physische Realität, ökonomische Realität, soziale Realität und demokratische Realität.
- Der Change-Prozess wurde neu formuliert: Maßstab klären, Wirkungspfade lesen, Wertschöpfung kartieren, Daten/WÖk-ID anschließen, Wirkungslast bestimmen, Strategie/Portfolio/CAPEX priorisieren, Führung und Mitarbeitende befähigen, Pilotierung und Rückkopplung verankern.
- Die Missverständnisbox wurde korrigiert; die vorher wiederholten Standardtexte wurden durch spezifische Unternehmensabgrenzungen ersetzt.

## Visual-Korrekturen

- Das alte PNG `woek_08_unternehmen_change_roadmap` ist auf der Seite nicht mehr primär eingebunden.
- Neues kontrolliertes SVG: `/assets/visuals/model/woek_unternehmensfuehrung_roadmap_v2.svg`
- Mobile Variante: `/assets/visuals/model/woek_unternehmensfuehrung_roadmap_v2_mobile.svg`
- Das neue Visual zeigt:
  - alte Steuerung: Umsatz, Marge, KPI, Effizienz, Bericht
  - WÖk-Referenz: Wirkung als tatsächliche Zustandsveränderung
  - neue Führung: Strategie, Geschäftsmodell, Kapital, Lieferketten, Kultur, Mitarbeitende
  - Change-Prozess mit Maßstab, Wirkungspfaden, Wertschöpfung, Daten, WÖk-ID, Wirkungslast, Strategie, Führung, Mitarbeitenden, Pilot/Change und Rückkopplung

## Registry

- `content/visuals/visual-source-registry.json` ergänzt um `woek_unternehmensfuehrung_roadmap_v2`
- `docs/visual-registry.md` aktualisiert
- `woek_08_unternehmen_change_roadmap` als superseded markiert

## Offene Übertragung auf weitere /fuer/-Seiten

Die Qualitätslogik gilt für alle Zielgruppenseiten: nicht Nutzenmarketing, sondern systemische Übersetzung der Wirkungsökonomie. Die Unternehmensseite ist jetzt der präzisere Referenzfall für Führung, Change, Wertschöpfung und Organisation.
