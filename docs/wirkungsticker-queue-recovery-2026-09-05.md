...
- Der erste echte Prüflauf `33992426418` schloss erfolgreich ab: vier lokale Altlasten erledigt, ein exakter HTTP-Alias bereinigt, eine Meldung veröffentlicht. Er fand zusätzlich zwei inzwischen hinzugekommene Quellen im großen Wahlkomplex (17 statt 15). Deshalb wurde der Regressionstest um diese realen Neuzugänge plus nachgeladene Textauszüge erweitert. Häufige Metadaten werden nun mit expliziten abweichenden Werten verlustfrei ausgelagert; wiederholte kurze Zeichenketten nur bei tatsächlicher Platzersparnis. Null, fehlende Felder, Interessenrollen und abweichende Provenienz bleiben unterscheidbar. Redundante Promptformulierungen wurden ohne Gate-Änderung gekürzt.
...
- Der zweite echte Prüflauf `33993005575` schloss ohne Eingabegrößen- oder Quellenintegritätsblockaden ab. Restbestand: 14 Meldungen unter der vorsorglich erhöhten Relevanzschwelle der Budgetstufe 1; drei fehlerhafte Ausgaben mit automatischen Wiederholungsterminen. Budgetpriorisierung wird deshalb als `AI_BUDGET_BLOCKED` statt als bloße Batch-Wartezeit protokolliert. Die 25-Euro-Obergrenze und die Budgetstufen wurden nicht geändert; eine Änderung der vorzeitigen Drosselung wurde separat zur Entscheidung gestellt.
