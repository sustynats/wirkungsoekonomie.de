...
3. `evidence-packets.mjs` und `fitAnalysisInput()` referenzieren identische Textteile verlustfrei. `excerpt_from:[field,start,length]` bezieht sich auf denselben Quellenkurztext/-titel, `excerpt_text` auf einen story-lokalen Textkatalog. Gemeinsame Metadaten wie Quellenregister-ID, Herausgeber, Rolle und Provenienz stehen einmal in Defaults; jede URL und jedes Dokumentdatum bleibt separat erhalten. Quellen- und Beleg-IDs bleiben unverändert; `resolveEvidenceReferences()` bindet Modellantworten weiterhin an die Originalpassagen.
...
Eine weiterhin zu große Akte bleibt vollständig vorgemerkt (`AI_INPUT_TOO_LARGE`) und wird im Monitoring separat als Eingabeproblem ausgewiesen. Sie blockiert nicht die Slots anderer geeigneter Kandidaten. Technische Größenfehler werden nicht als abgeschlossene redaktionelle Prüfung gecacht: Der nächste geeignete Lauf packt sie mit der aktuellen verlustfreien Kompaktionslogik erneut. Auch große Mehrquellenakten behalten mindestens eine exakte Belegpassage je Quelle sowie sämtliche Identitäten, Rollen und Provenienzangaben. Dies ist ein sichtbarer Fehlerzustand, keine stillschweigende erfolgreiche Veröffentlichung und keine kostenpflichtige Endlosschleife.
