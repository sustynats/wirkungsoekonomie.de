...
3. Nur Primärquellen-Storys oberhalb der dynamischen Budgetschwelle gehen in einem kleinen Batch an die bestehende WÖk-KI. Quelleninhalte sind als `UNTRUSTED_SOURCE_DATA` gekapselt. Die KI darf nur gelieferte Claims verwenden. Der Relevanzfilter 3.0 verlangt drei unabhängig erfüllte Publikationsgates: eine tatsächlich neue Entwicklung, materielle mögliche Zustandsveränderungen und eine hinreichende Primärquellenbasis. Mindestens zwei Materialitätsfaktoren oder ein ausdrücklich begründeter außergewöhnlich starker Faktor müssen vorliegen. Reine Wiederholungen, Kontextstücke und Routinewerte ohne materielle Veränderung werden nicht veröffentlicht.
