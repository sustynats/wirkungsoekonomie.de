...
Die öffentliche Seite `/news/` ist die chronologische Deployment- und Veröffentlichungsübersicht der Wirkungsökonomie ab 2026. Sie ist nicht mit dem Wirkungsticker gleichzusetzen; der Wirkungsticker ist ein eigenständiges Produkt unter `/wirkungsticker/`.
...
`/news/` registriert keine Web-App und keinen Service Worker. Die installierbare App gehört ausschließlich zum Wirkungsticker und ist technisch auf `/wirkungsticker/` begrenzt.
...
Der Wirkungsbrief steht auf `/news/` nach der Chronik. Der RSS-Feed `/feeds/neuigkeiten.xml` enthält dieselben stabilen Update-IDs wie die Webseite.
