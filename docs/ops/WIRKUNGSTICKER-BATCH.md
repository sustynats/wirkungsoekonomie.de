...
### Zeitliche Zuordnung nach kostenloser Ablehnung (8. September 2026)

Ein lokales Kapazitäts-Nein kostet nichts und verbraucht keinen fachlichen Versuch. Wird derselbe idempotente Key später angenommen, darf dessen bezahlter Auftrag nicht auf das Datum der kostenlosen Ablehnung zurückdatiert werden. `started_at` bleibt deshalb unveränderte Journalhistorie. Das additive Feld `cost_started_at` führt den Beginn der konkreten Reserve bzw. des angenommenen Auftrags; `cost_started_at_basis` unterscheidet `local_reservation_created_at` und `provider_created_at` aus authentifizierten Transportmetadaten. Unbekannte oder ungültige Zusatzfelder fallen auf die historische Zuordnung zurück, niemals auf Nullkosten.

Kostenfenster, UTC-Monatsbudget, Stundenlimit und Berliner Tagesbericht verwenden dieselbe Zuordnung. Die bisherige Aufbewahrung schützt zusätzlich Kosten des aktuellen Monats, auch wenn die kostenlose Vorgeschichte älter ist. Eine erneute Annahme entfernt das alte Abschlussdatum aus dem aktiven Auftrag, erhält es aber mit bisherigem Zeitbezug, Status und Kosten in `batch_timing_history`. Wiederholtes Abholen eines fertigen Ergebnisses verschiebt dessen Abschlussdatum nicht weiter.

Der ohnehin vorhandene private Metadatenabruf ergänzt die Zeitzuordnung auch für bereits abgeschlossene oder angewendete Alt-Aufträge. Voraussetzung sind übereinstimmender Key, Story, Fingerprint, Prompt-Hash, Art, Versuch und Batch-Modus sowie ein plausibler Zeitstempel. Diese Reparatur ändert weder Kostenbeträge noch Anwendung, Quellen oder Artikelversionen. Es gibt keinen zusätzlichen POST, keine neue KI-Berechnung und keinen separaten Backfill-Job. Änderungen am bisherigen Zeitbezug bleiben im Journal nachvollziehbar. Details und Regressionen: [Batch-Kostenzeit-Reparatur](WIRKUNGSTICKER-BATCH-COST-TIME-2026-09-08.md).
