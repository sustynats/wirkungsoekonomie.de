...
export function ConsequenceCheck({ consequenceCheck }) {
  ...
  const hasRequiredEffectFields = (item) =>
    Boolean(item?.narrative && item?.mechanism && item?.impactPath && item?.dimension && item?.reason && item?.text);
  const narrativeItems = consequenceCheck.ifNarrativeWins.slice(0, 3).filter(hasRequiredEffectFields);
  if (narrativeItems.length < 3) return "";
  const narrativeRows = narrativeItems.map((item, index) => `<article class="card v3-order-card"><p class="v2-badge">${esc(orderLabels[index] || item.level)}</p><h3 class="card-title">${esc(item.dimension)}</h3><p class="card-text">${esc(item.text)}</p><p class="card-text"><strong>Narrativ:</strong> ${esc(item.narrative)}</p><p class="card-text"><strong>Wirkmechanismus:</strong> ${esc(item.mechanism)}</p><p class="card-text"><strong>Wirkungspfad:</strong> ${esc(item.impactPath)}</p><p class="card-text"><strong>Begründung:</strong> ${esc(item.reason)}</p></article>`).join("");
  const responseRows = consequenceCheck.ifCorrectlyHandled.slice(0, 3).filter((item) => item?.text).map((item) => `<article class="card v3-response-card"><p class="v2-badge">${esc(item.level)}</p><p class="card-text">${esc(item.text)}</p>${item.affectedSystems?.length ? chips(item.affectedSystems) : ""}</article>`).join("");
  const mpd = [
    ["Mensch", consequenceCheck.mpd?.mensch],
    ["Planet", consequenceCheck.mpd?.planet],
    ["Demokratie", consequenceCheck.mpd?.demokratie],
  ].filter(([, text]) => String(text || "").trim());
  const mpdBlock = mpd.length
    ? `<div class="card v3-mpd-risk-card"><p class="card-kicker">Risiken nach Mensch, Planet und Demokratie</p><div class="v3-mpd-risk-grid">${mpd.map(([label, text]) => `<div><strong>${esc(label)}</strong><span>${esc(text)}</span></div>`).join("")}</div></div>`
    : "";
  const woekText = consequenceCheck.woekAssessment || consequenceCheck.nonActionCost;
  return `<section class="section section-soft v3-layer v3-layer-consequences" id="folgencheck" data-v3-consequence-check><div><div class="section-header"><p class="hero-kicker">Wirkung statt bloßer Faktenprüfung</p><h2>Folgencheck: Was dieses Narrativ bewirkt</h2><p>Jede Wirkungskarte wird nur angezeigt, wenn Narrativ, Wirkmechanismus, Wirkungspfad, Dimension und Begründung vorhanden sind.</p></div><div class="card-grid three v3-consequence-orders">${narrativeRows}</div>${mpdBlock}<div class="card-grid two"><article class="card v3-check-column"><p class="card-kicker">Rote Linie / Unterlassungskosten</p><p>${esc(consequenceCheck.nonActionCost)}</p>${consequenceCheck.lockInRisk ? `<p><strong>Lock-in:</strong> ${esc(consequenceCheck.lockInRisk)}</p>` : ""}${consequenceCheck.feedbackLoop ? `<p><strong>Rückkopplung:</strong> ${esc(consequenceCheck.feedbackLoop)}</p>` : ""}</article><article class="card v3-check-column"><p class="card-kicker">Wirkungsökonomische Einordnung</p><p>${esc(woekText)}</p>${responseRows ? `<div class="card-grid">${responseRows}</div>` : ""}</article></div></div></section>`;
}
