import { request, el, button, renderResults, renderPoll, validLink, STATUS_LABELS } from './polls.mjs';
...
function drawList(){
  ...
  for(const p of visible){
    ...
    b.append(el('strong',p.title),el('span',`${STATUS_LABELS[p.effective_status]} · ${p.results.total} Stimmen · /umfragen/${p.slug}/`));
    li.append(b);list.append(li);
  }
  document.getElementById('poll-admin-count').textContent=`${polls.length} Umfragen insgesamt`;
}
...
async function drawFeedback(){
  ...
  async function page(offset=0){
    ...
    try{
      ...
      status.textContent=data.total?`${data.total} interne Rückmeldungen insgesamt`:'Noch kein Feedback eingegangen.';
