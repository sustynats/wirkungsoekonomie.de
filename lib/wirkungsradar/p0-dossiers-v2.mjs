...
function derivedFrameShiftPlaybook(item) {
  ...
  return {
    ...
    answerFormats: {
      ...
      medium30s: responses.medium30s?.text || responses.live?.text || item.sayThisNow,
      long2min: responses.long2min?.text || responses.panel?.text || `${item.sayThisNow} Entscheidend ist die ganze Bilanzgrenze: Fakten, Folgen, Alternativen, Unterlassungskosten und bessere Lösung.`,
