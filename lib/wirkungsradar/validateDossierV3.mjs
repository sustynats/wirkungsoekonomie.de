...
export function validateDossierV3(dossier, html = "") {
  ...
  if (html) {
    ...
    if (order[1][1] >= 0 && order[2][1] >= 0 && order[2][1] < order[1][1]) errors.push("Faktenlage steht vor Kurzantwort");
    if (order[4][1] >= 0 && order[1][1] >= 0 && order[4][1] < order[1][1]) errors.push("Theorie steht vor Antworten");
    if (!html.includes('id="host-antworten"')) errors.push("Host-Antworten fehlen im HTML");
    if (!/Kurzantwort - 10 Sekunden/.test(html)) errors.push("Kurzantwort - 10 Sekunden fehlt oben");
    if (!/Direkt nutzbare Antworten/.test(html)) errors.push("Antwortblock fehlt oder steht nicht sichtbar");
    if (/Live-Karten|Wirkungsradar-Live|Gute R(?:ü|ue)ckfrage|Host-Cockpit|v3 Antwortformat/.test(html)) errors.push("legacy public label noch sichtbar");
    const contentHtml = publicContent(html);
    const beforeAnswers = contentHtml.slice(0, Math.max(0, contentHtml.indexOf('id="host-antworten"')));
    if (/Wirkstoff|Wirkungspfad/.test(beforeAnswers)) errors.push("Methodikbegriff vor Antwortblock sichtbar");
